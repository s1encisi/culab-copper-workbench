...
def main():
    ...
    c.setFont("TestChinese", 21); c.drawString(48, 785, "合成 OCR 验证页")
    c.setFont("TestChinese", 12); c.drawString(48, 752, "仅用于软件测试，数值不代表工厂设置。")
    c.setFont("TestChinese", 15); c.drawString(48, 710, "一、浓度记录")
    ...
    for i, row in enumerate(rows):
        for j, value in enumerate(row): c.drawString(xs[j] + 12, ys[i] - 26, value)
    c.setFont("TestChinese", 11); c.drawString(48, 540, "表注：所有数值均为合成测试样例。")
    c.setFont("TestChinese", 15); c.drawString(48, 495, "二、表达式及符号")
    c.setFont("TestChinese", 18); c.drawString(62, 457, "c = m / V")
    c.setFont("TestChinese", 12); c.drawString(48, 425, "c 为质量浓度，m 为溶质质量，V 为溶液体积。")
    ...
    print(json.dumps({"fixture": str(args.output / "scan.pdf"), "reportlab": reportlab.Version,
