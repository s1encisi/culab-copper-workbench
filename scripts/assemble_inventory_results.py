...
if __name__ == "__main__":
    ...
    for identifier in args.comparison_id:
        ...
        assert record["status"] == "completed" and record["result"]["status"] == "completed"
        folder = service.directory(identifier)
        manifest, protocol = read_training(folder)
        ...
        uncertainty.extend(
            {**r, "seed": seed, "comparison_id": identifier} for r in record["result"].get("uncertainty_metrics", [])
        )
        sources.append(
            {
                "seed": seed,
                ...
                "status": "completed",
                "manifest_sha256": file_hash(folder / "training_manifest.json"),
    ...
    reference = service.get(args.reference_comparison_id)
    assert reference["status"] == "completed"
    folder = service.directory(args.reference_comparison_id)
    manifest, reference_protocol = read_training(folder)
    assert manifest["source"] == source_signature(data)
    ...
    old = pd.read_csv(folder / "oof_predictions.csv")
    ...
    expected = {
        "Persistence": [seeds[0]],
        **dict.fromkeys(methods, seeds),
        **{m: [reference_seed] for m in baseline_methods},
    }
    root = run_root / "model_inventory_studies" / digest(args.request_key)[:32]
    root.mkdir(parents=True, exist_ok=False)
    source_hashes = {
        (key if "/" in key else "src/copper_mvp/" + key): value for key, value in first["code_hashes"].items()
    }
    ...
    protocol = {
        "schema_version": "recovered-inventory-study.v1",
        ...
        "reference_comparison_id": args.reference_comparison_id,
        "source_comparisons": sources,
        "code_hashes": source_hashes,
        "recovery_note": (
            "Existing completed fits and predictions are reused after hash/conf"
            "iguration verification; no validation-based selection."
        ),
    }
    ...
    result.update(
        ...
        source_reference=args.reference_comparison_id,
        protocol_sha256=file_hash(root / "protocol.json"),
        predictions_sha256=file_hash(root / "predictions.csv"),
        recovery_assembler_sha256=file_hash(Path(__file__)),
        created_at=utc_now(),
    )
