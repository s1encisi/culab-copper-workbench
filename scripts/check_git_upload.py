...
RULES = {
    ...
    "provider-token": re.compile(
        r"(?<![A-Za-z0-9])(?:gh[pousr]_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]{20,}|sk-[A-Za-z0-9_-]{20,}|AKIA[0-9A-Z]{16})"
    ),
    "personal-absolute-path": re.compile(
        r"(?i)(?:[A-Z]:[/\\](?:Users|postgraduate1)|" + "/" + r"home/|" + "/" + r"Users/)"
    ),
    "embedded-auth-url": re.compile(r"https?://[^\s/@:]+:[^\s/@]+@"),
    "assigned-credential": re.compile(
        r"(?i)(?:api[_-]?key|access[_-]?token|password|client[_-]?secret)[\"\x27]?\s*[:=]\s*[\"\x27]([^\"\x27\n]{8,})[\"\x27]"
    ),
}
...
def path_reason(path):
    ...
    parts = parsed.parts
    if (
        path in PRIVATE_ROOT_FILES
        or (parts and parts[0] in PRIVATE_DIRS)
        or path.startswith("configs/contracts/")
        or any(
            p in {"node_modules", "__pycache__", ".pytest_cache"} or p.startswith((".venv", ".pytest_tmp"))
            for p in parts
        )
