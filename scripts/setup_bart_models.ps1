param([string]$Python = '../.analysis_work/venvs/copper-mvp/Scripts/python.exe')
...
& $Python -X utf8 -B -m pip install --disable-pip-version-check --no-deps --only-binary=:all: --require-hashes --index-url https://pypi.org/simple --target $BartTarget -r $BartRequirements
