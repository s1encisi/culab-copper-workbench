...
def main():
    ...
    for index, case in enumerate(plan["cases"]):
        ...
        if case["session"] not in sessions:
            session = call("/api/v2/sessions", {"title": "G3 真实验收 · " + case["session"], "context": case["context"]})
            sessions[case["session"]] = session["id"]
        task = call("/api/v2/sessions/" + sessions[case["session"]] + "/messages",
                    {"question": case["question"], "context": case["context"],
                     "request_key": "g3-live-20260911-" + str(index), "max_cost_cny": 0.5})
        ...
        write_json(output, result)
        print(json.dumps({"case": index + 1, "status": task["status"], "usage": task["usage"],
                          "model_calls": task["model_calls"], "tool_calls": task["tool_calls"]}), flush=True)
    ...
    write_json(output, result)
    print(json.dumps({"status": result["status"], "spent_cny": result["spent_cny"],
                      "unsettled_cny": result["unsettled_cny"]}), flush=True)
    return 0
