...
if __name__=="__main__":
    ...
    data=DataRepository();seeds=list(range(20260911,20260916))
    emit=lambda value:print(json.dumps(value,ensure_ascii=False),flush=True)
    root,result=run_inventory_study(data,args.run_root,args.reference_comparison_id,args.request_key,
        TABULAR_METHODS,seeds,progress=emit,max_wall_seconds=21600)
    if result["status"]!="completed":raise RuntimeError("Neural study incomplete; controls not started")
    emit({"neural_study":root.name,"status":result["status"]})
    control_root,control=run_matched_controls(data,args.run_root,root.name,args.request_key+"-mlp-controls",progress=emit)
