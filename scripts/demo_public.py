...
def scenario(root,ack_loss):
    ...
    if ack_loss:device.inject_fault({"ack_loss":True})
    link=DemoDeviceLink(device)
    service=CommandService(store,access,link,autostart=False)
    try:
        request={"request_key":"public-demo","reason":"Public synthetic 100 A to 105 A example",
        ...
        for label,actor,hash_value in [
            ...
            try:service.approve(actor,proposal["id"],{"payload_hash":hash_value})
            except WorkbenchError as error:blocked[label]=error.code
            else:raise AssertionError(label)
        service.approve(owner,proposal["id"],{"payload_hash":proposal["payload_hash"]})
        service.step()
        states=[service.get(owner,proposal["id"])["status"]]
        for _ in range(20):
            device.tick(1);service.step()
            result=service.get(owner,proposal["id"])
            if result["status"]!=states[-1]:states.append(result["status"])
            if result["status"] in ("VERIFIED","FAILED","PARTIAL","REJECTED"):break
        device_record=device.get_command_status(proposal["id"])
        checks={"no_write_before_approval":no_write_before_approval,"idempotent_proposal":idempotent,
            "exact_approval_and_role_checked":len(blocked)==2,"verified_feedback":result["status"]=="VERIFIED",
            "one_device_write":device_record["write_count"]==1,"one_delivery_attempt":result["outbox"]["attempts"]==1,
            "three_good_samples":result["result"]["points"][POINTS[0]]["good_samples"]==3}
        if not all(checks.values()):raise AssertionError(checks)
        return {"scenario":"ack_loss" if ack_loss else "normal","states":states,"checks":checks,
            "final_status":result["status"],"device_write_count":device_record["write_count"],
            "ack_received":result["result"]["ack_received"],"blocked_actions":blocked}
    finally:service.close()

def run(output):
    root=Path(output)/uuid.uuid4().hex[:12];root.mkdir(parents=True)
    evidence_id="E-"+digest({"demo":"public-facts-v1"})[:20]
    evidence={"evidence_id":evidence_id,"tool":"event_context","data":{
        "summary":{"dataset":"synthetic","local_fact_names":["current_cu","current_as"]},
        "local_facts":{"current_cu":{"value":12.5,"unit":"g/L"},"current_as":{"value":1000.,"unit":"mg/L"}}}}
    ...
    report={"schema_version":"public-demo.v1","dataset":"synthetic","decision_mode":"deterministic replay",
        "paid_model_calls":0,"real_plant_connected":False,"answer":answer,"model_visible_evidence":model_visible,
        "scenarios":results,"passed":True}
    (root/"report.json").write_text(json.dumps(report,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
    lines=["# CuLab public demo","",
        ...
        "| --- | --- | ---: | --- |"]
    lines += ["| "+r["scenario"]+" | "+r["final_status"]+" | "+str(r["device_write_count"])+" | "+str(r["ack_received"])+" |" for r in results]
    lines += ["","Checks: exact approval, role enforcement, idempotency, three feedback samples and ACK-loss reconciliation all passed.",
