...
if __name__=="__main__":
    ...
    if not 1<=args.cases<=20:parser.error("cases must be between 1 and 20")
    import numpy as np
    ...
    request=PortfolioRequest(request_key=args.request_key,mode=args.mode,event_ids=tuple(events),seeds=tuple(args.seeds),total_budget=args.budget)
    service=PortfolioStudies(args.run_dir,data,models);actor=Principal("owner","owner")
    identifier=digest([actor.project_id,actor.user_id,request.request_key])[:32]
    print(json.dumps({"id":identifier,"mode":args.mode,"cases":len(events) if data else 1,"seeds":len(args.seeds)}),flush=True)
