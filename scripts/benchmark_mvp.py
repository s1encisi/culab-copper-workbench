...
def main():
        ...
        for repeat in range(3):
            for i, event in enumerate(events):
                ...
                for arm in ["fixed", "graph"] if (repeat + i) % 2 == 0 else ["graph", "fixed"]:
                    ...
                    start = perf_counter()
                    results[arm] = (
                        workbench.executor.submit(fixed, event["event_id"], key).result()
                        if arm == "fixed"
                        else graph(event["event_id"], key)
                    )
                    measurements.append(
                        {
                            "arm": arm,
                ...
                for target in ("cu", "as"):
                    differences.append(
                        abs(
                            results["fixed"]["predictions"][target]["value"]
                            - results["graph"]["predictions"][target]["value"]
                        )
                    )
    finally:
        workbench.close()
        ...
        summaries.append(
            {
                ...
                "throughput_runs_per_second": float(len(values) / values.sum() * 1000),
                "concurrency": 1,
                "failures": 0,
                "llm_calls": 0,
                "tokens": 0,
    ...
    report = {
        "created_at": utc_now(),
        "model_bundle": bundle,
        "sample": "最近8个双段OOF事件，3轮交替顺序，双臂各1次预热",
        "common_work": "相同事件、模型、检查、运行索引、节点记录和结果文件；图路径另存 LangGraph checkpoint",
        "max_prediction_difference": max(differences),
        "summary": summaries,
        "measurements": measurements,
    }
    write_json(output / "benchmark.json", report)
    print(
        {"report": str(output / "benchmark.json"), "max_prediction_difference": max(differences), "summary": summaries}
    )
