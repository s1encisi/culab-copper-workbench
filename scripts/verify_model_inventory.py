...
def verify_inventory(run_root, study_id, output):
    ...
    report = {"status": "running", "study_id": study_id, "artifacts": [], "explanations": [], "release_sources": []}
        ...
        for entry in manifest["artifacts"]:
            ...
            if fold == "DEVELOPMENT":
                ...
                if method in NEURAL_METHODS:
                    ...
                    full = (
                        model.predict_context(data, all_events)
                        if model.spec.get("input_kind") == "anchored_process_sequence"
                    ...
                    passed, numerical = neural_replay_error(
                        predicted, expected, X[:, :2], model.fit_metadata["target_delta_scale"], method=method
                    )
                ...
                if method == "NGBoost":
                    ...
                    for column, target in enumerate(("cu", "as")):
                        expected_std = (
                            uncertainty[uncertainty.target.eq(target)].set_index("event_id").loc[events, "std"]
                        )
                ...
                if method in ("BART", "TabPFN"):
                    ...
                    for column, target in enumerate(("cu", "as")):
                        expected_q = (
                            uncertainty[uncertainty.target.eq(target)]
                            .set_index("event_id")
                            .loc[events, ["q10", "q50", "q90"]]
                            .to_numpy(float)
                        )
                        if method == "TabPFN":
                            scale = model.fit_metadata["target_delta_scale"][column]
                            valid, detail = neural_replay_error(
                                quantiles[:, :, column], expected_q, X[:, column, None], scale, method=method
                            )
                            assert valid, detail
                        else:
                ...
                if method == "SymbolicRegression":
                    ...
                    standardized[:, ~np.asarray(payload["active_input_mask"], bool)] = 0.0
                    delta = np.column_stack([evaluate_expression(m["ast"], standardized) for m in payload["models"]])
                    reconstructed = (
                        X[:, :2]
                        + np.asarray(payload["target_delta_mean"])
                        + np.asarray(payload["target_delta_scale"]) * delta
                    )
                    ...
                    report["explanations"].append(
                        {
                            ...
                            "affine_reconstruction": True,
                            "expressions": [m["expression"] for m in payload["models"]],
                ...
                if method == "EBM":
                    ...
                    reconstructed = np.column_stack(
                        [native.eval_terms(native_matrix(X)).sum(axis=1) + native.intercept_ for native in model.models]
                    )
                    ...
                    payload = {
                        "scope": "model_associations",
                        "feature_names": data.feature_columns,
                        "target_delta_mean": explained.target_scaler.mean_.tolist(),
                        "target_delta_scale": explained.target_scaler.scale_.tolist(),
                        "targets": [],
                    }
                    for target, native in zip(("cu", "as"), model.models, strict=True):
                        missing_bin = [float(scores.flat[0]) for scores in native.term_scores_[:114]]
                        payload["targets"].append(
                            {
                                "target": target,
                                "intercept": float(native.intercept_),
                                "term_features": native.term_features_,
                                "term_names": native.term_names_,
                                "term_importances": native.term_importances().tolist(),
                                "bins": [
                                    [v.tolist() if isinstance(v, np.ndarray) else v for v in levels]
                                    for levels in native.bins_
                                ],
                                ...
                                "probe_main_effects": (
                                    native.eval_terms(native_matrix(X))[:, :114]
                                    * explained.target_scaler.scale_[len(payload["targets"])]
                                ).tolist(),
                            }
                        )
                ...
                if method == "Cubist":
                    ...
                    lower, upper = np.nanquantile(data.X.loc[data.training_ids(fold)].iloc[:, 2], [0.05, 0.95])
                    ...
                    report["explanations"].append(
                        {
                            ...
                            "path": path.relative_to(output).as_posix(),
                            "sha256": file_hash(path),
                            "feature_probe": data.feature_columns[2],
    ...
    for fold in sorted({r["fold"] for r in ebm_records}):
        payloads = [
            json.loads((output / r["path"]).read_text(encoding="utf-8")) for r in ebm_records if r["fold"] == fold
        ]
            ...
            stability.append(
                {
                    ...
                    "mean_probe_main_effect_seed_sd": float(np.std(effects, axis=0, ddof=1).mean())
                    if len(terms) > 1
                    else None,
                    "maximum_probe_main_effect_seed_sd": float(np.std(effects, axis=0, ddof=1).max())
                    if len(terms) > 1
                    else None,
                }
            )
    ...
    if symbolic_records:
        ...
        for index, target in enumerate(("cu", "as")):
            ...
            overlap = [
                len(a & b) / len(a | b) if a | b else 1.0 for i, a in enumerate(features) for b in features[i + 1 :]
            ]
            report["symbolic_structure_stability"][target] = {
                "fitted_expressions": len(features),
                "distinct_structures": len(signatures),
                "structure_counts": dict(signatures),
    ...
    for method in methods:
        for target in ("cu", "as"):
            ...
            shadow = lifecycle.start_shadow(
                actor, candidate["id"], {"request_key": "shadow-" + key, "samples_per_fold": 4}
            )
            assert shadow["status"] == "completed" and shadow["evidence"]["parity_passed"]
            report["release_sources"].append(
                {
                    "method": method,
                    "target": target,
                    "candidate_id": candidate["id"],
                    "shadow_id": shadow["id"],
                    "samples": shadow["evidence"]["samples"],
                    "parity_passed": True,
    ...
    print(
        json.dumps(
            {
                "status": "PASSED",
                "fit_artifacts": report["fit_artifacts"],
                "replayed_events": report["actual_replayed_events"],
                "release_sources": len(report["release_sources"]),
            }
        ),
        flush=True,
    )
