...
from copper_mvp.data import OFFSETS, DataRepository
from copper_mvp.data_contracts import SOURCE_TIMEZONE, ReplayQuery, source_time
...
def main():
    ...
    output = (
        args.output or PROJECT_ROOT / "runs/g1" / datetime.now(UTC).strftime("acceptance_%Y%m%dT%H%M%SZ")
    ).resolve()
    ...
    for event, row in data.frame.iterrows():
        ...
        if (
            source_time(admission["decision_at"]) != source_time(row.decision_at)
    ...
    for fold in sorted(data.cv.fold_id.unique()):
        ...
        if not all(
            (event, target) in available and available[(event, target)].decision_at < cutoff
            for event in train_ids
            for target in ("cu", "as")
        ):
    ...
    if args.before:
        ...
        actual = {
            ...
            "x_hash": matrix_hash(X),
            "y_hash": matrix_hash(y),
    ...
    artifacts = {
        "after.json": report,
        "dependencies.json": service.dependencies(include_paths=True),
        "evidence_at_decision.json": before,
        "evidence_at_maturity.json": mature,
    }
    ...
    markdown += (
        "全开发集标签重建与原训练标签逐值一致；所有准入时间及有效锚点通过检"
        "查；五个时间折的训练标签均已成熟。输入文件哈希保持一致。\n"
        "\n"
    )
    if baseline_checks:
        markdown += (
            "改造前后 "
            f"{baseline_checks['predictions_compared']}"
            " 份既有预测完全一致，完整 X/y 和所选旧事件上下文保持一致。\n"
            "\n"
        )
    markdown += "## 一个真实事件的时间线\n\n"
    ...
    markdown += (
        "\n"
        "决策时点状态为 LABELS_NOT_AVAILABLE，不返回未来标签；标签可得时点"
        "状态为 EVALUABLE。预测按历史虚拟决策时间回放，本次实际计算时间另存"
        "于证据记录的 created_at。\n"
        "\n"
    )
    markdown += (
        "[决策时点证据](evidence_at_decision.json) · [成熟后证据](evidence_"
        "at_maturity.json) · [本地依赖清单](dependencies.json) · [完整验收"
        "结果](after.json)\n"
        "\n"
    )
    markdown += (
        "本报告验证 G1 的功能、时间边界和兼容性；不把它作为模型精度提高或真"
        "实在线运行的证明。2026 外评和外部 API 未被调用。\n"
    )
    with (output / "report.md").open("x", encoding="utf-8", newline="\n") as stream:
        stream.write(markdown)
    print(
        json.dumps(
            {k: v for k, v in report.items() if k not in ("source_hashes", "example_event_id")},
            ensure_ascii=False,
            indent=2,
        )
    )
