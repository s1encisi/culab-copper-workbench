...
if __name__ == "__main__":
    ...
    folds = [f["fold_id"] for f in protocol["folds"] if f["fold_id"] != "DEVELOPMENT"]
    ...
    for artifact in manifest["artifacts"]:
        if artifact["status"] == "completed":
            assert file_hash(source / artifact["path"]) == artifact["sha256"]
    identifier = digest(args.request_key)[:32]
    root = base / identifier
    assert not root.exists()
    shutil.copytree(source, root)
    protocol["request"]["request_key"] = args.request_key
    protocol["recovery"] = {
        "source_comparison": args.source_id,
        "source_manifest_sha256": file_hash(source / "training_manifest.json"),
    ...
    state.update(
        ...
        fingerprint=digest(protocol["recovery"]),
    )
    write_json(root / "state.json", state)
    ids = data.training_ids(None)
    cutoff = source_time(data.frame.decision_at.max())
    ...
    old_elapsed = manifest["elapsed_ms"]
    for method in methods:
        if ("DEVELOPMENT", method) in completed:
            continue
        ...
        path = root / "artifacts/DEVELOPMENT" / method / "model.joblib"
        ...
        manifest["artifacts"] = [
            a for a in manifest["artifacts"] if (a["fold_id"], a["method_id"]) != ("DEVELOPMENT", method)
        ] + [entry]
        manifest["timings"] = [
            t for t in manifest["timings"] if (t["fold_id"], t["method_id"]) != ("DEVELOPMENT", method)
        ]
        manifest["timings"].append(
            {
                "fold_id": "DEVELOPMENT",
                "method_id": method,
                "train_events": len(ids),
                "status": "completed",
        ...
        manifest.update(
            ...
            elapsed_ms=old_elapsed + (time.perf_counter() - started) * 1000,
        )
        write_json(root / "training_manifest.json", manifest)
    assert source_signature(data) == manifest["source"]
    manifest["status"] = "completed"
    manifest["recovery"] = protocol["recovery"]
    write_json(root / "training_manifest.json", manifest)
    result = evaluate_comparison(data, root)
    ...
    write_json(root / "state.json", state)
    print(
        json.dumps({"status": "completed", "comparison_id": identifier, "reused_oof_sha256": manifest["oof_sha256"]}),
        flush=True,
    )
