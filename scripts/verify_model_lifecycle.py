...
if __name__ == "__main__":
    ...
    for index, (kind, source, method, seed) in enumerate(cases):
        ...
        artifact = service.register(
            actor,
            {
                "request_key": "artifact-" + str(index),
                "source_kind": kind,
                "source_id": source,
                "method_id": method,
                "seed": seed,
        ...
        shadow = service.start_shadow(
            actor, artifact["id"], {"request_key": "shadow-" + str(index), "samples_per_fold": 4}
        )
        ...
        try:
            service.propose(
                actor,
                {
                    "request_key": "publish-" + str(index),
                    "candidate_id": artifact["id"],
                    "shadow_id": shadow["id"],
                    "target": "cu",
                    "expected_version": 0,
                    "reason": "Verify benchmark gate without changing defaults",
                },
            )
        ...
        item = {
            ...
            "parity_passed": shadow["evidence"]["parity_passed"],
            "qualification": gate,
            "release_rejected": rejected,
        }
        results.append(item)
        write_json(
            args.output / "verification.json", {"status": "running", "cases": results, "defaults_unchanged": True}
        )
