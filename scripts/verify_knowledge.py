...
FIXTURES = {
    ...
    "labels": ("合成时间合同", "实验室化验结果只有发布完成后才能作为标签使用。迟到标签不能回填到早于发布时间的历史决策中。"),
    "validation": ("合成模型协议", "按时间顺序设置训练集和验证集。预处理和特征选择只在训练样本内拟合，验证结果不参与训练。"),
    "control": ("合成控制规程", "优化器输出候选方案。候选经审批与联锁检查后才可进入模拟命令执行。文档本身不能授予设备写入权限。"),
    "rollback": ("合成发布流程", "模型发布时记录工件版本、审批和影子验证。回退使用此前登记的模型工件，恢复后重新核对状态。"),
    "missing": ("合成观测合同", "传感器缺失时保留缺失标记和观测年龄。向后匹配历史信号，不允许使用决策时刻以后的未来值。"),
}
QUESTIONS = [
...
def main():
    ...
    paths += [ROOT / "src/copper_mvp/api_knowledge.py", ROOT / "configs/runtime/knowledge_embedding.json", Path(__file__)]
    source_hashes = {p.relative_to(ROOT).as_posix(): hashlib.sha256(p.read_bytes()).hexdigest() for p in paths}
    report = {"status": "running", "scope": "synthetic offline integration; not a reviewed industrial RAG benchmark",
    ...
    try:
        ...
        with TestClient(create_app(args.output / "runtime", data), base_url="http://127.0.0.1") as client:
            ...
            reader_key = client.post("/api/v2/access-keys", json={"user_id": "reader", "role": "viewer"}).json()["access_code"]
            headers = {"Authorization": "Bearer " + reader_key}
            for doc_id, (title, text) in FIXTURES.items():
                metadata = {"doc_id": doc_id, "title": title, "author": "synthetic-fixture",
                            "source": "verify_knowledge.py synthetic fixture", "authorization": "local test only",
                            "license": "synthetic fixture", "classification": "internal", "version_label": "v1",
                            "format": "md", "effective_at": "2025-01-01T00:00:00Z", "readers": ["reader"]}
            ...
            for question, expected in QUESTIONS:
                start = perf_counter()
                response = client.post("/api/v2/knowledge/search",
                                       json={"query": question, "limit": 20}, headers=headers)
            ...
            assert withdrawn
            revoked = client.put("/api/v2/knowledge/documents/units/access",
                                 json={"revoked": True, "readers": ["reader"]})
            assert revoked.status_code == 200
            after = client.post("/api/v2/knowledge/search", json={"query": "铜浓度单位", "limit": 20}, headers=headers).json()
            assert all(v["citation"]["doc_id"] != "units" for v in after["items"])
            assert client.get("/api/v2/knowledge/citations/" + withdrawn[0]["citation"]["chunk_id"],
                              headers=headers).status_code == 404
            metrics = {}
            for channel in ("hybrid", "bm25", "dense"):
                metrics[channel] = {f"recall_at_{k}": float(np.mean([v["expected"] in v["ranking"][channel][:k]
                                                                  for v in records])) for k in (1, 5)}
            report.update(status="passed", queries=len(records), documents=len(FIXTURES),
                          retrieval=metrics, citation_checks=citation_checks, citation_correctness=1.0,
                          unauthorized_document_leaks=0, revoked_citation_readable=False,
                          latency_ms={"p50": float(np.percentile(timings, 50)), "p95": float(np.percentile(timings, 95))},
                          outbound_requests=len(blocked), local_loopback_connections=len(loopback_connections), records=records,
            ...
            assert not blocked
            assert source_hashes == {p.relative_to(ROOT).as_posix(): hashlib.sha256(p.read_bytes()).hexdigest()
                                     for p in paths}
    ...
    print(json.dumps({k: v for k, v in report.items() if k not in ("records", "embedding", "source_hashes")}, ensure_ascii=False))
