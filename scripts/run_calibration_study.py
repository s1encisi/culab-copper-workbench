...
if __name__=="__main__":
    ...
    if args.methods:values["methods"]=args.methods
    if args.seeds:values["seeds"]=args.seeds
    request=CalibrationRequest(**values)
    service=CalibrationService(args.run_root,DataRepository());actor=Principal("owner","owner")
    ...
    print(json.dumps({"study_id":state["id"],"status":result["status"],"common_events":result.get("result",{}).get("common_events"),
