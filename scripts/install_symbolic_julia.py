...
plan = json.loads((root / "configs/runtime/symbolic_julia.json").read_text(encoding="utf-8"))
import os

dependencies = (
    Path(os.environ.get("COPPER_SYMBOLIC_RUNTIME_DIR", root / "runs/dependencies/pysr-1.5.9")).resolve().parent
)
        ...
        with partial.open("ab" if offset else "wb") as stream:
            ...
            while True:
                ...
                if size - last >= 16 * 1024 * 1024:
                    print(json.dumps({"downloaded_bytes": size, "total_bytes": plan["size"]}), flush=True)
                    last = size
...
record = {
    **plan,
    ...
    "verified_sha256": digest(archive),
    "version_output": result.stdout.strip(),
    "status": "installed",
}
(dependencies / "julia_installation.json").write_text(json.dumps(record, indent=2), encoding="utf-8")
print(
    json.dumps({"status": "installed", "version": record["version_output"], "executable": str(executable)}), flush=True
)
