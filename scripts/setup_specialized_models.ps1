...
& $Python -X utf8 -B -m pip install --no-cache-dir --no-compile --disable-pip-version-check --no-deps --no-build-isolation --only-binary=:all: --no-binary=autograd-gamma --require-hashes --index-url https://pypi.org/simple --target $SpecialTarget -r $SpecialRequirements
...
& $Python -X utf8 -B -m pip install --no-cache-dir --no-compile --disable-pip-version-check --no-deps --only-binary=:all: --require-hashes --index-url https://pypi.org/simple --target $CubistTarget -r $CubistRequirements
