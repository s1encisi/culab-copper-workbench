...
def main():
    ...
    if args.output.exists():raise RuntimeError("Choose a fresh verification output")
    args.output.mkdir(parents=True)
    data=DataRepository();service=CalibrationService(args.run_root,data);actor=Principal("owner","owner")
    state=service.get(actor,args.study_id);assert state["status"]=="completed"
    folder=service.directory(args.study_id);manifest,protocol=read_calibration_training(folder)
    ...
    try:
        for entry in manifest["artifacts"]:
            ...
            for index in sample:
                ...
                if entry["method_id"] in NEURAL_METHODS:
                    passed,_=neural_replay_error(predicted,expected[[index]],data.X.loc[[events[index]]].to_numpy(float)[:,:2],
                                                model.fit_metadata["target_delta_scale"])
                    assert passed
                else:np.testing.assert_allclose(predicted,expected[[index]],rtol=1e-10,atol=1e-9)
            ...
            report["artifacts"].append({"method_id":entry["method_id"],"seed":entry["seed"],"fold_id":entry["fold_id"],
                "full_batch_events":len(events),"p50_api_ms":float(np.percentile(latencies,50)),"p95_api_ms":float(np.percentile(latencies,95))})
        ...
        with TestClient(create_app(args.output/"api-runtime",data),base_url="http://127.0.0.1") as client:
            ...
            response=client.post("/api/v2/calibrations/"+args.study_id+"/predict",json={"event_id":event,"method_id":"Persistence"})
            assert response.status_code==200,response.text
            assert response.json()["evaluation_mode"]=="historical_oof_replay"
            viewer=client.post("/api/v2/access-keys",json={"user_id":"viewer","role":"viewer"}).json()["access_code"]
            forbidden=client.post("/api/v2/calibrations",json={"request_key":"unauthorized-fit"},headers={"Authorization":"Bearer "+viewer})
    ...
    print(json.dumps({"status":report["status"],"artifacts":len(report["artifacts"]),
                      "replayed_events":report["replayed_events"],"api_cases":report["api_cases"]}))
