...
def main():
    ...
    request = ComparisonRequest(request_key=args.request_key, methods=tuple(args.methods),
                                seed=args.seed, max_wall_seconds=args.max_wall_seconds)
    service = ComparisonService(root, DataRepository())
    with ThreadPoolExecutor(max_workers=1) as executor:
        state = service.submit(request, executor)
        run_id = state["run_id"]
        print(json.dumps({"run_id": run_id, "reused": state["reused"], "root": str(service.directory(run_id))}), flush=True)
        previous = None
        while True:
    ...
    print(json.dumps({"status": result["status"], "common_events": result["common_events"],
                      ...
                      "automatic_promotion": False}, ensure_ascii=False), flush=True)
