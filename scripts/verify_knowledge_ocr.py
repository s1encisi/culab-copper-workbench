...
def corrections(parse_hash):
    return {"expected_parse_hash": parse_hash, "note": "Compared all text, table cells and formula case with the synthetic source image.",
            "corrections": [{"block_id": 0, "regions": [
                {"kind": "heading", "text": "合成 OCR 验证页"},
                {"kind": "paragraph", "text": "仅用于软件测试，数值不代表工厂设置。\n一、浓度记录"},
                {"kind": "table", "headers": ["字段", "合成值", "单位"],
                 "rows": [["铜浓度", "12.5", "g/L"], ["砷浓度", "24.0", "mg/L"]],
                 "footnotes": ["表注：所有数值均为合成测试样例。"], "bbox": [120, 400, 1350, 765]},
                {"kind": "heading", "text": "二、表达式及符号"},
                {"kind": "equation", "text": "c = m / V", "definitions": "c 为质量浓度，m 为溶质质量，V 为溶液体积。",
...
def main():
    ...
    paths = [*sorted((ROOT / "src/copper_mvp").glob("knowledge_*.py")),
             ROOT / "src/copper_mvp/api_knowledge.py", ROOT / "configs/runtime/knowledge_ocr.json", Path(__file__)]
    ...
    try:
        with TestClient(create_app(args.output / "runtime", DataRepository()), base_url="http://127.0.0.1") as client:
            ...
            spec = {"doc_id": "scan-fixture", "title": "合成扫描测试", "author": "test-author", "source": "synthetic OCR fixture",
                    "authorization": "local software verification", "license": "synthetic", "version_label": "v1",
                    "format": "pdf", "effective_at": "2025-01-01T00:00:00Z", "readers": ["reader"]}
            ...
            assert client.post(base + "/ocr", json={"expected_parse_hash": initial["parse_hash"]},
            ...
            stale = client.post(base + "/review", json={"expected_parse_hash": ocr["parse_hash"],
                                                       "accepted": True, "note": "stale parse"})
            assert stale.status_code == 409
            approved = client.post(base + "/review", json={"expected_parse_hash": corrected["parse_hash"],
                                                           "accepted": True, "note": "Verified synthetic image, units and uppercase V."})
            assert approved.status_code == 200, approved.text
            indexed = client.post(base + "/index")
            assert indexed.status_code == 200, indexed.text
            retrieved = client.post("/api/v2/knowledge/search", json={"query": "铜和砷浓度使用什么单位？", "limit": 20}, headers=reader)
            ...
            report.update(status="passed", ocr_lines=len(ocr_block["details"]["ocr"]["lines"]),
                          ocr_minimum_score=ocr_block["details"]["ocr"]["minimum_score"],
                          raw_ocr_text=ocr_block["text"], ocr_signature=ocr_block["details"]["ocr"]["engine_signature"],
                          table_units_preserved=True, formula_case_corrected=True, stale_review_rejected=True,
                          parse_history_count=len(history), deletion_removed_parse_history=True,
                          original_unchanged=True, outbound_requests=len(blocked),
                          source_hashes=source_hashes)
            assert source_hashes == {p.relative_to(ROOT).as_posix(): hashlib.sha256(p.read_bytes()).hexdigest() for p in paths}
    ...
    print(json.dumps({k: v for k, v in report.items() if k not in ("source_hashes", "raw_ocr_text")}, ensure_ascii=False))
