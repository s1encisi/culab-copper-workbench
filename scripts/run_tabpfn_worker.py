...
from copper_mvp.tabpfn_runtime import disable_worker_network, prepare_tabpfn_runtime
...
weight = Path(
    os.environ.get(
        "COPPER_TABPFN_WEIGHT", PROJECT_ROOT / "runs/dependencies/tabpfn-weights/tabpfn-v2-regressor-v2_default.ckpt"
    )
)
        ...
        with contextlib.redirect_stdout(sys.stderr):
            ...
            if action == "fit":
                if request["context_id"] == cache_id:
                    ...
                    X, y = data["X"], data["y"]
                    actual_context = hashlib.sha256(
                        X.tobytes()
                        + y.tobytes()
                        + str(request["seed"]).encode()
                        + WEIGHT_SHA256.encode()
                        + str(request["n_estimators"]).encode()
                    ).hexdigest()
                    ...
                    for target in (0, 1):
                        model = TabPFNRegressor(
                            ...
                            device="cpu",
                            n_jobs=1,
                            random_state=request["seed"] + target,
                ...
                if request["context_id"] != cache_id:
                    raise ValueError("Requested context is not loaded")
                path = local_path(request["input_path"])
                assert file_hash(path) == request["input_sha256"]
                X = np.load(path, allow_pickle=False)["X"]
                start = time.perf_counter()
                outputs = [m.predict(X, output_type="main", quantiles=[0.1, 0.5, 0.9]) for m in models]
