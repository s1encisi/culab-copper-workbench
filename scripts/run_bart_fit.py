...
if __name__ == "__main__":
    ...
    try:
        result = fit_bart_posterior(data["X"], data["y"], request["settings"], request["seed"], directory, RUNTIME)
        ...
        write_json(
            directory / "state.json", {"status": "failed", "type": type(exc).__name__, "message": str(exc)[:400]}
        )
        raise
