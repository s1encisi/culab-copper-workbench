...
G6m 内的独立改动另有固定分支，便于逐项比较；`codex/g6m` 继续承接未完成的知识能力。
...
| `codex/g6m-rag` | G6m memory | 正文授权、逐片段引用与派生回答清理 | `ccc3dc6` |
| `codex/g6m-rerank` | G6m RAG | 可选本地重排与逐条规则解析 | 见固定分支 HEAD |

这些分支目前只在本地。相邻两个检查点之间的差异对应表中该项功能；后续修改通过新的提交和检查点记录。

## G6n 共享时序校准

`codex/g6n` 从 G6m 的 `1cde3e3` 建立，包含独立拟合/校准/验证划分、边际与联合 C90、原生区间对照及历史回放接口。基础校准测试、真实两方法试跑和保存工件回放已完成；默认六方法的正式对照继续在此分支验收。该阶段尚未更改发布指针。

## G6n 校准生命周期接入

`codex/g6n-release` 从 `codex/g6n` 的 `33f7623` 建立，把基础模型和校准器一起登记到原生命周期，并将区间写入预测记录。该分支与正在运行的校准/模型验证隔离，完成后通过明确合并保留依赖关系。
