...
[运行演示](DEMO.md) · [面试问答与简历表述](INTERVIEW_QA.md) · [交付状态](CLOSEOUT.md) · [分支与演进](UPGRADE_BRANCHES.md) · [公开审查](PUBLICATION_REVIEW.md) · [真实领域评测](DOMAIN_EVALUATION.md)
...
- 提示词与领域资料的对照评测：问题、引用、实际回答、调用费用和复核可追溯。
...
- 公开合成演示的正常执行和 ACK 丢失恢复均通过，演示本身不调用外部模型。
- G7 已完成 4 个真实模型任务，费用 0.2666 元：RAG 两题生成带引用回答，Prompt 两题达到次数上限；据此修复目录摘要过大的问题。
- 新增领域评测及研究服务 7 项针对性测试通过，前端构建与领域页面浏览器检查完成。
