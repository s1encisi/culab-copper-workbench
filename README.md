...
TabPFN 的正式比较与完整批次回放均固定为单线程数值环境。独立诊断确认：匹配该设置后，完整批次与保存记录满足原严格容差；改变父进程 BLAS 线程数可能使边界样本发生额外变化。

单条、子批次和倒序查询使用 float32 计算，会产生不同的舍入路径。TabPFN 的这类回放采用标准化目标变化量上的 1e-4 相对/绝对数值容差，其他神经方法保留 1e-5。完整批次仍使用 1e-10 相对与 1e-9 绝对检查。该策略不改变模型、权重、数据、预测精度或发布门槛；全量工件验收按单独记录执行。
