...
class EnsembleRequest(RoutingRequest):
    ...
    def seed_scope(self):
        if not 1<=len(self.seeds)<=8 or len(set(self.seeds))!=len(self.seeds) or any(s<0 or s>=2**31 for s in self.seeds):
...
class EnsembleStudies(RoutingStudies):
    ...
    def perform_study(self,root,request,progress):
        return run_ensemble_experiment(self.data,self.comparisons.directory(request.comparison_id),root,request,progress)
    ...
    def resume(self,actor,identifier,executor=None):
        ...
        with self.lock:
            ...
            expected=digest({"request":request.model_dump(),"source":source["fingerprint"],
                "policy":self.policy_spec(),"code":{name:file_hash(Path(__file__).with_name(name)) for name in self.signature_files}})
