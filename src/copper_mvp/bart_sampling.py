...
from copper_mvp.bart_trees import draw_predictions, export_posterior
...
def fit_bart_posterior(X, y, settings, seed, output, runtime):
    ...
    for target, name in enumerate(("cu", "as")):
            ...
            with warnings.catch_warnings(record=True) as notices:
                ...
                with pm.Model() as model:
                    ...
                    try:
                        ...
                        def update_sampling(trace, draw, name=name, chain=chain, chain_seed=chain_seed):
                        ...
                        trace = pm.sample(
                            ...
                            step=[tree_step, noise_step],
                            progressbar=False,
                            compute_convergence_checks=False,
                            callback=update_sampling,
                        )
        ...
        passed = (
            ...
            and maximum_rhat <= settings["maximum_rhat"]
            and minimum_bulk >= settings["minimum_ess"]
            and minimum_tail >= settings["minimum_ess"]
            and divergences == 0
        )
        diagnostics = {
            "passed": bool(passed),
            "maximum_rhat": maximum_rhat,
            "minimum_bulk_ess": minimum_bulk,
            "minimum_tail_ess": minimum_tail,
            "divergences": divergences,
            "variables": diagnostic_rows,
            "training_probe_indices": probe.tolist(),
            "maximum_tree_trace_error": max(parity),
        }
    ...
    if not all(t["diagnostics"]["passed"] for t in targets):
        notices.append(
            {
                "category": "BARTSamplingDiagnostics",
