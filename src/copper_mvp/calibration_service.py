...
class CalibrationService:
    def __init__(self, root, data):
        self.root = Path(root) / "calibration_studies"
    ...
    def list(self, actor):
        self.authorize(actor)
        return [
            observed_job_state(json.loads(path.read_text(encoding="utf-8")))
            for path in sorted(self.root.glob("*/state.json"), key=lambda p: p.stat().st_mtime, reverse=True)[:100]
        ]
    ...
    def execute(self, identifier, request):
        ...
        try:
            ...
            update(
                status="failed",
                finished_at=utc_now(),
                error={"code": getattr(error, "code", type(error).__name__), "message": str(error)[:800]},
            )
    ...
    def predict(self, actor, identifier, event_id, method_id, seed=None):
        ...
        artifact = next(
            (
                ...
                if item["fold_id"] == fold and item["method_id"] == method_id and item["seed"] == seed
            ),
            None,
        )
        ...
        calibrator = SplitC90.from_manifest(
            json.loads((root / artifact["calibrator_path"]).read_text(encoding="utf-8"))
        )
        ...
        return {
            ...
            "seed": seed,
            "fold_id": fold,
            "point": {"cu": float(point[0, 0]), "as": float(point[0, 1])},
            "units": {"cu": "g/L", "as": "mg/L"},
            "decision_at": source_time(self.data.row(event_id).decision_at).isoformat(),
            "computed_at": utc_now(),
            "dataset_version": protocol["source"]["dataset_version"],
            "intervals": {
                mode: {
                    target: {
                        "lower": float(bounds[0, 0, j]),
                        "upper": float(bounds[0, 1, j]),
            ...
            "calibrator_sha256": artifact["calibrator_sha256"],
            "base_fit_cutoff_at": artifact["base_fit_cutoff_at"],
            "calibration_fit_cutoff_at": artifact["calibration_fit_cutoff_at"],
            "evaluation_mode": "historical_oof_replay",
