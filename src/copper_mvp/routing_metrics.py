...
class RoutingPolicy(BaseModel):
    ...
    minimum_coverage: float = Field(default=.95, gt=0, le=1)
    minimum_improvement: float = Field(default=.05, ge=0, lt=1)
    worst_mode_tolerance: float = Field(default=.05, ge=0)
...
def block_interval(differences, decisions, policy):
    ...
    return {"low": float(lo), "high": float(hi), "blocks": len(aggregates), "status": "COMPUTED",
...
class MatureMetrics:
    def __init__(self, forecasts, labels, attributes, methods, scales, profiles, policy=None):
        ...
        if forecasts.duplicated(["event_id", "method_id"]).any() or set(zip(forecasts.event_id, forecasts.method_id)) != expected:
        ...
        for row in forecasts.itertuples(index=False):
            ...
            if (source_time(row.decision_at) != self.times[i] or source_time(row.fit_cutoff_at) > self.times[i]
        ...
        for method_index, method in enumerate(self.methods):
            ...
            self.forecast_ids[method] = [digest({"event_id": e, "method": method, "values": values[i].tolist(),
                                                "fit_cutoff_at": rows.loc[e, "fit_cutoff_at"]}) for i, e in enumerate(self.events)]
        ...
        self.revision_times = np.sort([r.available_at.timestamp() for chain in self.chains.values() for r in chain if r.revision > 1])
    ...
    def _eligible(self, visible, cutoff):
        return [e for e, t in zip(self.events, self.times) if t < cutoff and all(
            (e, target) in visible and visible[(e, target)].quality_eligible
            and visible[(e, target)].value is not None for target in ("cu", "as"))]
    ...
    def _window(self, eligible, visible, cutoff, count, days):
        p = self.policy
        base = [e for e in eligible if source_time(self.attributes[e]["decision_at"]) > cutoff - timedelta(days=days)][-count:]
        ...
        for j, method in enumerate(self.methods):
            ...
            value = n / len(base) if base else 0.
            coverage[method] = {"eligible_events": len(base), "valid_predictions": n, "missing_predictions": len(base)-n, "fraction": value}
            ...
            if profile is None or profile.get("p95_ms") is None or profile["p95_ms"] > p.maximum_p95_ms:
                ...
                pool.append(method)
        common = [e for e in base if all(self.valid[self.position[e], self.methods.index(m)] for m in pool)] if pool else []
        ...
        for method in pool:
            ...
            for target_index, target in enumerate(("cu", "as")):
                ...
                metrics[method][target] = {**numerical_metrics(truth[target], predicted), "groups": groups} if common else None
        revisions = [visible[(e, t)].record_id for e in common for t in ("cu", "as")]
        return {"event_ids": common, "common_event_hash": digest(common), "label_revision_hash": digest(revisions),
                "label_record_ids": revisions, "window_eligible_events": len(base), "n": len(common),
                "coverage": coverage, "excluded": excluded, "eligible_methods": pool,
                "metrics": metrics, "start": (cutoff - timedelta(days=days)).isoformat(),
    ...
    def snapshot(self, as_of, mode=None):
        ...
        blocks = block_interval(np.zeros(long["n"]), [self.attributes[e]["decision_at"] for e in long["event_ids"]], p) if long["n"] else {"blocks": 0}
        mode_n = sum(self.attributes[e]["mode"] == mode for e in long["event_ids"]) if mode is not None else long["n"]
        enough = short["n"] >= p.minimum_events and long["n"] >= p.minimum_events and mode_n >= p.minimum_mode_events and blocks["blocks"] >= p.minimum_blocks
        snapshot = safe({"schema_version": "metric-snapshot.g5a.v1", "evaluation_mode": "historical_replay",
            "comparison_unit": "fixed_method_training_policy_across_predeclared_outer_folds",
            "as_of": cutoff.isoformat(), "policy_hash": p.policy_hash, "window_basis": "origin_decision_at",
            "status": "READY" if enough else "INSUFFICIENT_LABELS", "mode": mode, "mode_events": mode_n,
            "calendar_blocks": blocks["blocks"], "block_days": p.block_days, "mature_events": len(eligible),
    ...
    def score(self, snapshot, target, method, mode):
        ...
        error = mode_metric["mae"] if mode_metric and mode_metric["n"] >= self.policy.minimum_mode_events else metric["mae"]
        ...
        components = {"error": error/scale, "worst_mode": worst/scale, "absolute_bias": abs(metric["bias"])/scale,
                      "latency": min(1., self.profiles[method]["p95_ms"]/self.policy.maximum_p95_ms), "api_cost": 0.}
        weights = {"error": .55, "worst_mode": .15, "absolute_bias": .10, "latency": .10, "api_cost": .10}
    ...
    def paired_gate(self, snapshot, target, challenger, champion):
        ...
        for name, window in (("short",short), ("long",long)):
            ...
            if old <= 0 or new > old * (1-p.minimum_improvement):
                return {"passed": False, "reason": "MINIMUM_IMPROVEMENT", "window": name, "old_mae": old, "new_mae": new}
        ...
        for mode, old in old_groups.items():
            if old["n"] >= p.minimum_mode_events and new_groups[mode]["mae"] > old["mae"] * (1+p.worst_mode_tolerance):
        ...
        errors = np.abs(self.predictions[positions, self.methods.index(challenger), t]-actual) - np.abs(self.predictions[positions, self.methods.index(champion), t]-actual)
        interval = block_interval(errors, [self.attributes[e]["decision_at"] for e in long["event_ids"]], p)
        return {"passed": interval["high"] is not None and interval["high"] < 0, "reason": "PAIRED_INTERVAL",
                "paired_mae_difference": float(errors.mean()), "ci95": interval}
