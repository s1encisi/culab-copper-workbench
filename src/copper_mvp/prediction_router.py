...
class PolicyRouter:
    ...
    def decide(self, event_id):
        ...
        refresh = distinct or previous["revisions"] != revisions or at-source_time(previous["snapshot"]["as_of"]) >= timedelta(hours=p.maximum_snapshot_age_hours)
        ...
        if refresh:
            ...
            self.evidence[key] = {"snapshot": snapshot, "revisions": revisions,
                                 "confirmation_count": maturity if distinct else previous["confirmation_count"]}
        else:
            snapshot = previous["snapshot"]
        decisions = {}
        for target_index, target in enumerate(("cu", "as")):
            state = self.states.setdefault((key, target), {"champion": "Persistence", "streak": 0,
                "challenger": None, "confirmation_snapshots": [], "last_switch_time": None, "last_switch_count": 0})
            pool = sorted(set(snapshot["short"]["eligible_methods"]) & set(snapshot["long"]["eligible_methods"]))
            result = {"event_id": event_id, "target": target, "decision_at": at.isoformat(),
                "metric_snapshot_id": snapshot["id"], "metric_as_of": snapshot["as_of"],
                "policy_hash": p.policy_hash, "mode": mode, "candidate_methods": pool, "scores": {},
                "new_confirmation_window": distinct, "evaluation_mode": "historical_replay",
                "deployment": "shadow_replay", "live_model_change": False}
            ...
            if snapshot["status"] == "READY" and "Persistence" in pool:
                ...
                if challenger != choice:
                    ...
                    if distinct:
                        ...
                        state["confirmation_snapshots"] = (history+[snapshot["id"]])[-p.consecutive_windows:] if gate["passed"] else []
                    ...
                    elapsed = (at-source_time(state["last_switch_time"])).total_seconds()/86400 if state["last_switch_time"] else float("inf")
                    cooldown_ok = elapsed >= p.cooldown_days or maturity-state["last_switch_count"] >= p.cooldown_labels
                    if gate["passed"] and state["challenger"] == challenger and state["streak"] >= p.consecutive_windows and cooldown_ok:
                        choice, reason = challenger, "SHADOW_SWITCH"
                        state.update(champion=choice, streak=0, challenger=None, confirmation_snapshots=[], last_switch_time=at.isoformat(), last_switch_count=maturity)
                    else:
                        reason = "WAIT_CONFIRMATION" if gate["passed"] and cooldown_ok else "COOLDOWN" if gate["passed"] else "KEEP_CHAMPION"
                elif distinct:
                    state.update(streak=0, challenger=None, confirmation_snapshots=[])
            ...
            result.update(selected_method=choice, value=float(predicted), reason=reason,
