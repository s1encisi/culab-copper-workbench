...
class RoutingStudies:
    ...
    def perform_study(self, root, request, progress):
        return run_replay(
            self.data,
            self.comparisons.directory(request.comparison_id),
            root,
            progress=progress,
        ...
        fingerprint = digest(
            {
                ...
                "code": {n: file_hash(Path(__file__).with_name(n)) for n in self.signature_files},
            }
        )
        with self.lock:
            if root.exists():
    ...
    def decisions(self, actor, identifier, event_id):
        ...
        return {
            ...
            "snapshots": evidence,
            "live_model_change": False,
            "batch_recorded_at": manifest.get("created_at"),
            "evaluation_mode": "historical_replay",
        }
