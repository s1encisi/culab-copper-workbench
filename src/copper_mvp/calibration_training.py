...
DETERMINISTIC={"Persistence","DeltaRidge","ElasticNet","Huber","PLS","BayesianRidge","ARD",
               "Quantile","MultiTaskElasticNet","KernelRidge","SVR","KNN","GaussianProcess"}
TARGETS=("cu","as")
...
def visible_labels(ledger,ids,cutoff):
    ...
    for event in ids:
        ...
        if any(v is None or not v.quality_eligible or v.value is None or v.available_at>source_time(cutoff) for v in pair):
...
def temporal_split(data,ledger,outer_ids,valid_ids,cutoff,request,fold):
    ...
    fit=[];calibration=[];purged=[]
    for event in ordered:
        decision=source_time(data.row(event).decision_at)
        if decision>=start:
            calibration.append(event)
        elif all((event,t) in visible and visible[(event,t)].quality_eligible and visible[(event,t)].value is not None
                 and visible[(event,t)].available_at<=start for t in TARGETS):
            fit.append(event)
        else:
            purged.append(event)
    ...
    return {"fold_id":fold,"base_fit_cutoff_at":start.isoformat(),"calibration_fit_cutoff_at":source_time(cutoff).isoformat(),
            "outer_ids":ordered,"outer_event_hash":digest(ordered),
            "fit_ids":fit,"calibration_ids":calibration,"validation_ids":list(valid_ids),"purged_ids":purged,
            "fit_event_hash":digest(fit),"calibration_event_hash":digest(calibration),"validation_event_hash":digest(valid_ids),
            "fit_labels_hash":fit_labels_hash,"calibration_labels_hash":calibration_labels_hash}
...
def native_interval(model,data,ids):
    ...
    if output["kind"]=="marginal_standard_deviation":
        mean=np.asarray(output["mean"],float);std=np.asarray(output["std"],float)
        if mean.shape!=(len(ids),2) or std.shape!=mean.shape or not np.isfinite([mean,std]).all() or (std<=0).any():
            raise WorkbenchError("原生概率输出不合法","CALIBRATION_NATIVE")
        return {"lower":mean-1.6448536269514722*std,"upper":mean+1.6448536269514722*std,
                "center":mean,"std":std,"coverage":.9,"kind":"native_normal_marginals"}
    if output["kind"] in ("raw_marginal_quantiles","posterior_predictive_quantiles"):
        values=np.asarray(output["values"],float);levels=output["levels"]
        if values.shape!=(len(ids),len(levels),2) or not np.isfinite(values).all():
            raise WorkbenchError("原生分位数输出不合法","CALIBRATION_NATIVE")
        return {"lower":values[:,0,:],"upper":values[:,-1,:],"center":values[:,len(levels)//2,:],
                "coverage":float(levels[-1]-levels[0]),"kind":"native_quantiles"}
    return None
...
def train_calibration_study(data,request,output,progress=lambda value:None):
    ...
    source=source_signature(data);ledger=DataService(data).labels
    splits=[]
    for fold in sorted(data.cv.fold_id.unique()):
        ...
        splits.append(temporal_split(data,ledger,data.training_ids(fold),valid,
                      rows.fold_fit_cutoff_at.iloc[0],request,fold))
    ...
    full_ids=[e for e in full_ids if all((e,t) in visible and visible[(e,t)].quality_eligible and visible[(e,t)].value is not None for t in TARGETS)]
    ...
    protocol={"schema_version":"calibration-study.g6n.v1","app_version":APP_VERSION,"request":request.model_dump(mode="json"),
              "source":source,"splits":splits,"expected_method_seeds":expected_seeds,
              "methods":{m:[method_spec(m,seed) for seed in seeds] for m,seeds in expected_seeds.items()},
              "evaluation_as_of":full_cutoff.isoformat(),"code_hashes":hashes,"created_at":utc_now(),
              "scale":"sample standard deviation of fit-segment target deltas; zero scale disables joint region",
              "independence":"base preprocessing/parameters use fit segment only; calibration uses later mature labels; outer labels scored separately",
              "native_comparison":"native uncertainty from the same reduced-fit model, not a model trained on calibration labels",
              "external_2026_read":False,"automatic_promotion":False,
              "latency_policy":{"single_event_samples":32,"selection":"equally_spaced_outer_validation_events",
                                "scope":"in_memory_base_model_plus_calibrator","native_threads":1}}
    write_json(output/"protocol.json",protocol)
    for name in hashes:
        destination=output/"executed_source"/name;destination.parent.mkdir(parents=True,exist_ok=True)
        destination.write_bytes((PROJECT_ROOT/name).read_bytes())
    predictions=[];calibration_predictions=[];artifacts=[];started=perf_counter()

    def persist(status):
        for name,rows in (("predictions.csv",predictions),("calibration_predictions.csv",calibration_predictions)):
            temporary=output/(name+".tmp")
            columns=None if rows else (["event_id","fold_id","method_id","seed","status","decision_at","cu","as"] if name=="predictions.csv"
                                       else ["event_id","fold_id","method_id","seed","cu","as"])
            pd.DataFrame(rows,columns=columns).to_csv(temporary,index=False);temporary.replace(output/name)
        manifest={"schema_version":"calibration-training.g6n.v1","status":status,"source":source,
                  "protocol_sha256":file_hash(output/"protocol.json"),"predictions_sha256":file_hash(output/"predictions.csv"),
                  "calibration_predictions_sha256":file_hash(output/"calibration_predictions.csv"),"artifacts":artifacts,
                  "elapsed_ms":(perf_counter()-started)*1000,"runtime":{"python":platform.python_version(),"numpy":np.__version__},
        ...
        for split in splits:
            ...
            for method,seeds in expected_seeds.items():
                for seed in seeds:
                    ...
                    progress({"phase":"fit_and_calibrate","fold_id":split["fold_id"],"method_id":method,"seed":seed})
                    entry={"fold_id":split["fold_id"],"method_id":method,"seed":seed,"status":"running",
                           "fit_count":len(fit_ids),"calibration_count":len(cal_ids),
                           "base_fit_cutoff_at":split["base_fit_cutoff_at"],"calibration_fit_cutoff_at":split["calibration_fit_cutoff_at"]}
                    fit_started=perf_counter();clock=fit_started
                    try:
                        ...
                        if model.spec["requires_fit"]:
                            if model.spec.get("input_kind") in ("raw_event_sequence","anchored_process_sequence"):
                                model.fit_context(data,fit_ids,source_time(split["base_fit_cutoff_at"]),
                                                  max_wall_seconds=request.max_wall_seconds-(perf_counter()-started))
                            else:
                                model.fit(fit_X,fit_y,max_wall_seconds=request.max_wall_seconds-(perf_counter()-started))
                        ...
                        clock=perf_counter()
                        calibrator=SplitC90.fit(cal_point,cal_y,scale,request.coverage,request.minimum_calibration)
                        entry["calibrator_fit_ms"]=(perf_counter()-clock)*1000
                        folder=output/"artifacts"/split["fold_id"]/method/str(seed);folder.mkdir(parents=True,exist_ok=True)
                        model_path=folder/"model.joblib";joblib.dump(model,model_path,compress=3)
                        cal_path=folder/"calibrator.json";write_json(cal_path,calibrator.manifest())
                        entry.update(status="completed",model_path=model_path.relative_to(output).as_posix(),
                                     model_sha256=file_hash(model_path),calibrator_path=cal_path.relative_to(output).as_posix(),
                                     calibrator_sha256=file_hash(cal_path),joint_available=calibrator.joint_radius is not None,
                                     warnings=model.fit_warnings,model_spec_hash=model.spec["content_hash"])
                        for i,event in enumerate(cal_ids):
                            calibration_predictions.append({"event_id":event,"fold_id":split["fold_id"],"method_id":method,"seed":seed,
                                                            "cu":cal_point[i,0],"as":cal_point[i,1]})
                        if valid_ids:
                            clock=perf_counter();point=model.predict_context(data,valid_ids)
                            ...
                            for i,event in enumerate(valid_ids):
                                row={"event_id":event,"fold_id":split["fold_id"],"method_id":method,"seed":seed,"status":"completed",
                                     "decision_at":source_time(data.row(event).decision_at).isoformat(),
                                     "base_fit_cutoff_at":split["base_fit_cutoff_at"],"calibration_fit_cutoff_at":split["calibration_fit_cutoff_at"],
                                     "cu":point[i,0],"as":point[i,1],"cu_scale":scale[0],"as_scale":scale[1]}
                                for mode,values in bounds.items():
                                    for j,target in enumerate(TARGETS):
                                        row[mode+"_"+target+"_lower"]=values[i,0,j];row[mode+"_"+target+"_upper"]=values[i,1,j]
                                if native is not None:
                                    ...
                                    for j,target in enumerate(TARGETS):
                                        ...
                                        if "std" in native:row["native_"+target+"_std"]=native["std"][i,j]
                                predictions.append(row)
                    except Exception as error:
                        entry.update(status="failed",error={"code":getattr(error,"code",type(error).__name__),"message":str(error)[:600]})
                        for event in valid_ids:
                            predictions.append({"event_id":event,"fold_id":split["fold_id"],"method_id":method,"seed":seed,
                                                "status":"failed","decision_at":source_time(data.row(event).decision_at).isoformat()})
                    entry["total_ms"]=(perf_counter()-fit_started)*1000
                    artifacts.append(entry);persist("running")
    if source_signature(data)!=source or any(file_hash(PROJECT_ROOT/name)!=value for name,value in hashes.items()):
        persist("source_changed");raise WorkbenchError("校准期间源文件发生变化","SOURCE_CHANGED")
