...
from copper_mvp.ensemble_evaluation import evaluate_ensembles,verify_fold,error_correlation
from copper_mvp.model_evaluation import read_training
from copper_mvp.model_training import source_signature

ENSEMBLE_FILES=("ensemble_models.py","ensemble_time.py","ensemble_fusion.py","ensemble_training.py","ensemble_evaluation.py","ensemble_experiment.py","model_adapters.py","modeling.py","model_registry.py")
...
def experiment_signature():
    return {"settings":EnsembleSettings().model_dump(),"code_hashes":{name:file_hash(Path(__file__).with_name(name)) for name in ENSEMBLE_FILES}}
...
def run_ensemble_experiment(data,comparison_root,root,request,progress=lambda value:None):
    ...
    for old in source_protocol["folds"]:
        ...
        valid=data.cv.loc[data.cv.fold_id.eq(old["fold_id"]) & data.cv.fold_role.eq("VALIDATION"),"origin_event_id"].tolist()
    ...
    protocol={"schema_version":"ensemble-experiment.g5b.v1","request":request.model_dump(mode="json"),
        "source":source_manifest["source"],"source_protocol_sha256":file_hash(comparison_root/"protocol.json"),
        "source_oof_sha256":source_manifest["oof_sha256"],"base_methods":BASE_METHODS,
        "base_seed":source_protocol["request"]["seed"],"settings":settings.model_dump(),"signature":signature,
        "folds":folds,"expected_methods":expected,"evaluation_as_of":source_protocol["evaluation_as_of"],
        "feature_columns":source_protocol["feature_columns"],"evaluation_mode":"historical_replay",
        ...
        "shared_fit_accounting":"Each fitted base or bag member is charged once; aliases and ablations share declared parents.",
        ...
        "external_2026_read":False,"method_inventory_increment":0,
        "runtime":{"python":platform.python_version(),"numpy":np.__version__,"sklearn":sklearn.__version__,
                   "platform":platform.platform(),"logical_cpus":os.cpu_count(),"native_threads":1}}
    ...
    old_timings=json.loads((root/"training_costs.json").read_text(encoding="utf-8")) if (root/"training_costs.json").exists() else []
        ...
        with threadpool_limits(limits=1):
            for fold in folds:
                ...
                if (folder/"fold_manifest.json").exists():
                    ...
                    result=train_outer_fold(book,fold,settings,request.seeds,protocol["base_seed"],folder,budget)
                    verify_fold(folder,result,book,fold)
                ...
                for method in BASE_METHODS:
                    ...
                    original=source_predictions[source_predictions.method_id.eq(method)].set_index("event_id").loc[fresh.index,["cu","as"]]
                ...
                correlations[fold["fold_id"]]={target:error_correlation(
                ...
                progress({"phase":"fold_completed","fold_id":fold["fold_id"],"completed_folds":len(results),"fits_recorded":len(old_timings)+len(budget.fits)})
    finally:
        write_json(root/"training_costs.json",old_timings+budget.fits)
        ...
        for result in results:
            with (root/result["directory"]/"weight_decisions.jsonl").open(encoding="utf-8") as source:
                for line in source:
                    ...
                    decision={"event_id":evidence["event_id"],"target":evidence["target"],"method_id":"DynamicConvex",
                        "fold_id":result["fold_id"],"metric_snapshot_id":evidence["id"],"weights":evidence["weights"],
                        "as_of":evidence["as_of"],"evaluation_mode":"historical_replay"}
    ...
    for row in fits:
        ...
        current=groups.setdefault(group,{"fits":0,"fit_ms":0.,"failed":0,"warnings":0})
        current["fits"]+=int(row.get("requires_fit",True));current["fit_ms"]+=row["fit_ms"]
        current["failed"]+=row["status"]=="failed";current["warnings"]+=len(row.get("warnings",[]))
    training_resources={"fit_groups":groups,"all_recorded_fit_entries":len(fits),
        "actual_fit_calls":sum(bool(r.get("requires_fit",True)) for r in fits),"current_invocation_elapsed_ms":(time.perf_counter()-budget.start)*1000,
        "current_invocation_cpu_seconds":time.process_time()-budget.cpu_start,"peak_process_rss_mb":process_peak_mb(),
        "memory_scope":"whole current study process, including loaded data and shared models",
        "full_training_cost_ledger_sha256":file_hash(root/"training_costs.json"),"reference_predictions_reused":reference.method_id.unique().tolist(),
        "llm_calls":0,"tokens":0,"api_cost_cny":0}
    manifest={"schema_version":"ensemble-output.g5b.v1","created_at":utc_now(),"source":protocol["source"],
        "protocol_sha256":file_hash(root/"protocol.json"),"predictions_sha256":file_hash(root/"predictions.csv"),
        "decisions_sha256":file_hash(root/"decisions.jsonl"),"folds":results,"training_resources":training_resources,
        "inner_residual_correlations":correlations,"baseline_refit_parity":{"rtol":1e-10,"atol":1e-9,"passed":True}}
