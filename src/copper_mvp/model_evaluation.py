...
def numerical_metrics(actual, predicted):
    ...
    return {
        ...
        "r2": float(1 - np.square(error).sum() / total) if total > 0 and len(error) > 1 else None,
    }
...
def evaluate_comparison(data, root: Path):
    ...
    actual_keys = set(zip(frame.event_id, frame.method_id, strict=True))
    ...
    for row in frame.itertuples(index=False):
        if (
            row.fold_id != data.fold_for[row.event_id]
    ...
    for method, rows in by_method.items():
        ...
        coverage[method] = {
            ...
            "mature_eligible_events": len(eligible),
            "valid_events": len(ids),
            "missing_or_failed_events": len(data.fold_for) - len(ids),
                ...
                metrics.append(
                    {
                        ...
                        **values,
                        ...
                        "paired_mae_difference": float(delta_errors.mean()),
                        "paired_mae_ci95": interval,
    ...
    for method in methods:
        ...
        resources.append(
            {
                "method_id": method,
                "cv_fit_ms": sum(r["fit_ms"] for r in cv),
                ...
                "prediction_scope": sorted({r.get("prediction_scope", "single_row_matrix_inference") for r in cv}),
                "failed_fits_or_predictions": sum(r["status"] == "failed" for r in rows),
    ...
    result = safe(
        {
            "schema_version": "model-comparison.g2a.v1",
            "created_at": utc_now(),
            "status": "completed"
            if events and manifest["status"] == "completed" and len(events) == len(data.fold_for)
            else "partial_failure",
            "evaluation_mode": "historical_replay",
            "methods": methods,
            "evaluation_as_of": protocol["evaluation_as_of"],
            ...
            "evaluator_elapsed_ms": (time.perf_counter() - started) * 1000,
            "automatic_promotion": False,
            "optimization_proxy_approval": False,
            "best_observed_mae": {
                t: min((r for r in metrics if r["target"] == t), key=lambda r: r["mae"])["method_id"]
                for t in ("cu", "as")
            }
            if metrics
            else {},
    ...
    csv_rows = [
        {
            **{k: v for k, v in row.items() if k != "paired_mae_ci95"},
            "paired_mae_ci95_low": row["paired_mae_ci95"]["low"],
            "paired_mae_ci95_high": row["paired_mae_ci95"]["high"],
        }
        for row in metrics
    ]
    ...
    report += (
        ...
        " 种，共同 OOF 事件 "
        f"{len(events)}"
        " 个。所有方法使用相同事件集合，原输出单位保留。\n"
        "\n"
    )
    report += (
        "| 方法 | 目标 | MAE | RMSE | R² | 偏差 | 相对 Persistence 的 MAE 差 |\n|---|---|---:|---:|---:|---:|---:|\n"
    )
    for row in metrics:
    ...
    report += (
        "\n"
        "MAE 差为本方法减 Persistence，负值表示本次共同事件上的误差更低。完"
        "整 CSV/JSON 保留周块重采样区间和各折结果。\n"
        "\n"
    )
    report += (
        "| 方法 | 五折拟合(ms) | 单事件推理 p50(ms) | p95(ms) | 计时样本 | "
        "拟合警告 | 失败 |\n"
        "|---|---:|---:|---:|---:|---:|---:|\n"
    )
    for row in resources:
        ...
        report += (
            ...
            f"{p95}"
            " | "
            f"{row['timing_samples']}"
            " | "
            f"{row['fit_warning_count']}"
            " | "
            f"{row['failed_fits_or_predictions']}"
            " |\n"
        )
    report += (
        "\n"
        "推理计时不含排队、磁盘加载或 HTTP 开销；事件序列方法还重建截至该事"
        "件的因果历史。JSON 中分别记录矩阵推理与上下文重建范围，不能直接混"
        "比。\n"
        "\n"
    )
    report += (
        "[完整评价](evaluation.json) · [指标 CSV](metrics.csv) · [各折指标]"
        "(fold_metrics.csv) · [预先固定协议](protocol.json) · [训练和工件清"
        "单](training_manifest.json)\n"
        "\n"
    )
    report += (
        "预测排名和优化响应代理资格分开登记。本次比较不改变旧版默认模型、不"
        "自动批准新代理，也不使用 2026 外评。方法和参数均在查看本轮指标前固"
        "定。\n"
    )
