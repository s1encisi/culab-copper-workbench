...
def specialized_dependencies():
    target = Path(
        os.environ.get("COPPER_SPECIALIZED_RUNTIME_DIR", str(PROJECT_ROOT / "runs/dependencies/specialized-models-v1"))
    )
...
class SpecializedModel:
    ...
    def fit(self, X, y, sample_weight=None):
                ...
                if self.method_id == "CatBoost":
                    ...
                    model = NGBRegressor(
                        **params,
                        Dist=Normal,
                        Score=LogScore,
                        Base=DecisionTreeRegressor(max_depth=3, min_samples_leaf=10, random_state=self.seed),
                    ...
                    model = ExplainableBoostingRegressor(
                        **params, random_state=self.seed, feature_types=["continuous"] * 110 + ["nominal"] * 4
                    )
                    model.fit(inputs, target_column, sample_weight=sample_weight)
                else:
        ...
        self.fit_metadata = {
            ...
            "training_rows": len(X),
            "input_order_used": self.method_id == "CatBoost",
