...
from copper_mvp.optimization import nondominated, positive_scale
...
TOOL_DESCRIPTIONS = {
    ...
    "probe_constraints": "在原范围内均匀探测9/17/33个点每维，计算As/非负约束排除率和探测前沿。计数属于新探测，不是原NSGA-II轨迹。",
    "probe_response": "检查固定历史下Cu/As模型响应平台与功率跨度；joint扫描联合网格，stage3/stage4扫描单轴。返回聚合统计，不返回原始行或绝对电流。",
    "compare_probe_resolution": "比较9、17、33点每维的有限网格结果，检查探测分辨率影响；不证明全局收敛，不改变约束或重新训练。",
}
...
class DiagnosticTools:
    def __init__(self, source: dict, data: DataRepository, models: ModelManager):
        ...
        self.resolved = models.resolve(self.request.event_id, self.request.model_profile, self.request.model_scope, result["bundle_id"])
        ...
        if not np.allclose(self.reference_y, expected, rtol=1e-9, atol=1e-8) or not np.isclose(self.base @ self.voltages / 1000, result["reference"]["f2"]):
    ...
    def inspect_run(self) -> dict:
        ranges = []
        for stage, base in zip(self.stages, self.base):
            r = self.ranges.get(stage)
            ranges.append({
                "stage": stage, "quantity": "current", "quantity_label": "电流", "unit": "A", "varies": stage in self.variable_stages,
        ...
        return {
            "evidence_scope": "original_saved_run", "models": self.result["models"],
            "model_scope": self.result["model_scope"], "front_points": self.result["total_front_points"],
            "search_evaluations": self.result["search_evaluations"],
            "feasible_evaluations": self.result["feasible_evaluations"],
            "feasible_rate": self.result["feasible_rate"], "stop_reason": self.result["stop_reason"],
            "configured_evaluation_budget": self.request.evaluation_budget,
            "variable_count": len(self.variable_stages), "ranges_relative_to_reference": ranges,
            "fixed_quantities": ["电压", "历史锚点", "当前化验浓度", "温度", "流量", "工况"],
            "as_allowance_mg_l": self.request.epsilon_as,
            "as_constraint_definition": "候选As预测 <= 同一模型的参考As预测 + epsilon_As；as_allowance_mg_l就是epsilon_As。0表示不允许增加，不表示As浓度为0。",
            "original_rejection_breakdown_available": False,
            "interpretation": "原运行只保存总可行率，未保存逐候选拒绝原因；后续探测只能提供新证据。电压和历史固定，单个前沿点本身不是故障。",
        }
    ...
    def evaluate_currents(self, currents: np.ndarray) -> dict:
        ...
        G = np.column_stack(((y[:, 1] - self.reference_y[1] - self.request.epsilon_as) / self.scale[1], -y[:, 0] / self.scale[0], -y[:, 1] / self.scale[1]))
            ...
            for stage, base in zip(self.stages, self.base):
                ...
                if stage in selected:
                    ...
                    if r and not r["lower"] <= base <= r["upper"]:
                        raise WorkbenchError("单轴探测的固定参考电流在原范围外，请使用joint联合探测", "REFERENCE_OUTSIDE_RANGE")
    ...
    def probe_constraints(self, points_per_axis: int = 17) -> dict:
        ...
        return {
            "evidence_scope": "new_uniform_grid_probe", "points_per_axis": points_per_axis,
            "probe_points": len(p["F"]), "feasible_points": int(p["feasible"].sum()),
            "feasible_rate": float(p["feasible"].mean()), "probe_front_points": p["front_count"],
            "rejected_by_as": int(violated[:, 0].sum()), "rejected_only_by_as": int(as_only.sum()),
            "negative_cu": int(violated[:, 1].sum()), "negative_as": int(violated[:, 2].sum()),
            "as_margin_range_mg_l": [float(v) for v in [np.min(self.reference_y[1] + self.request.epsilon_as - p["y"][:, 1]), np.max(self.reference_y[1] + self.request.epsilon_as - p["y"][:, 1])]],
    ...
    def probe_response(self, points_per_axis: int = 17, axis: str = "joint") -> dict:
        ...
        if len(feasible_f):
            ...
            alignment = {"minimum_power_point_also_minimizes_cu": bool(best[0] <= feasible_f[:, 0].min() + 1e-8), "min_power_point_cu_above_probe_min_g_l": float(best[0] - feasible_f[:, 0].min()), "points_dominated_by_min_power_point": int(dominates.sum()), "feasible_probe_points": len(feasible_f)}
        def describe(values):
            if not len(values):
                return {"unique_cu_rounded_8dp": 0, "cu_span_g_l": None, "as_span_mg_l": None}
            return {"unique_cu_rounded_8dp": len(np.unique(np.round(values[:, 0], 8))), "cu_span_g_l": float(np.ptp(values[:, 0])), "as_span_mg_l": float(np.ptp(values[:, 1]))}
        return {
            "evidence_scope": "new_uniform_grid_probe", "axis": axis,
            "points_per_axis": points_per_axis, "probe_points": len(y),
            "all_points": describe(y), "feasible_points": describe(feasible_y),
            "power_span_kw": float(np.ptp(p["F"][:, 1])), "probe_front_points": p["front_count"],
            ...
            "interpretation": "平台只描述这些探测点上的模型响应。一个点同时最小化两目标可以解释探测前沿仅有一点；可行点很多时不能说可行域塌缩。响应取值少本身不证明平台导致单点前沿，也不证明连续区域或实际工艺不敏感。",
        }
    ...
    def compare_probe_resolution(self) -> dict:
        ...
        for size in (9, 17, 33):
            p = self._probe(size)
            rows.append({"points_per_axis": size, "probe_points": len(p["F"]), "feasible_rate": float(p["feasible"].mean()), "probe_front_points": p["front_count"], "cu_span_g_l": float(np.ptp(p["y"][:, 0]))})
        return {"evidence_scope": "new_uniform_grid_probe", "resolutions": rows, "interpretation": "这是固定范围内三种网格密度的对照。相同前沿点数不等于已证明NSGA-II全局收敛。"}
