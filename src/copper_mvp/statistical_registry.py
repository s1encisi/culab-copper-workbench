...
IMPLEMENTATIONS = {
    "LocalLinearKernel": "statsmodels.nonparametric.kernel_regression.KernelReg",
...
PARAMETERS = {
    "LocalLinearKernel": {"reg_type": "ll", "bandwidth": [0.5, 0.5], "input_columns": [0, 1]},
    "PSplineGAM": {"n_knots": 5, "degree": 3, "alpha": 1.0, "extrapolation": "linear", "input_columns": [0, 1]},
    "SARIMAX": {
        "order": [1, 0, 1],
        "trend": "n",
        "exog_intercept": True,
        ...
        "exog_columns": ["stage3_current_a__t_minus_0h", "stage4_current_a__t_minus_0h"],
        "exog_alignment": "previous-origin values for observed response; current-origin values for next forecast",
    },
...
def statistical_spec(method, seed):
    ...
    result = {
        "schema_version": "model-registry.g6b.v1",
        "method_id": method,
        "design_id": DESIGN_IDS[method],
        "method_version": "g6b.fixed.v2",
        "implementation": IMPLEMENTATIONS[method],
        "package_version": "0.14.6",
        "dependencies": {"statsmodels": "0.14.6", "patsy": "1.0.1"},
        "requires_fit": True,
        "status": "registered",
        "feature_count": 114,
        ...
        "target_scaling": "training_only_standardization",
        "multi_output": "joint_VAR" if method == "VAR" else "two_independent_estimators",
        "preprocessing": "full_event_index_quality_mask_and_training_only_scale"
        if sequence
        else "training_only_scale_of_current_Cu_As",
        "preset_parameters": deepcopy(PARAMETERS[method]),
        "seed": seed,
        "stochastic": False,
        "capabilities": {
            "sample_weight": "unsupported",
            "missing": "native_state_filter"
            if method in ("SARIMAX", "ETS")
            else "causal_forward_fill_adapter"
            if sequence
            else "current_inputs_required",
            "incremental": "state_update_without_parameter_refit" if sequence else "unsupported",
            "uncertainty": "marginal_std" if sequence else "unsupported",
        ...
        "optimization_proxy_approval": "not_granted",
        "causal_control": False,
        "automatic_promotion": False,
        "sequence_time_basis": "recorded_event_steps_not_fixed_hours" if sequence else None,
    }
