...
from copper_mvp.temporal_registry import TEMPORAL_TRAINING,TEMPORAL_ARCHITECTURES
...
class TemporalModel:
    ...
    def fit_context(self,data,train_ids,cutoff,max_wall_seconds=None):
        ...
        if not all((e,t) in labels and labels[(e,t)].quality_eligible and labels[(e,t)].value is not None
                   for e in train_ids for t in ("cu","as")):
        ...
        started=time.perf_counter();history=[];updates=0
        with tensor_random_scope(torch,self.seed):
            ...
            for epoch in range(self.training["epochs"]):
                ...
                for indices in torch.randperm(len(train_ids),generator=generator).split(self.training["batch_size"]):
                    ...
                    if not torch.isfinite(loss):raise WorkbenchError("时序模型损失非有限","NEURAL_LOSS")
                    loss.backward()
                    norm=torch.nn.utils.clip_grad_norm_(network.parameters(),self.training["gradient_clip"])
                    if not torch.isfinite(norm):raise WorkbenchError("时序模型梯度非有限","NEURAL_GRADIENT")
        ...
        self.fit_metadata={"training_rows":len(train_ids),"training_event_hash":digest(train_ids),
            "fit_cutoff":cutoff.isoformat(),"sequence_steps":7,"sequence_features":sequence.shape[-1],
            "context_features":context.shape[-1],"target_delta_mean":self.target_scaler.mean_.tolist(),
            "target_delta_scale":self.target_scaler.scale_.tolist(),"epochs":self.training["epochs"],
            "optimizer_updates":updates,"training_history":history,"parameter_max_changes":changes,
            "parameters":sum(p.numel() for p in network.parameters()),"elapsed_seconds":time.perf_counter()-started,
            "hidden_state_carried_between_events":False,"training_labels_in_sequence":False,
            "device":"cpu","native_threads":1,"early_stopping":False}
        return self

    def __getstate__(self):
        values=self.__dict__.copy();values.pop("_network",None);return values
    ...
    def network(self):
        if not hasattr(self,"_network"):
            ...
            from copper_mvp.temporal_networks import TemporalNetwork
            with tensor_random_scope(torch,self.seed):
                self._network=TemporalNetwork(self.method_id,self.architecture,
                    self.fit_metadata["sequence_features"],self.fit_metadata["context_features"])
            self._network.load_state_dict(self.state);self._network.eval()
    ...
    def predict_context(self,data,event_ids):
        ...
        with torch.inference_mode():
            for start in range(0,len(event_ids),128):
                result.append(network(torch.from_numpy(sequence[start:start+128]),
                                      torch.from_numpy(context[start:start+128])).numpy())
        ...
        if not np.isfinite(values).all():raise WorkbenchError("时序预测非有限","MODEL_OUTPUT_VALUES")
        return values
