...
class ComparisonService:
    ...
    def submit(self, request: ComparisonRequest, executor):
        ...
        with self.lock:
            ...
            state = {"run_id": run_id, "status": "queued", "created_at": utc_now(),
                     "owner_pid": os.getpid(), "request": request.model_dump(mode="json"),
                     "fingerprint": fingerprint, "automatic_promotion": False}
    ...
    def execute(self, run_id, request):
        ...
        try:
            ...
            update(status="completed", finished_at=utc_now(), evaluation_status=result["status"],
                   evaluation_sha256=file_hash(root / "evaluation.json"), progress={"phase": "completed"})
        except Exception as exc:
            update(status="failed", finished_at=utc_now(),
                   error={"code": getattr(exc, "code", type(exc).__name__), "message": "比较未完成，已保留现有工件和失败状态"})
    ...
    def get(self, run_id, *, result=True):
        ...
        if state["status"] in ("queued", "running") and not process_alive(state.get("owner_pid")):
            state = {**state, "status": "interrupted", "error": {"code": "COMPARISON_INTERRUPTED",
        ...
        if result and state["status"] == "completed":
            ...
            if evaluated["protocol_sha256"] != manifest["protocol_sha256"] or evaluated["oof_sha256"] != manifest["oof_sha256"]:
    ...
    def predict(self, run_id, request: ComparisonPredictionRequest):
        ...
        result = {"schema_version": "comparison-prediction.g2a.v1", "run_id": run_id,
                ...
                "context_at": decision.isoformat(), "computed_at": utc_now(), "online_prediction_claim": False,
                "snapshot_id": snapshot["id"], "artifact_sha256": artifact["sha256"],
