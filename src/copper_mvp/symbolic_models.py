...
from copper_mvp.symbolic_registry import SYMBOLIC_PARAMETERS,SYMBOLIC_FEATURE_INDICES,SYMBOLIC_FEATURE_ROLES,symbolic_source_hashes
...
class SymbolicModel:
    ...
    def fit(self,X,y,sample_weight=None,max_wall_seconds=None):
        ...
        self.preprocessor=Pipeline([("impute",SimpleImputer(strategy="median",keep_empty_features=True)),
                                    ("scale",StableNumericScale())])
        ...
        write_json(directory/"request.json",{"seed":self.seed,"settings":self.settings,
                    "input_sha256":file_hash(directory/"input.npz"),"code_hashes":symbolic_source_hashes()})
        environment=os.environ.copy()
        for name in ("OMP_NUM_THREADS","MKL_NUM_THREADS","OPENBLAS_NUM_THREADS"):environment[name]="1"
        environment["LANGSMITH_TRACING"]=environment["LANGCHAIN_TRACING_V2"]="false"
        timeout=1800 if max_wall_seconds is None else max(1.,min(1800,max_wall_seconds))
        kwargs={"creationflags":subprocess.CREATE_NO_WINDOW} if os.name=="nt" else {"start_new_session":True}
        with (directory/"stdout.log").open("wb") as out,(directory/"stderr.log").open("wb") as err:
            process=subprocess.Popen([sys.executable,"-X","utf8","-B",str(PROJECT_ROOT/"scripts/run_symbolic_fit.py"),
                "--directory",str(directory)],cwd=PROJECT_ROOT,env=environment,stdout=out,stderr=err,**kwargs)
        ...
        self.models=result["models"];self.fit_warnings=result["fit_warnings"]
        self.fit_metadata={"training_rows":len(X),"selected_feature_indices":list(SYMBOLIC_FEATURE_INDICES),
            "target_delta_mean":self.target_scaler.mean_.tolist(),"target_delta_scale":self.target_scaler.scale_.tolist(),
            "fit_run_id":directory.name,"runtime":result["runtime"],"source_hashes":result["code_hashes"],
            "dimensionally_valid_by_normalization":True,"native_loss_parity":True,
    ...
    def equations(self):
        ...
        return {"input_roles":list(SYMBOLIC_FEATURE_ROLES),"input_indices":list(SYMBOLIC_FEATURE_INDICES),
            "input_mean":scale.mean_.tolist(),"input_scale":scale.scale_.tolist(),
            "imputation_values":self.preprocessor.named_steps["impute"].statistics_.tolist(),
            "active_input_mask":scale.active_.tolist(),
            "target_delta_mean":self.target_scaler.mean_.tolist(),"target_delta_scale":self.target_scaler.scale_.tolist(),
            "form":"next_target = current_target + delta_mean + delta_scale * f(z); inactive z=0, active z=(imputed_input-mean)/scale",
            "models":self.models,"scope":"model associations; no causal-control interpretation"}
