...
def export_posterior(collections):
    ...
    for collection in collections:
        ...
        for tree in collection[0]:
            ...
            arrays = (np.array(features, dtype=np.int32), np.array(values, dtype=np.float64),
                      np.array(left, dtype=np.int32), np.array(right, dtype=np.int32))
    ...
    return {"offsets": offsets, "features": features, "values": values, "left": left,
            "right": right, "draw_trees": indices}
...
def numeric_evaluator():
    directories = [PROJECT_ROOT/"runs/dependencies/numba-0.61.2",
                   PROJECT_ROOT/"runs/dependencies/pymc-bart-0.7.0"]
...
def draw_predictions(posterior, X):
    ...
    values = numeric_evaluator()(X, *(posterior[k] for k in
        ("offsets", "features", "values", "left", "right")))
...
def mixture_quantiles(means, sigma, levels=(.1, .5, .9)):
