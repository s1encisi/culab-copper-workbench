...
class Workbench:
    ...
    def submit(self, request: RunRequest) -> dict:
        ...
        if uses_response and request.model_profile != "Persistence":
            try:
                ...
                if (
                    ...
                    or request.model_profile != "auto"
                    or request.bundle_id is not None
                    or exc.code != "MODEL_NOT_READY"
                ):
                    raise
        ...
        fingerprint = digest(
            {
                "request": {k: v for k, v in payload.items() if k != "request_key"},
                "dataset": self.data.dataset_version,
        ...
        fingerprint = digest(
            {
                ...
                "agent": AGENT_VERSION,
                "settings": self.diagnostic_agent.settings.model_dump(),
            }
        )
        payload = {
            ...
            "source_run_id": source_run_id,
            "question": request.question,
            "request_key": request.request_key or f"diag:{source_run_id}:{fingerprint[:20]}",
            "model": self.diagnostic_agent.settings.model,
        }
        run, reused = self.store.create(payload, fingerprint)
        if not reused:
    ...
    def _execute(self, run_id: str, request: dict):
        ...
        try:
            ...
            self.store.fail(
                run_id, getattr(exc, "code", type(exc).__name__), redact_text(str(exc)), (perf_counter() - start) * 1000
            )

    def _graph(self, saver):
        def progress(run_id):
            return lambda node, detail, duration: self.store.trace(
                run_id, node, "completed", utc_now(), duration, detail
            )
        ...
        def wrap(name, fn):
            def node(state):
                ...
                try:
                    result = fn(state)
                    self.store.trace(
                        state["run_id"], name, "completed", started, (perf_counter() - clock) * 1000, {"result": "完成"}
                    )
                    return result
                except Exception as exc:
                    self.store.trace(
                        ...
                        "failed",
                        started,
                        (perf_counter() - clock) * 1000,
                        {"code": getattr(exc, "code", type(exc).__name__), "message": redact_text(str(exc))},
                    )
                    raise
        ...
        def optimize(state):
            result = solve(
                ...
                self.store.directory(state["run_id"]),
        ...
        def diagnostic(state):
            ...
            return save_draft(
                state,
                {
                    "kind": "diagnostic",
                    "scenario": scenario,
                    "data_kind": "IN_MEMORY_FAULT_COPY",
                    "blocked": blocked,
                    ...
                    "message": "正常输入通过"
                    if not blocked
                    else "异常被实际请求合同或结果检查拦截，原始数据保持不变。",
                },
            )
        ...
        def entry(state):
            ...
            if r["task_type"] == "train":
                return "TRAIN"
            if r["task_type"] == "diagnostic":
                return "DIAGNOSTIC"
            if r["task_type"] == "optimize" and r["mode"] == "benchmark":
                return "OPTIMIZE"
            return "A1"
