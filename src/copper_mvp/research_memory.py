...
from copper_mvp.common import WorkbenchError, digest, dumps, file_hash, utc_now
...
class SessionMemory:
    ...
    def selected_references(self, actor, context, *, strict=False):
        ...
        for raw in context.get("knowledge_refs", []):
            ...
            try:
                ...
                refs.append(
                    {
                        **{k: citation[k] for k in ("doc_id", "version", "chunk_id", "hash", "parse_hash")},
                ...
                approvals.append(
                    {
                        ...
                        "approval": {
                            k: approval.get(k)
                            for k in ("id", "approver_id", "decision", "revoked", "approved_at", "payload_hash")
                        }
                        if approval
                        else None,
    ...
    def _reference_anchor(references):
        return {
            **{k: v for k, v in references.items() if k != "resource_constraints"},
    ...
    def _authority(self, actor, session_id, current_source=None):
        ...
        with self.research.connection() as c:
            ...
            all_tasks = c.execute(
                "SELECT * FROM research_tasks WHERE session_id=? ORDER BY created_at DESC,id DESC", (session_id,)
            ).fetchall()
            selected = [
                row
            ...
            for row in selected:
                ...
                evidence_rows = c.execute(
                    "SELECT * FROM research_evidence WHERE task_id=? ORDER BY created_at,id", (row["id"],)
                ).fetchall()
                ...
                for record in evidence_rows:
                    ...
                    reference = {
                        "evidence_id": record["id"].split(":", 1)[1],
                ...
                constraints = {
                    key: settings[key]
                    for key in ("max_cost_cny", "max_calls", "max_tool_calls", "max_wall_seconds")
                    if key in settings
                }
                constraints.update(
        ...
        for task, anchor in zip(tasks, anchors, strict=True):
            task["references"] = self.selected_references(actor, task["context"])
            anchor["references"] = self._reference_anchor(task["references"])
            for evidence, reference_anchor in zip(task["evidence"], anchor["evidence"], strict=True):
                ...
                evidence["usable"] = (
                    task["source_current"] and not task["references"]["issues"] and not checked["issues"]
                )
        ...
        live = {
            ...
            "topic": session["title"],
            "goal": tasks[0]["goal"] if tasks else session["title"],
            "initial_goal": initial_goal,
            "selected": selected_refs,
            "tasks": tasks,
            "task_count": len(all_tasks),
            "unfinished_task_ids": [row["id"] for row in all_tasks if row["status"] not in ("completed", "cancelled")],
            "authority": "task_and_evidence_records",
            "memory_can_authorize_actions": False,
        }
        stored = {
            "context_hash": digest(context),
            "tasks": anchors,
            "selected": self._reference_anchor(selected_refs),
    ...
    def _save(self, actor, session_id, anchors, ttl_hours, action):
        ...
        with self.research.connection() as c:
            ...
            c.execute(
                """INSERT INTO session_memories VALUES(?,?,?,?,?,?,?,?)
            ...
            c.execute(
                "INSERT INTO memory_audit(session_id,actor_id,action,generation,at) VALUES(?,?,?,?,?)",
    ...
    def restore(self, actor, session_id, current_source=None, *, refresh=False):
        ...
        with self.research.connection() as c:
            saved = c.execute(
                "SELECT * FROM session_memories WHERE session_id=? AND owner_id=? AND project_id=?",
                (session_id, actor.user_id, actor.project_id),
            ).fetchone()
        if saved is None:
            with self.research.connection() as c:
                last = c.execute(
                    "SELECT action FROM memory_audit WHERE session_id=? ORDER BY id DESC LIMIT 1", (session_id,)
                ).fetchone()
        ...
        if refresh and (expired or repaired):
            ...
            ttl = max(
                ...
                int(
                    (
                        datetime.fromisoformat(saved["expires_at"]) - datetime.fromisoformat(saved["created_at"])
                    ).total_seconds()
                    / 3600
                ),
            )
            metadata = self._save(
                actor, session_id, anchors, ttl, "refresh_expired" if expired else "repair_from_authority"
            )
            status = "refreshed"
    ...
    def forget(self, actor, session_id):
        ...
        with self.research.connection() as c:
            ...
            c.execute(
                "INSERT INTO memory_audit(session_id,actor_id,action,generation,at) VALUES(?,?,?,0,?)",
    ...
    def provider_projection(memory, excluded_task=None):
        ...
        return {
            "topic": value["topic"],
            "initial_goal": value["initial_goal"],
            "unfinished_task_count": len(value["unfinished_task_ids"]),
            "memory_can_authorize_actions": False,
            "evidence_requires_tool_refresh": True,
            ...
            "tasks": [
                {
                    "goal": row["goal"],
                    "status": row["status"],
                    "source_current": row["source_current"],
                    "constraints": row["constraints"],
                    "evidence": [
                        {"evidence_id": e["evidence_id"], "local_fact_names": list(e["local_facts"])}
