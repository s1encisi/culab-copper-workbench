...
from copper_mvp.model_training import load_registered_model, source_signature
...
def prepare_replay(data, comparison_root, output):
    ...
    train_gaps = [
        (visible[(e, "cu")].available_at - visible[(e, "cu")].decision_at).total_seconds() / 3600 for e in train_ids
    ]
    ...
    with threadpool_limits(limits=1):
        ...
        for method in source_protocol["request"]["methods"]:
            model, artifact = load_registered_model(
                comparison_root, manifest, source_protocol, method, first_fold["fold_id"]
            )
            model.predict(X[:1])
            samples = []
            ...
            profiles[method] = {
                ...
                "scope": "first_outer_training_rows_warm_two_target_inference",
                "virtual_available_at": cutoff.isoformat(),
                "api_cost_cny": 0,
            }
    attributes = {}
    ...
    for event in data.fold_for:
        ...
        attributes[event] = {
            ...
            "quality": "complete_features"
            if np.isfinite(data.X.loc[event].to_numpy(float)).all()
    ...
    protocol = {
        "schema_version": "prediction-routing-study.g5a.v1",
        "created_at": utc_now(),
        ...
        "source_oof_sha256": manifest["oof_sha256"],
        "source_training_manifest_sha256": file_hash(comparison_root / "training_manifest.json"),
        "source_methods": source_protocol["request"]["methods"],
        ...
        "calibration": {
            ...
            "target_iqr": scales,
            "interval_boundaries_hours": boundaries,
        ...
        "confirmation_windows": (
            "new sets of at least 60 initially mature events; daily refreshes do not count as extra confirmations"
        ),
        "pool_scope": "benchmarked source methods eligible for offline shadow study only",
        "live_champion": "unchanged",
...
def run_replay(data, comparison_root: Path, output: Path, progress=lambda value: None, max_wall_seconds=600):
    ...
    with threadpool_limits(limits=1):
        for strategy in protocol["strategies"]:
            ...
            engine = MatureMetrics(
                forecasts,
                ledger,
                attributes,
                protocol["source_methods"],
                protocol["calibration"]["target_iqr"],
                protocol["calibration"]["resource_profiles"],
                RoutingPolicy(**strategy["policy"]),
            )
                ...
                records.append(
                    {
                        ...
                        "event_id": event,
                        "decision_at": attributes[event]["decision_at"],
                        "cu": chosen["cu"]["value"],
                        "as": chosen["as"]["value"],
                        "cu_method": chosen["cu"]["selected_method"],
                        "as_method": chosen["as"]["selected_method"],
                        "status": "completed"
                        if all(np.isfinite(chosen[t]["value"]) for t in ("cu", "as"))
                        else "failed",
                    }
                )
            ...
            resources.append(
                {
                    ...
                    "routing_p50_ms": float(np.percentile(samples, 50)),
                    "routing_p95_ms": float(np.percentile(samples, 95)),
                    "samples": len(samples),
                    "timing_scope": "routing from already computed forecasts; includes snapshot refresh/persistence",
                    "llm_calls": 0,
                    "tokens": 0,
                    "api_cost_cny": 0,
                    "retries": 0,
    ...
    manifest = {
        "schema_version": "routing-replay-output.g5a.v1",
        "status": "completed",
        "created_at": utc_now(),
...
def evaluate_replay(data, output):
    ...
    if (
        frame.duplicated(["event_id", "strategy"]).any()
        or set(zip(frame.event_id, frame.strategy, strict=True)) != expected
    ):
            ...
            metrics.append(
                {
                    ...
                    "target": target,
                    "unit": unit,
                    **numerical_metrics(truth, predicted),
                    "paired_mae_difference": float(difference.mean()),
                    "paired_week_ci95": interval,
                    "mode_metrics": groups,
                    "fold_metrics": folds,
    ...
    for strategy in [s["id"] for s in protocol["strategies"]]:
        ...
        routing[strategy] = {
            "reasons": dict(Counter(d["reason"] for d in rows)),
            "selected_models": {
                t: dict(Counter(d["selected_method"] for d in rows if d["target"] == t)) for t in ("cu", "as")
            },
            "switches": sum(d["reason"] == "SHADOW_SWITCH" for d in rows),
        }
    result = safe(
        {
            "schema_version": "routing-evaluation.g5a.v1",
            "status": "completed",
            "created_at": utc_now(),
            "evaluation_mode": "historical_replay",
            ...
            "replay_manifest_sha256": file_hash(output / "replay_manifest.json"),
    ...
    report += (
        "\n"
        "最终评价统一使用七天时间块重采样；路由的一天与七天块方案作为预先声"
        "明的敏感性对照。时间块独立性是区间计算的假设。\n"
        "\n"
    )
    report += (
        "[完整指标与选择理由](evaluation.json) · [预先固定协议](protocol.js"
        "on) · [预测账本](predictions.csv) · [逐事件决策](decisions.jsonl)\n"
        "\n"
    )
