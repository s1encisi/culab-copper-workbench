...
from copper_mvp.common import digest, file_hash, utc_now, write_json
...
def run_matched_controls(data, run_root, study_id, request_key, progress=print):
    ...
    source = run_root / "model_inventory_studies" / study_id
    state = json.loads((source / "state.json").read_text(encoding="utf-8"))
    assert state["status"] == "completed"
    original = json.loads((source / "protocol.json").read_text(encoding="utf-8"))
    ...
    expected.update(dict.fromkeys(controls.values(), seeds))
    protocol = {
        **original,
        "schema_version": "neural-mlp-controls.g6j.v1",
        "request_key": request_key,
        "expected_method_seeds": expected,
        "controls": controls,
        "source_study_id": study_id,
        "source_predictions_sha256": file_hash(source / "predictions.csv"),
        "source_evaluation_sha256": file_hash(source / "evaluation.json"),
        "control_code_hashes": tabular_source_hashes(),
        "new_methods_added": 0,
        "matching": (
            "same preprocessing, target scaling, AdamW, learning rate, epochs, "
            "batch sequence and weights; parameter difference <0.1%"
        ),
    }
    ...
    try:
        for seed in seeds:
            ...
            candidate_manifest = json.loads(
                (run_root / "model_comparisons" / comparison / "training_manifest.json").read_text(encoding="utf-8")
            )
            for fold, ids, valid, cutoff in scopes:
                labels = ledger.latest(cutoff)
                assert all(
                    labels[(e, t)].quality_eligible and labels[(e, t)].value is not None
                    for e in ids
                    for t in ("cu", "as")
                )
                ...
                for method in methods:
                    ...
                    candidate = next(
                        a for a in candidate_manifest["artifacts"] if a["fold_id"] == fold and a["method_id"] == method
                    )
                    ...
                    assert model.fit_metadata["parameter_budget_relative_difference"] < 0.001
                    ...
                    if valid:
                        ...
                        for e, value in zip(valid, prediction, strict=True):
                            rows.append(
                                {
                                    "event_id": e,
                                    "fold_id": fold,
                                    "method_id": controls[method],
                                    "seed": seed,
                                    "cu": float(value[0]),
                                    "as": float(value[1]),
                                    "status": "completed",
        ...
        for method, control in controls.items():
            for target in ("cu", "as"):
                ...
                for seed in seeds:
                    main = (
                        table[(table.method_id == method) & (table.seed == seed)]
                        .set_index("event_id")
                        .loc[events, target]
                        .to_numpy()
                    )
                    reference = (
                        table[(table.method_id == control) & (table.seed == seed)]
                        .set_index("event_id")
                        .loc[events, target]
                        .to_numpy()
                    )
                ...
                paired[method + ":" + target] = {
                    "candidate_minus_mlp_mae": float(differences.mean()),
                    "ci95": hierarchical_interval(differences, dates),
                }
        result.update(
            schema_version="neural-control-evaluation.g6j.v1",
            new_method_count=0,
            candidate_vs_matched_mlp=paired,
            resources=resources,
