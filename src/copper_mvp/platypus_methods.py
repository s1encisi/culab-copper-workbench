...
def load_platypus():
    ...
    try:
        ...
        raise WorkbenchError(
            "请运行 scripts/setup_platypus_methods.ps1 安装可选优化依赖", "OPTIMIZER_DEPENDENCY"
        ) from None
    if version != "1.4.1":
...
def platypus_method_spec(name):
    definitions = {
        "IBEA": (
            "O05",
            "binary hypervolume indicator fitness and iterative fitness removal",
            "adapted_constraint_first_fitness_comparator",
        ),
        "Epsilon-MOEA": ("O09", "epsilon-box archive and population/archive mating", "native_constraint_dominance"),
        "SMPSO": (
            "O11",
            "constricted velocity, leader archive and polynomial perturbation",
        ...
        "MO-CMA-ES": (
            "O14",
            "covariance adaptation with nondominated rank/crowding survival",
            "native_constraint_dominance",
        ),
    }
    ...
    parameters = {
        ...
        "tail_budget": "evaluate_native_order_prefix_then_finish_without_incomplete_state_update",
        "randomness": "per_run_context_local_python_Random",
    }
    ...
    if name == "IBEA":
        parameters.update(
            kappa=0.05,
            indicator_reference=2.0,
            indicator_objectives="shared_scaled_raw_objectives",
    ...
    if name == "SMPSO":
        parameters.update(
            swarm_size=64,
            leader_capacity=64,
            velocity="native_constriction_and_half_box_limit",
            mutation="PM",
            mutation_eta=20,
        )
    ...
    if name == "PAES":
        parameters.update(
            parents=1,
            initial_parent="seeded_choice_from_shared_nondominated_samples",
            archive_initialization="all_shared_initial_samples",
            mutation="PM",
            mutation_eta=20,
        )
    if name == "MO-CMA-ES":
    ...
    return {
        ...
        "display_name": name,
        "design_id": design,
        "method_version": "g6d.fixed.v2",
        "mechanism": mechanism,
        "constraint_handling": constraints,
        "parameters": parameters,
        "dependencies": {"Platypus-Opt": "1.4.1"},
        "variables": "continuous",
        "objectives": 2,
        "inequality_constraints": True,
        "package": "Platypus-Opt",
        ...
        def evaluate(self, solutions):
            ...
            for i, solution in enumerate(solutions):
                solution.fitness = sum(
                    math.exp(-self.fitcomp[j][i] / self.max_fitness / self.kappa)
...
class PlatypusAdapter:
    ...
    def setup(self, problem, termination=None, seed=None, verbose=False):
        ...
        initial = []
        for x, f, g in zip(self.initial.get("X"), self.initial.get("F"), self.initial.get("G"), strict=True):
        ...
        variator = p.GAOperator(
            p.SBX(probability=0.9, distribution_index=15), p.PM(probability=1 / problem.n_var, distribution_index=20)
        )
        ...
        if self.name == "IBEA":
            self.native = p.IBEA(
                ...
                population_size=64,
                variator=variator,
                fitness_evaluator=FixedIndicator(kappa=0.05, rho=2.0),
                fitness_comparator=ConstraintFitness(),
                **options,
            )
        elif self.name == "Epsilon-MOEA":
            self.native = p.EpsMOEA(
                native_problem, epsilons=[0.01, 0.01], population_size=64, variator=variator, **options
            )
            ...
            self.native = p.PAES(
                ...
                divisions=8,
                capacity=64,
                generator=SharedGenerator([parent]),
                variator=p.PM(probability=1 / problem.n_var, distribution_index=20),
            )
        elif self.name == "PESA-II":
            self.native = p.PESA2(
                native_problem, population_size=64, divisions=8, capacity=64, variator=variator, **options
            )
        elif self.name == "MO-CMA-ES":
            self.native = SharedCMAES(
                native_problem,
                offspring_size=64,
                sigma=0.2,
        ...
        class SharedEvaluator(p.Evaluator):
            def evaluate_all(self, jobs, **kwargs):
                ...
                if selected:
                    ...
                    for job, f, g in zip(selected, F / owner.scale, G - 1e-8, strict=True):
