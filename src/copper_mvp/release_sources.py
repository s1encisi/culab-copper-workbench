...
from copper_mvp.model_training import load_registered_model, safe_artifact_path, source_signature
...
class ArtifactSources:
    ...
    def load(self, actor, request):
        ...
        if request.source_kind == "model_comparison":
            ...
            entries = [
                a
                for a in training["artifacts"]
                if a["method_id"] == method and a["status"] == "completed" and a["fold_id"] != "DEVELOPMENT"
            ]
            artifacts = [
                {
                    "fold_id": a["fold_id"],
                    "path": a["path"],
                    "sha256": a["sha256"],
                    "fit_cutoff_at": a["fit_cutoff_at"],
                    "kind": "registered_model",
                    "train_event_hash": a["train_event_hash"],
                }
                for a in entries
            ]
            ...
            quality = {
                ...
                "n": metrics["n"],
                "paired_ci95": metrics["paired_mae_ci95"],
                "negative_predictions": metrics["negative_predictions"],
                "coverage": result["coverage"][method]["coverage"],
                "p95_ms": resource["single_predict_p95_ms"],
                "mode_metrics": {},
                "baseline_modes": {},
            }
            if method == "BART":
                quality["sampling_diagnostics_passed"] = all(
                    a.get("fit_metadata", {}).get("diagnostics_passed", False) for a in entries
                )
            ...
            for mode in sorted({v["mode_code"] for v in self.data.mode_cards.values()}):
                events = [
                    e
                    for e in candidate_rows.index
                    if self.data.mode_cards[e]["mode_code"] == mode
                    and (e, target) in labels
                    and labels[(e, target)].quality_eligible
                    and labels[(e, target)].value is not None
                ...
                if events:
                    ...
                    quality["mode_metrics"][mode] = {
                        "n": len(events),
                        "mae_mean_over_seeds": float(
                            np.abs(candidate_rows.loc[events, target].to_numpy() - truth).mean()
                        ),
                    }
                    quality["baseline_modes"][mode] = {
                        "n": len(events),
                        "mae_mean_over_seeds": float(
                            np.abs(baseline_rows.loc[events, target].to_numpy() - truth).mean()
                        ),
                    }
            bindings = {
                "protocol.json": file_hash(root / "protocol.json"),
                "training_manifest.json": file_hash(root / "training_manifest.json"),
                "oof_predictions.csv": training["oof_sha256"],
            ...
            for fold in manifest["folds"]:
                ...
                entry = next(
                    (v for v in fold["artifacts"] if v.get("method", v["name"]) == method and v.get("seed") == seed),
                    None,
                )
                if entry is None:
                    raise WorkbenchError("集成模型工件不完整", "ARTIFACT_SOURCE")
                artifacts.append(
                    {
                        "fold_id": fold["fold_id"],
                        "path": (Path(fold["directory"]) / entry["path"]).as_posix(),
                        "sha256": entry["sha256"],
            ...
            baseline = next(v for v in result["metrics"] if v["method_id"] == "Persistence" and v["target"] == target)
            coverage = [
                v["valid_predictions"] / v["expected_events"]
            ...
            quality = {
                ...
                "baseline_mae": baseline["mae"],
                "n": metric["n"],
                "paired_ci95": metric["paired_ci95"],
                "negative_predictions": metric["negative_predictions"],
                "coverage": min(coverage),
                "p95_ms": result["resources"][method]["p95_ms"],
                "mode_metrics": metric["mode_metrics"],
                "baseline_modes": baseline["mode_metrics"],
                "seed_scope": "All predeclared seeds; selecting an artifact seed does not discard adverse seeds.",
            }
            bindings = {
                "protocol.json": file_hash(root / "protocol.json"),
                "replay_manifest.json": file_hash(root / "replay_manifest.json"),
                "predictions.csv": manifest["predictions_sha256"],
                "decisions.jsonl": manifest["decisions_sha256"],
                "evaluation.json": state["evaluation_sha256"],
            }
        ...
        if method in ("TabNet", "FTTransformer", "NODE"):
            ...
            tabular_runtime()
            dependencies.update({name: metadata.version(name) for name in family["dependencies"]})
        if family.get("input_kind") == "anchored_process_sequence":
        ...
        descriptor = safe(
            {
                ...
                "feature_spec_hash": digest(self.data.feature_columns),
                "source": source,
                "bindings": bindings,
                "artifacts": artifacts,
                "benchmark": quality,
                ...
                "license": {
                    "project": "Not declared in the project metadata; no new license is assigned here",
                    "dependencies": {
                        name: (
                            metadata.metadata(name).get("License-Expression")
                            or metadata.metadata(name).get("License")
                            or "See installed distribution metadata"
                        )[:160]
                        for name in dependencies
                    },
                },
                "calibrator": None,
                "calibrator_missing_reason": "No prediction-interval calibration was fitted for these artifacts",
                "scopes": ["historical_oof"],
                "proxy_approved": False,
                "causal_control": False,
                "evaluation_mode": "historical_replay",
                "benchmark_event_hash": result["common_event_hash"],
            }
        )
    ...
    def verify(self, descriptor):
        ...
        for artifact in descriptor["artifacts"]:
            ...
            if (
                artifact.get("calibrator_path")
                and file_hash(safe_artifact_path(root, artifact["calibrator_path"])) != artifact["calibrator_sha256"]
            ):
        ...
        if descriptor["method_id"] in ("TabNet", "FTTransformer", "NODE"):
            ...
            tabular_runtime()
        if descriptor.get("family", {}).get("input_kind") == "anchored_process_sequence":
            from copper_mvp.neural_runtime import load_tensor_runtime
    ...
    def event(self, event_id):
        ...
        return {
            "event_id": event_id,
            "decision_at": source_time(row.decision_at).isoformat(),
            "snapshot_id": snapshot["id"],
            "dataset_version": self.data.dataset_version,
            "current": {"cu": finite(row.origin_cu_g_l), "as": finite(row.origin_as_mg_l)},
        }
    ...
    def predict(self, descriptor, event):
        ...
        if entry["kind"] == "stateful_replay":
            ...
            return replay["values"][event][target], {
                "artifact_sha256": entry["sha256"],
                "execution": "recomputed_stateful_replay",
                "model_elapsed_ms": replay["event_ms"][event],
                "replay_total_ms": replay["total_ms"],
            }
        key = entry["sha256"]
        ...
        with threadpool_limits(limits=1):
            values = (
                self.cache[key].predict_context(self.data, [event])
                if descriptor["source_kind"] == "model_comparison"
                else self.cache[key].predict(self.data.X.loc[[event]].to_numpy(float))
            )
        return float(values[0, ("cu", "as").index(descriptor["target"])]), {
            "artifact_sha256": key,
    ...
    def dynamic_replay(self, descriptor, entry):
        ...
        mixers = [
            AdaptiveTarget(
                ...
                t,
                state["parameters"][t],
                state["initial_weights"][t],
                state["scale"][t],
                [(e, initial[i, :, t]) for i, e in enumerate(state["initial_history_event_ids"])],
                EnsembleSettings(**state["settings"]),
            )
            for t in (0, 1)
        ]
    ...
    def probe(self, descriptor, samples_per_fold):
        ...
        for fold in sorted(set(self.data.fold_for.values())):
            ...
            for position in positions:
                ...
                start = time.perf_counter()
                value, evidence = self.predict(descriptor, event)
                elapsed = (time.perf_counter() - start) * 1000
                timings.append(evidence.get("model_elapsed_ms", elapsed))
                ...
                if descriptor["method_id"] in NEURAL_METHODS:
                    ...
                    valid, numerical = neural_replay_error(
                        value, expected, origin, scale, method=descriptor["method_id"]
                    )
        ...
        self.verify(descriptor)
        return {
            "scope": "historical_runtime_shadow",
            "independent_new_labels": False,
            "samples": len(rows),
            "parity_passed": all(r["parity"] for r in rows),
