...
def classical_router():
    ...
    def state(root, result=False):
        ...
        if result and value["status"] in ("completed", "completed_with_missing_predictions"):
            ...
            if (
                file_hash(root / "protocol.json") != evaluation["protocol_sha256"]
                or file_hash(root / "predictions.csv") != evaluation["predictions_sha256"]
            ):
    ...
    def studies(request: Request):
        ...
        return {
            "items": sorted(
                [state(p.parent) for p in root.glob("*/state.json")], key=lambda v: v["created_at"], reverse=True
            )
        }
    ...
    def export(request: Request, identifier: str):
        root = folder(request, identifier)
        current = state(root, True)
