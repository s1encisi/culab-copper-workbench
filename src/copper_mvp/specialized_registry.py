...
def specialized_spec(name, seed):
    ...
    record = {
        "schema_version": "specialized-model-registry.g6g.v1",
        "method_id": name,
        "method_version": "g6g.fixed.v2",
        "implementation": {
            "CatBoost": "CatBoostRegressor",
            "NGBoost": "NGBRegressor Normal LogScore natural gradient",
            "EBM": "ExplainableBoostingRegressor",
            "Cubist": "Cubist rule-based linear regression",
        }[name],
        "package_version": PACKAGES[package],
        "dependencies": {package: PACKAGES[package]},
        "requires_fit": True,
        "status": "registered",
        "feature_count": 114,
        "multi_output": "two_independent_estimators",
        "target_transform": "delta_from_current",
        "target_scaling": "train_only_standard_delta",
        "preprocessing": "native_numeric_missing_and_explicit_nominal_modes"
        if name in ("CatBoost", "EBM")
        else "train_median_missing_indicators_stable_scale_and_fixed_one_hot",
        "sample_weight": "unsupported" if name == "Cubist" else "native",
        "internal_validation": "disabled",
        "training_order": "input_order_used; comparison_must_verify_chronology"
        if name == "CatBoost"
        else "training_rows_only",
        "uncertainty": "uncalibrated_normal_marginals" if name == "NGBoost" else "unsupported",
        "targets": [{"name": "cu", "unit": "g/L"}, {"name": "as", "unit": "mg/L"}],
        "capabilities": {
            "sample_weight": name != "Cubist",
            "uncertainty": "marginal_std" if name == "NGBoost" else "unsupported",
        },
        "preset_parameters": dict(PARAMETERS[name]),
        "seed": seed,
