...
def train_comparison(data, request: ComparisonRequest, output: Path, progress=lambda detail: None):
    ...
    for fold in sorted(data.cv.fold_id.unique()):
        ...
        if any(source_time(data.row(e).decision_at) >= cutoff for e in train_ids) or any(
            source_time(data.row(e).decision_at) < cutoff for e in valid_ids
        ):
    ...
    protocol = {
        "schema_version": "comparison-protocol.g2a.v1",
        ...
        **source,
        ...
        "expected_oof_events": len(all_validation),
        "parameter_policy": "fixed_before_evaluation_no_search",
        "seed_policy": "one_predeclared_seed_all_fold_results_retained",
        "comparison_cohort": "common_finite_two_target_predictions_and_mature_eligible_labels",
        "uncertainty": {
            "method": "paired_calendar_week_block_bootstrap",
            "replicates": 1000,
            "confidence": 0.95,
            "minimum_blocks": 20,
            "seed": request.seed,
        ...
        "latency_policy": {
            "scope": "warm_single_row_two_target_prediction",
            "samples_per_fold": 32,
            "selection": "evenly_spaced_validation_positions",
            "native_threads": 1,
        },
    ...
    if any(
        method_spec(m, request.seed).get("input_kind") in ("raw_event_sequence", "current_result_pair")
        ...
        protocol["latency_policy"]["event_context_methods"] = {
            "methods": [
                m for m in request.methods if method_spec(m, request.seed).get("input_kind") == "raw_event_sequence"
            ],
            "samples_per_fold": 8,
            "scope": "loaded_parameters_with_causal_event_context_reconstruction",
            "mean_and_std_computed_together": True,
        }
    ...
    scopes = [
        (
            f["fold_id"],
            data.training_ids(f["fold_id"]),
            data.cv.loc[
                data.cv.fold_id.eq(f["fold_id"]) & data.cv.fold_role.eq("VALIDATION"), "origin_event_id"
            ].tolist(),
            source_time(f["fit_cutoff_at"]),
        )
        for f in folds
    ]
    ...
    def persist(status):
        ...
        current = safe(
            {
                "schema_version": "comparison-training.g2a.v1",
                "status": status,
                "created_at": utc_now(),
                "evaluation_mode": "historical_replay",
                "source": source,
                "protocol_sha256": file_hash(output / "protocol.json"),
                "oof_sha256": file_hash(output / "oof_predictions.csv"),
                "artifacts": artifacts,
                "timings": timings,
                "runtime": runtime,
        ...
        for fold, train_ids, valid_ids, cutoff in scopes:
            ...
            for method_id in request.methods:
                ...
                entry = {
                    ...
                    "train_count": len(train_ids),
                    "train_event_hash": digest(train_ids),
                    "requires_fit": model.spec["requires_fit"],
                    "spec_hash": model.spec["content_hash"],
                }
                try:
                        ...
                        try:
                            if model.spec.get("input_kind") == "anchored_process_sequence":
                                model.fit_context(
                                    ...
                                    train_ids,
                                    cutoff,
                                    max_wall_seconds=request.max_wall_seconds - (time.perf_counter() - started),
                                )
                                ...
                                if method_id == "CatBoost":
                                    ...
                                    if any(a > b for a, b in zip(ordered_times, ordered_times[1:], strict=False)):
                    ...
                    entry.update(
                        {"path": path.relative_to(output).as_posix(), "sha256": file_hash(path), "status": "completed"}
                    )
                    if valid_ids:
                        clock = time.perf_counter()
                            ...
                            for i, event in enumerate(valid_ids):
                                for t, target in enumerate(("cu", "as")):
                                    ...
                                    if distribution["kind"] == "marginal_standard_deviation":
                                        ...
                                        row.update(
                                            {
                                                "q10": float(distribution["values"][i, 0, t]),
                                                "q50": float(distribution["values"][i, 1, t]),
                        ...
                        timing["prediction_scope"] = (
                            "event_context_reconstruction" if contextual else "single_row_matrix_inference"
                        )
                        positions = np.unique(
                            np.linspace(0, len(valid_ids) - 1, min(8 if contextual else 32, len(valid_ids))).astype(int)
                        )
...
def load_registered_model(root: Path, manifest: dict, protocol: dict, method_id: str, fold_id: str):
    ...
    if (
        recorded is None
        or entry["spec_hash"] != recorded["content_hash"]
    ...
    if (
        ...
        or model.method_id != method_id
        or model.seed != protocol["request"]["seed"]
        or model.spec["content_hash"] != entry["spec_hash"]
    ):
