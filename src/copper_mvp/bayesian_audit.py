...
def audit_bayesian(record, frame, prepared, folder):
    ...
    for row in iterations:
        ...
        expected_indices = np.arange(len(past)) if len(past) <= 256 else np.r_[np.arange(64), np.arange(len(past)-192, len(past))]
        ...
        if "acquisition_rng_state" in saved:
            with tensor_random_scope(torch, saved["iteration_seed"]), torch.no_grad():
                ...
                if name == "ParEGO":
                    ...
                    acq = qLogNParEGO(model, saved["train_X"], scalarization_weights=weights, sampler=sampler,
                        objective=objective, constraints=constraints or None, prune_baseline=False, cache_root=True)
                elif name == "NEHVI":
                    acq = qLogNoisyExpectedHypervolumeImprovement(model, ref_point=[-1.1, -1.1], X_baseline=saved["train_X"],
                        sampler=sampler, objective=objective, constraints=constraints or None, prune_baseline=False, cache_root=True)
                else:
        ...
        checked += 1
    return {"checked_iterations": checked, "extra_candidate_evaluations": extra, "gp_artifacts": artifacts,
