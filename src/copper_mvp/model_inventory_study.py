...
def run_inventory_study(
    data, run_root, reference_id, request_key, methods, seeds, progress=lambda value: None, max_wall_seconds=1800
):
    ...
    baseline_methods = [m for m in old_protocol["request"]["methods"] if m != "Persistence" and m not in methods]
    reference_seed = old_protocol["request"]["seed"]
    expected = {
        "Persistence": [seeds[0]],
        **{m: list(seeds) for m in methods},
    ...
    protocol = {
        "schema_version": "model-inventory-study.g6g.v1",
        "request_key": request_key,
        "methods": [method_spec(m, seeds[0]) for m in methods],
        "seeds": list(seeds),
        "expected_method_seeds": expected,
        "source": source,
        "code_hashes": code_hashes,
        "reference_comparison_id": reference_id,
        ...
        "max_wall_seconds_per_comparison": max_wall_seconds,
        "native_threads": 1,
    ...
    try:
        for seed in seeds:
            request = ComparisonRequest(
                request_key=f"{request_key}-seed-{seed}",
            ...
            state["comparisons"].append({"seed": seed, "comparison_id": identifier, "status": current["status"]})
            write_json(root / "state.json", state)
        ...
        if source_signature(data) != source or any(file_hash(PROJECT_ROOT / p) != h for p, h in code_hashes.items()):
        ...
        for item in state["comparisons"]:
            ...
            bindings.append(
                {
                    **item,
                    "training_sha256": file_hash(folder / "training_manifest.json"),
        ...
        result.update(
            schema_version="model-inventory-evaluation.g6g.v1",
            new_method_count=len(methods),
            registered_total=len(METHOD_IDS),
            compared_methods=len(expected),
            resources=resources,
            uncertainty_metrics=uncertainty,
            reference_resources=reference["result"]["resources"],
            source_comparisons=bindings,
        ...
        lines += [
            "",
            "概率指标、逐种子结果、时间块配对区间、分工况误差与资源记录见完整评价。不确定性按各方法的登记合同报告，尚未进行独立时序校准。",
            "",
