...
class ResearchStore:
        ...
        with self.connection() as c:
            ...
            c.executescript("""
                CREATE TABLE IF NOT EXISTS research_sessions (
                    ...
                    title TEXT NOT NULL, context_json TEXT NOT NULL, created_at TEXT NOT NULL);
                CREATE TABLE IF NOT EXISTS research_messages (
                    id INTEGER PRIMARY KEY AUTOINCREMENT, session_id TEXT NOT NULL, role TEXT NOT NULL,
                    text TEXT NOT NULL, task_id TEXT, created_at TEXT NOT NULL);
                CREATE TABLE IF NOT EXISTS research_tasks (
                    id TEXT PRIMARY KEY, session_id TEXT NOT NULL, owner_id TEXT NOT NULL,
                    request_key TEXT NOT NULL, fingerprint TEXT NOT NULL, request_json TEXT NOT NULL,
                    status TEXT NOT NULL, version INTEGER NOT NULL DEFAULT 1,
                    worker_id TEXT, fence INTEGER NOT NULL DEFAULT 0, lease_until REAL,
                    checkpoint_json TEXT NOT NULL DEFAULT '{}', result_json TEXT, error_json TEXT,
                    elapsed_ms REAL NOT NULL DEFAULT 0, model_calls INTEGER NOT NULL DEFAULT 0,
                    tool_calls INTEGER NOT NULL DEFAULT 0, cache_hits INTEGER NOT NULL DEFAULT 0,
                ...
                CREATE TABLE IF NOT EXISTS research_evidence (
                    ...
                    payload_json TEXT NOT NULL, content_hash TEXT NOT NULL, source_version TEXT NOT NULL, input_hash TEXT NOT NULL,
                    created_at TEXT NOT NULL);
                CREATE TABLE IF NOT EXISTS research_calls (
                    call_id TEXT PRIMARY KEY, task_id TEXT NOT NULL, session_id TEXT NOT NULL,
                    owner_id TEXT NOT NULL, requested_day TEXT NOT NULL, usage_json TEXT, outcome TEXT NOT NULL);
                CREATE INDEX IF NOT EXISTS research_task_session ON research_tasks(session_id,created_at);
                CREATE INDEX IF NOT EXISTS research_event_task ON research_events(task_id,seq);
                CREATE INDEX IF NOT EXISTS research_evidence_task ON research_evidence(task_id);
            """)

    def event(self, c, task_id, kind, payload):
        c.execute("INSERT INTO research_events(task_id,kind,payload_json,created_at) VALUES(?,?,?,?)",
    ...
    def create_session(self, actor, title, context):
        ...
        with self.connection() as c:
            c.execute("INSERT INTO research_sessions VALUES(?,?,?,?,?,?)",
    ...
    def session(self, actor, session_id):
        with self.connection() as c:
            row = c.execute("SELECT * FROM research_sessions WHERE id=? AND owner_id=? AND project_id=?",
                            (session_id, actor.user_id, actor.project_id)).fetchone()
            if row is None:
                raise WorkbenchError("没有找到该会话", "SESSION_NOT_FOUND")
            messages = c.execute("SELECT role,text,task_id,created_at FROM research_messages WHERE session_id=? ORDER BY id", (session_id,)).fetchall()
        result = dict(row); result["context"] = json.loads(result.pop("context_json"))
        result["messages"] = [dict(m) for m in messages]
        return result

    def sessions(self, actor):
        with self.connection() as c:
            rows = c.execute("SELECT id,title,created_at FROM research_sessions WHERE owner_id=? AND project_id=? ORDER BY created_at DESC LIMIT 100",
                             (actor.user_id, actor.project_id)).fetchall()
    ...
    def create_task(self, actor, session_id, request):
        ...
        with self.connection() as c:
            ...
            old = c.execute("SELECT id,fingerprint FROM research_tasks WHERE owner_id=? AND request_key=?",
                            (actor.user_id, request["request_key"])).fetchone()
            if old:
                if old["fingerprint"] != fingerprint:
                    raise WorkbenchError("该请求键已用于不同问题或上下文", "REQUEST_CONFLICT")
                identifier, reused = old["id"], True
            else:
                active = c.execute("SELECT id FROM research_tasks WHERE session_id=? AND status IN ('queued','running','pausing','cancelling','paused')", (session_id,)).fetchone()
                if active:
                    raise WorkbenchError("请先完成、取消或恢复当前会话的任务", "SESSION_BUSY")
                identifier, reused = uuid.uuid4().hex, False
                c.execute("""INSERT INTO research_tasks(id,session_id,owner_id,request_key,fingerprint,request_json,status,created_at)
                             VALUES(?,?,?,?,?,?,'queued',?)""",
                          (identifier, session_id, actor.user_id, request["request_key"], fingerprint, dumps(request), utc_now()))
                c.execute("UPDATE research_sessions SET context_json=? WHERE id=?", (dumps(request["context"]), session_id))
                c.execute("INSERT INTO research_messages(session_id,role,text,task_id,created_at) VALUES(?,'user',?,?,?)",
    ...
    def task(self, actor, task_id):
        ...
        with self.connection() as c:
            task["usage"] = dict(c.execute("""SELECT COALESCE(SUM(b.spent),0) spent_cny,
                COALESCE(SUM(CASE WHEN b.spent IS NULL THEN b.reserved ELSE 0 END),0) unsettled_cny
                FROM budget b JOIN research_calls r ON r.call_id=b.call_id WHERE r.task_id=?""", (task_id,)).fetchone())
        return task

    def events(self, actor, task_id, after=0):
        self.session(actor, self.internal_task(task_id)["session_id"])
        with self.connection() as c:
            rows = c.execute("SELECT * FROM research_events WHERE task_id=? AND seq>? ORDER BY seq LIMIT 1000", (task_id, after)).fetchall()
    ...
    def claim(self, task_id, worker_id, lease_seconds=180):
        ...
        with self.connection() as c:
            ...
            c.execute("""UPDATE research_tasks SET worker_id=?,fence=?,lease_until=?,status=?,
    ...
    def _owned(self, c, task_id, worker_id, fence):
        row = c.execute("SELECT * FROM research_tasks WHERE id=?", (task_id,)).fetchone()
        if not row or row["worker_id"] != worker_id or row["fence"] != fence or (row["lease_until"] or 0) <= time.time():
            raise WorkbenchError("任务租约已被其他执行器接管", "LEASE_LOST")
        return row

    def renew(self, task_id, worker_id, fence):
        with self.connection() as c:
            count = c.execute("""UPDATE research_tasks SET lease_until=? WHERE id=? AND worker_id=? AND fence=?
                                 AND status IN ('running','pausing','cancelling')""",
                              (time.time() + 180, task_id, worker_id, fence)).rowcount
        return bool(count)

    def boundary(self, task_id, worker_id, fence, checkpoint, elapsed_ms):
        with self.connection() as c:
            c.execute("BEGIN IMMEDIATE"); row = self._owned(c, task_id, worker_id, fence)
            status = "paused" if row["status"] == "pausing" else "cancelled" if row["status"] == "cancelling" else "running"
            changed = int(status != row["status"])
            c.execute("UPDATE research_tasks SET checkpoint_json=?,elapsed_ms=?,status=?,version=version+? WHERE id=?",
    ...
    def add_evidence(self, task_id, worker_id, fence, tool, payload, source_version, input_hash, cached=False):
        ...
        with self.connection() as c:
            c.execute("BEGIN IMMEDIATE"); self._owned(c, task_id, worker_id, fence)
            c.execute("INSERT OR IGNORE INTO research_evidence VALUES(?,?,?,?,?,?,?,?)",
                      (task_id + ":" + identifier, task_id, tool, dumps(payload), signature, source_version, input_hash, utc_now()))
            if cached:
    ...
    def cached_evidence(self, task_id, input_hash):
        with self.connection() as c:
            row = c.execute("SELECT * FROM research_evidence WHERE task_id=? AND input_hash=?", (task_id, input_hash)).fetchone()
        if row:
            return {"evidence_id": row["id"].split(":", 1)[1], "tool": row["tool"],
                    "data": json.loads(row["payload_json"]), "content_hash": row["content_hash"]}
        return None
    ...
    def evidence(self, task_id):
        with self.connection() as c:
            rows = c.execute("SELECT * FROM research_evidence WHERE task_id=? ORDER BY created_at,id", (task_id,)).fetchall()
        return [{"evidence_id": r["id"].split(":", 1)[1], "tool": r["tool"], "data": json.loads(r["payload_json"]),
                 "content_hash": r["content_hash"], "source_version": r["source_version"]} for r in rows]

    def reserve_call(self, task_id, worker_id, fence, amount, month, day, settings):
        if not math.isfinite(amount) or amount < 0:
            raise WorkbenchError("费用估计无效", "BUDGET_INPUT")
        with self.connection() as c:
            c.execute("BEGIN IMMEDIATE"); task = self._owned(c, task_id, worker_id, fence)
            ...
            task_used = c.execute(f"SELECT {measure} FROM budget b JOIN research_calls r ON r.call_id=b.call_id WHERE r.task_id=?", (task_id,)).fetchone()[0]
            session_used = c.execute(f"SELECT {measure} FROM budget b JOIN research_calls r ON r.call_id=b.call_id WHERE r.session_id=?", (task["session_id"],)).fetchone()[0]
            day_used = c.execute(f"SELECT {measure} FROM budget b JOIN research_calls r ON r.call_id=b.call_id WHERE r.owner_id=? AND r.requested_day=?", (task["owner_id"], day)).fetchone()[0]
            if (global_used + amount > settings.monthly_limit_cny or task_used + amount > settings.max_cost_cny
                or session_used + amount > 10 or day_used + amount > 20):
    ...
    def unknown_calls(self, task_id):
        with self.connection() as c:
            return c.execute("SELECT COUNT(*) FROM research_calls WHERE task_id=? AND outcome='unknown'", (task_id,)).fetchone()[0]

    def settle_call(self, call_id, usage):
        with self.connection() as c:
            c.execute("BEGIN IMMEDIATE")
            c.execute("UPDATE budget SET spent=?,status='settled' WHERE call_id=?", (usage["estimated_cost_cny"], call_id))
    ...
    def finish(self, task_id, worker_id, fence, result, elapsed_ms):
        with self.connection() as c:
            ...
            c.execute("""UPDATE research_tasks SET status='completed',result_json=?,elapsed_ms=?,finished_at=?,
                         worker_id=NULL,lease_until=NULL,version=version+1 WHERE id=?""",
                      (dumps(result), elapsed_ms, utc_now(), task_id))
            c.execute("INSERT INTO research_messages(session_id,role,text,task_id,created_at) VALUES(?,'assistant',?,?,?)",
    ...
    def fail(self, task_id, worker_id, fence, code, message, elapsed_ms):
        with self.connection() as c:
            c.execute("BEGIN IMMEDIATE"); self._owned(c, task_id, worker_id, fence)
            c.execute("""UPDATE research_tasks SET status='failed',error_json=?,elapsed_ms=?,finished_at=?,
                         worker_id=NULL,lease_until=NULL,version=version+1 WHERE id=?""",
    ...
    def pending(self):
        with self.connection() as c:
            rows = c.execute("""SELECT id FROM research_tasks WHERE status='queued'
                OR (status IN ('running','pausing','cancelling') AND COALESCE(lease_until,0)<?)""", (time.time(),)).fetchall()
