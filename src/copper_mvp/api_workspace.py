...
def workspace_router():
    ...
    def context(request):
        ...
        if actor.project_id != PROJECT:
            raise WorkbenchError("项目不可访问", "FORBIDDEN")
    ...
    def summary(request: Request):
        ...
        with wb.store.connection() as c:
            own = c.execute(
                "SELECT status,COUNT(*) n FROM research_tasks WHERE owner_id=? GROUP BY status", (actor.user_id,)
            ).fetchall()
            if actor.role == "owner":
                budget = c.execute(
                    "SELECT COUNT(*) calls,COALESCE(SUM(spent),0) spent,COALESCE(SUM(CA"
                    "SE WHEN spent IS NULL THEN reserved ELSE 0 END),0) reserved FROM b"
                    "udget"
                ).fetchone()
                scope = "project"
            else:
                budget = c.execute(
                    """SELECT COUNT(*) calls,COALESCE(SUM(b.spent),0) spent,
                    COALESCE(SUM(CASE WHEN b.spent IS NULL THEN b.reserved ELSE 0 END),0) reserved
                    FROM budget b JOIN research_calls r ON r.call_id=b.call_id WHERE r.owner_id=?""",
                    (actor.user_id,),
                ).fetchone()
                scope = "own_research"
        return {
            ...
            "assistant_live": wb.research.allow_live,
            "mock_configured": wb.control.client is not None,
            ...
            "capabilities": {
                ...
                "domain_adaptation": hasattr(wb, "domain_adaptation"),
            },
        }
    ...
    def tasks(request: Request):
        ...
        with wb.store.connection() as c:
            research = c.execute(
                """SELECT id,status,version,created_at,elapsed_ms,model_calls,tool_calls,request_json,error_json
                                  FROM research_tasks WHERE owner_id=? ORDER BY created_at DESC LIMIT 100""",
                (actor.user_id,),
            ).fetchall()
        values = []
        for row in research:
            ...
            values.append(
                {
                    "id": row["run_id"],
                    "kind": "run",
                    "title": run_labels.get(row["task_type"], row["task_type"]),
                    "task_type": row["task_type"],
                    "status": row["status"],
                    "created_at": row["created_at"],
                    "elapsed_ms": row.get("duration_ms"),
                    "error": row.get("error"),
                }
            )
        for folder, kind in (
            ("model_comparisons", "model_comparison"),
            ("optimizer_comparisons", "optimizer_comparison"),
            ("calibration_studies", "calibration"),
        ):
            for path in sorted((wb.root / folder).glob("*/state.json"), key=lambda p: p.stat().st_mtime, reverse=True)[
                :60
            ]:
                state = observed_job_state(json.loads(path.read_text(encoding="utf-8")))
                values.append(
                    {
                        "id": state.get("run_id", state.get("id", path.parent.name)),
                        "kind": kind,
    ...
    def governance(request: Request):
        ...
        with wb.store.connection() as c:
            rows = c.execute(
                (
                    "SELECT e.kind,e.payload_json,e.task_id FROM research_events e\n"
                    "                              JOIN research_tasks t ON t.id=e.task"
                    "_id WHERE t.owner_id=? ORDER BY e.seq DESC LIMIT 100"
                ),
                (actor.user_id,),
            ).fetchall()
            usage = c.execute(
                """SELECT r.usage_json FROM research_calls r WHERE r.owner_id=? AND r.outcome='known'""",
                (actor.user_id,),
            ).fetchall()
        ...
        return {
            "scope": "own_research",
            "events": [
                {"kind": v["kind"], "task_id": v["task_id"], "detail": json.loads(v["payload_json"])} for v in rows
            ],
            "usage": {
                "calls": len(usage),
                "input_tokens": sum(v.get("input_tokens", 0) for v in usage),
