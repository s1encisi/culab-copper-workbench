...
from copper_mvp.ensemble_models import make_meta,simplex_weights,expand_weights
...
class AdaptiveTarget:
    ...
    def predict(self,event,predictions,*,evidence=True):
        ...
        strength=0.
        if enough and local:
            local_w=simplex_weights(np.array([row[1][list(subset)] for row in local]),
                np.array([row[2].value for row in local]),self.scale,self.parameters["penalty"],self.weights[list(subset)])
        ...
        detail=None
        if evidence:
            detail={"schema_version":"adaptive-weights.g5b.v1","evaluation_mode":"historical_replay",
                "event_id":event,"target":("cu","as")[self.target],"as_of":cutoff.isoformat(),
                "parameters":self.parameters,"status":"MATURE_WINDOW" if enough else "INITIAL_OOF_WEIGHTS",
                "mature_events":len(eligible),"mode_events":len(local),"mode":mode,"local_strength":strength,
                "event_ids":[row[0] for row in eligible],"label_record_ids":[row[2].record_id for row in eligible],
                "cohort_hash":key,"global_weights":self.weights.tolist(),"weights":weights.tolist(),
...
def choose_meta(book,P,current,events,meta_train,meta_valid,meta_cutoff,outer_cutoff,settings,scale):
    ...
    a=[positions[e] for e in meta_train];b=[positions[e] for e in meta_valid]
    ...
    for target in (0,1):
        ...
        for subset in settings.base_subsets:
            for alpha in settings.ridge_alphas:
                ...
                stacks.append({"target":target,"subset":list(subset),"alpha":alpha,
                    "score":float(np.abs(values-y_valid[:,target]).mean()),"validation_events":len(b)})
            for penalty in settings.convex_penalties:
                ...
                convex.append({"target":target,"subset":list(subset),"penalty":penalty,
                    "score":float(np.abs(values-y_valid[:,target]).mean()),"validation_events":len(b)})
                for window in settings.dynamic_windows:
                    ...
                    mixer=AdaptiveTarget(book,target,parameters,expand_weights(w,subset),scale[target],history,settings)
                    predicted=[mixer.predict(e,P[positions[e],:,target],evidence=False)[0] for e in meta_valid]
                    adaptive.append({"target":target,**parameters,"score":float(np.abs(np.asarray(predicted)-y_valid[:,target]).mean()),
                                     "validation_events":len(b)})
        chosen["stacking"].append(select_with_tolerance(stacks,settings.selection_tolerance,
            lambda r:(len(r["subset"]),-r["alpha"],r["score"])))
        chosen["convex"].append(select_with_tolerance(convex,settings.selection_tolerance,
            lambda r:(len(r["subset"]),-r["penalty"],r["score"])))
        chosen["adaptive"].append(select_with_tolerance(adaptive,settings.selection_tolerance,
            lambda r:(len(r["subset"]),-r["penalty"],-r["window_events"],r["score"])))
        candidates["stacking"].extend(stacks);candidates["convex"].extend(convex);candidates["adaptive"].extend(adaptive)
    return {"selected":chosen,"candidates":candidates,
            "meta_selection_train":book.evidence(meta_train,meta_cutoff),"meta_selection_validation_ids":meta_valid,
