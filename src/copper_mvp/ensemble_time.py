...
class EnsembleData:
    ...
    def evidence(self,events,cutoff):
        ...
        return {"fit_cutoff_at":source_time(cutoff).isoformat(),"train_count":len(events),
                "train_event_hash":digest(events),"label_record_hash":digest([r.record_id for r in records]),
    ...
    def inner_plan(self,train_events,outer_cutoff,settings):
        ...
        cuts=[self.decision(events[min(len(events)-1,int(len(events)*f))])
              for f in settings.inner_boundaries[:-1]]
        plan=[]
        for index,cutoff in enumerate(cuts):
            ...
            plan.append({"inner_fold":index+1,"train_event_ids":train,"validation_event_ids":valid,
                         "validation_event_hash":digest(valid),**self.evidence(train,cutoff)})
        return plan
...
def block_sample(events,decisions,days,seed):
    ...
    return indices,{"seed":seed,"block_days":days,"source_blocks":len(blocks),"drawn_blocks":draws.tolist(),
                    "sample_count":len(indices),"sample_event_hash":digest([events[i] for i in indices]),
