...
from copper_mvp.calibration import SplitC90,CALIBRATION_VERSION
...
from copper_mvp.model_training import source_signature,safe_artifact_path
from copper_mvp.release_contracts import TASK_ID,TARGET_UNITS
...
def load_calibrated_source(owner,actor,request):
    ...
    entries=[v for v in training["artifacts"] if v["method_id"]==request.method_id and v["seed"]==seed
             and v["status"]=="completed" and v["fold_id"]!="DEVELOPMENT"]
    ...
    artifacts=[{"fold_id":v["fold_id"],"path":v["model_path"],"sha256":v["model_sha256"],
                "fit_cutoff_at":v["calibration_fit_cutoff_at"],"base_fit_cutoff_at":v["base_fit_cutoff_at"],
                "kind":"calibrated_model","train_event_hash":splits[v["fold_id"]]["fit_event_hash"],
                "calibration_event_hash":splits[v["fold_id"]]["calibration_event_hash"],
                "calibrator_path":v["calibrator_path"],"calibrator_sha256":v["calibrator_sha256"],
                "model_spec_hash":v["model_spec_hash"]} for v in entries]
    ...
    for method,seeds in protocol["expected_method_seeds"].items():
        for current_seed in seeds:
            ...
            needed=["cu","as","marginal_c90_cu_lower","marginal_c90_cu_upper","marginal_c90_as_lower","marginal_c90_as_upper"]
    ...
    common={e for e in common if all((e,t) in labels and labels[(e,t)].quality_eligible and labels[(e,t)].value is not None for t in ("cu","as"))}
    ...
    timings=[t for row in training["artifacts"] if row["method_id"]==request.method_id and row["fold_id"]!="DEVELOPMENT"
             for t in row.get("single_prediction_ms",[])]
    quality={"mae":float(errors.mean()),"baseline_mae":float(np.abs(baseline-truth).mean()),"n":len(events),
             "paired_ci95":paired_week_interval(delta,decisions,{"minimum_blocks":20,"replicates":1000,"seed":protocol["request"]["seeds"][0]}),
             ...
             "p95_ms":float(np.percentile(timings,95)) if timings else None,"mode_metrics":{},"baseline_modes":{},
             "seed_scope":"All declared artifact seeds are retained in qualification; the selected seed only selects the saved estimator.",
             "interval_metrics":[v for v in evaluation["metrics"] if v["method_id"]==request.method_id and v["target"]==target],
    ...
    for mode in sorted({v["mode_code"] for v in owner.data.mode_cards.values()}):
        ...
        if mask.any():
            ...
            quality["baseline_modes"][mode]={"n":int(mask.sum()),"mae_mean_over_seeds":float(np.abs(baseline[mask]-truth[mask]).mean())}
    ...
    bindings={name:file_hash(root/name) for name in ("protocol.json","training_manifest.json","predictions.csv",
                                                   "calibration_predictions.csv","evaluation.json")}
    descriptor=safe({"schema_version":"model-artifact-set.g5c.v1","source_kind":"calibration_study","source_id":request.source_id,
          "method_id":request.method_id,"target":target,"unit":TARGET_UNITS[target],"seed":seed,"task_id":TASK_ID,
          "feature_columns":owner.data.feature_columns,"feature_spec_hash":digest(owner.data.feature_columns),
          "source":protocol["source"],"bindings":bindings,"artifacts":artifacts,"benchmark":quality,
          "family":family,"dependencies":dependencies,
          "license":{"project":"No new license assigned; see source project","dependencies":{name:"See installed distribution metadata" for name in dependencies}},
          "calibrator":{"schema_version":CALIBRATION_VERSION,"nominal_coverage":protocol["request"]["coverage"],
                        "modes":["marginal_c90","joint_c90"],"empirical_time_split":True,"unconditional_guarantee":False},
          "calibrator_missing_reason":None,"scopes":["historical_oof"],"proxy_approved":False,"causal_control":False,
          "evaluation_mode":"historical_replay","benchmark_event_hash":evaluation["common_event_hash"]})
...
def predict_calibrated_source(owner,descriptor,event):
    ...
    if entry is None:raise WorkbenchError("该事件没有校准回放工件","NO_OOF_MODEL")
    ...
    group_id=digest({"study_id":descriptor["source_id"],"method_id":descriptor["method_id"],
                     "seed":descriptor["seed"],"fold_id":fold,"calibrator":entry["calibrator_sha256"]})
    evidence={"artifact_sha256":entry["sha256"],"execution":"calibrated_model_inference",
              "model_elapsed_ms":(perf_counter()-start)*1000,"calibrator_sha256":entry["calibrator_sha256"],
              "calibration":{"schema_version":CALIBRATION_VERSION,"nominal_coverage":calibrator.coverage,"group_id":group_id,
                             "fit_cutoff_at":entry["fit_cutoff_at"],"base_fit_cutoff_at":entry["base_fit_cutoff_at"],
                             "calibration_count":calibrator.calibration_count,"unconditional_guarantee":False},
              "prediction_intervals":{mode:{"lower":float(value[0,0,target_index]),"upper":float(value[0,1,target_index]),
                                           "unit":descriptor["unit"],"joint_region":False,"projection_of_joint_region":mode=="joint_c90"} for mode,value in intervals.items()}}
    if "joint_c90" in intervals:
        bounds=intervals["joint_c90"]
        evidence["joint_prediction_region"]={"id":digest({"group_id":group_id,"event_id":event}),"calibration_group_id":group_id,"nominal_coverage":calibrator.coverage,
            "bounds":{target:{"lower":float(bounds[0,0,j]),"upper":float(bounds[0,1,j]),"unit":TARGET_UNITS[target]}
                      for j,target in enumerate(("cu","as"))},"unconditional_guarantee":False}
