...
CLASSES = {
    ...
    "Quantile": "sklearn.linear_model.QuantileRegressor",
...
PARAMETERS = {
    ...
    "ARD": {"max_iter": 500, "tol": 1e-5, "threshold_lambda": 10000.0},
    "RANSAC": {
    ...
    "Quantile": {"quantiles": [0.1, 0.5, 0.9], "alpha": 0.01, "solver": "highs"},
    "MultiTaskElasticNet": {"alpha": 0.01, "l1_ratio": 0.5, "max_iter": 20000, "tol": 1e-6, "selection": "cyclic"},
    "KernelRidge": {"alpha": 1.0, "kernel": "rbf", "gamma": 0.01},
    "SVR": {"C": 10.0, "epsilon": 0.1, "kernel": "rbf", "gamma": "scale"},
    "KNN": {"n_neighbors": 15, "weights": "distance", "p": 2},
    "GaussianProcess": {
        "kernel": "1.0 * RBF(length_scale=1.0), fixed",
        "alpha": 0.05,
        "optimizer": None,
        "normalize_y": False,
    },
...
def classical_spec(method_id, seed):
    uncertainty = (
        "marginal_std"
        if method_id in ("BayesianRidge", "ARD", "GaussianProcess")
        else "raw_quantiles"
        if method_id == "Quantile"
        else "unsupported"
    )
    spec = {
        "schema_version": "model-registry.g6a.v1",
        "method_id": method_id,
        "design_id": DESIGN_IDS[method_id],
        "method_version": "g6a.fixed.v1",
        "implementation": CLASSES[method_id],
        ...
        "multi_output": "native_independent_outputs_shared_kernel"
        if method_id == "GaussianProcess"
        else "native_joint"
        if method_id in JOINT
        ...
        "preprocessing_version": "g6a.numeric64eps.v1",
        "pca_components": REDUCTION.get(method_id),
        "numerical_constant_policy": (
            "Training variance <= (64*float64_eps*max(1,max_abs_training_column))^2 is inactive"
        ),
        "preset_parameters": deepcopy(PARAMETERS[method_id]),
        "seed": seed,
        "stochastic": method_id in STOCHASTIC_METHODS,
        "capabilities": {
            "missing": "adapter",
            "categorical": "adapter",
            "sample_weight": "adapter" if method_id in WEIGHTED else "unsupported",
            ...
            "uncertainty_calibration": "not_calibrated",
            "joint_prediction_region": "unsupported",
            "gaussian_process_std": "observation_std_including_fixed_alpha_noise"
            if method_id == "GaussianProcess"
            else None,
        },
        "minimum_training_events": 16 if method_id == "KNN" else 10,
        "forecast_use": "research_comparison",
        "optimization_proxy_approval": "not_granted_by_registration",
        "causal_control": False,
        "automatic_promotion": False,
        "parameter_policy": "predeclared_fixed_configuration_no_outer_label_tuning",
    }
