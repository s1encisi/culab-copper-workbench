...
class ModelLifecycle:
    ...
    def event(self, c, resource, actor, event, detail):
        c.execute(
            "INSERT INTO model_lifecycle_events(resource_id,actor_id,event,detail,created_at) VALUES(?,?,?,?,?)",
    ...
    def _artifact(c, identifier):
        ...
        if not row:
            raise WorkbenchError("模型工件记录不存在", "ARTIFACT_NOT_FOUND")
        result = dict(row)
        result["descriptor"] = json.loads(result["descriptor"])
        descriptor = result["descriptor"]
        if digest({k: v for k, v in descriptor.items() if k != "source_fingerprint"}) != descriptor.get(
            "source_fingerprint"
        ):
    ...
    def artifact(self, actor, identifier):
        ...
        with self.connection() as c:
            ...
            if result["project_id"] != actor.project_id or (
                result["owner_id"] != actor.user_id and actor.role != "owner"
            ):
                raise WorkbenchError("无权访问该工件", "FORBIDDEN")
            events = [
                dict(r)
                for r in c.execute(
                    "SELECT * FROM model_lifecycle_events WHERE resource_id=? ORDER BY seq", (identifier,)
                )
            ]
        for event in events:
    ...
    def artifacts(self, actor):
        ...
        with self.connection() as c:
            rows = c.execute(
                (
                    "SELECT id FROM model_artifacts WHERE project_id=? AND (owner_id=? "
                    "OR ?='owner') ORDER BY created_at DESC"
                ),
    ...
    def register(self, actor, request):
        ...
        with self.connection() as c:
            old = c.execute(
                "SELECT * FROM model_artifacts WHERE project_id=? AND owner_id=? AND request_key=?",
                (actor.project_id, actor.user_id, request.request_key),
            ).fetchone()
        if old:
            if old["fingerprint"] != fingerprint:
                raise WorkbenchError("请求键已用于不同工件", "REQUEST_CONFLICT")
        ...
        with self.connection() as c:
            ...
            if old:
                ...
                c.execute(
                    "INSERT INTO model_artifacts VALUES(?,?,?,?,?,?,'benchmarked',1,?)",
                    (
                        identifier,
                        actor.project_id,
                        actor.user_id,
    ...
    def start_shadow(self, actor, candidate_id, request, executor=None):
        ...
        with self.connection() as c:
            c.execute("BEGIN IMMEDIATE")
            old = c.execute(
                "SELECT * FROM model_shadows WHERE candidate_id=? AND owner_id=? AND request_key=?",
                (candidate_id, actor.user_id, request.request_key),
            ).fetchone()
            if old:
                if old["fingerprint"] != fingerprint:
                    raise WorkbenchError("影子请求键冲突", "REQUEST_CONFLICT")
                identifier = old["id"]
            else:
                c.execute(
                    "INSERT INTO model_shadows VALUES(?,?,?,?,?,'queued',NULL,NULL,?)",
                    (identifier, candidate_id, actor.user_id, request.request_key, fingerprint, self.clock()),
                )
        if not old:
            if executor:
                executor.submit(self._execute_shadow, identifier, candidate, request, self.auth(actor))
            else:
                self._execute_shadow(identifier, candidate, request, self.auth(actor))
        return self.shadow(actor, identifier)

    def _execute_shadow(self, identifier, candidate, request, actor_auth):
        with self.connection() as c:
            c.execute("UPDATE model_shadows SET status='running' WHERE id=?", (identifier,))
        try:
        ...
        with self.connection() as c:
            ...
            if current["version"] != candidate["version"] or current["status"] in ("retired", "quarantined"):
                status = "stale"
            c.execute(
                "UPDATE model_shadows SET status=?,evidence=?,evidence_hash=? WHERE id=?",
            ...
            self.event(
                ...
                actor_auth["user_id"],
                "shadow_" + status,
                {"shadow_id": identifier, "evidence_hash": digest(evidence)},
            )
    ...
    def shadow(self, actor, identifier):
        ...
        if not row:
            raise WorkbenchError("影子记录不存在", "SHADOW_NOT_FOUND")
        result = dict(row)
    ...
    def shadows(self, actor, candidate_id):
        ...
        with self.connection() as c:
            rows = c.execute(
                "SELECT id FROM model_shadows WHERE candidate_id=? ORDER BY created_at DESC", (candidate_id,)
            ).fetchall()
    ...
    def pointer(self, project, target, purpose="forecast", c=None):
        if c is None:
            with self.connection() as connection:
                return self.pointer(project, target, purpose, connection)
        row = c.execute(
            "SELECT * FROM model_pointers WHERE project_id=? AND target=? AND purpose=?", (project, target, purpose)
        ).fetchone()
        return (
            dict(row)
            if row
    ...
    def quality_gate(descriptor, shadow, purpose):
        ...
        if descriptor.get("causal_control"):
            failures.append("CAUSAL_CONTROL_NOT_SUPPORTED")
        if metric["n"] < 60 or metric["coverage"] < 0.95:
            failures.append("INSUFFICIENT_COMMON_COVERAGE")
        if metric["negative_predictions"]:
            failures.append("NEGATIVE_PREDICTIONS")
        if metric.get("p95_ms") is None or metric["p95_ms"] > 100:
            failures.append("INFERENCE_BUDGET")
        improvement = 1 - metric["mae"] / metric["baseline_mae"] if metric["baseline_mae"] > 0 else 0.0
        if improvement < 0.05:
            failures.append("MINIMUM_IMPROVEMENT")
        interval = metric["paired_ci95"]
        if interval.get("high") is None or interval["high"] >= 0:
            failures.append("PAIRED_INTERVAL")
        for mode, base in metric.get("baseline_modes", {}).items():
            ...
            if current and base["n"] >= 30 and current["mae_mean_over_seeds"] > base["mae_mean_over_seeds"] * 1.05:
                failures.append("WORST_MODE_DEGRADATION")
                break
        ...
        return {
            ...
            "policy": "release.forecast.fixed.v1",
            "scope": "historical research forecast; shadow adds runtime verification only",
        }
    ...
    def proposal(self, actor, identifier):
        ...
        if not row:
            raise WorkbenchError("发布提案不存在", "RELEASE_NOT_FOUND")
        row = dict(row)
        ...
        row["approval"] = json.loads(row["approval"]) if row["approval"] else None
    ...
    def proposals(self, actor):
        ...
        with self.connection() as c:
            rows = c.execute(
                (
                    "SELECT id FROM model_release_proposals WHERE project_id=? AND (own"
                    "er_id=? OR ?='owner') ORDER BY created_at DESC"
                ),
                (actor.project_id, actor.user_id, actor.role),
            ).fetchall()
    ...
    def propose(self, actor, request, *, rollback=False):
        ...
        with self.connection() as c:
            old = c.execute(
                "SELECT * FROM model_release_proposals WHERE project_id=? AND owner_id=? AND request_key=?",
                (actor.project_id, actor.user_id, request.request_key),
            ).fetchone()
        if old:
            if old["request_fingerprint"] != request_fingerprint:
                raise WorkbenchError("发布请求键冲突", "REQUEST_CONFLICT")
        ...
        identifier = uuid.uuid4().hex
        payload = {
            "schema_version": "model-release.g5c.v1",
            "id": identifier,
            "project_id": actor.project_id,
            "task_id": TASK_ID,
            "operation": "rollback" if rollback else "promote",
            "target": request.target,
            "unit": TARGET_UNITS[request.target],
            "purpose": request.purpose,
            "scope": "historical_research",
            "candidate_id": request.candidate_id,
            "candidate_version": candidate["version"] if candidate else 0,
            "source_fingerprint": candidate["descriptor"]["source_fingerprint"]
            if candidate
            else "builtin-persistence-v1",
            "shadow_id": shadow["id"] if shadow else None,
            "shadow_hash": shadow["evidence_hash"] if shadow else None,
            "previous": pointer,
        ...
        with self.connection() as c:
            c.execute("BEGIN IMMEDIATE")
            if (
                self.pointer(actor.project_id, request.target, request.purpose, c)["version"]
                != request.expected_version
            ):
                raise WorkbenchError("模型指针版本已变化", "VERSION_CONFLICT")
            c.execute(
                "INSERT INTO model_release_proposals VALUES(?,?,?,?,?,?,?,?, 'pending',NULL,?)",
                (
                    identifier,
                    actor.project_id,
                    actor.user_id,
    ...
    def approve(self, actor, identifier, decision):
        ...
        with self.connection() as c:
            ...
            if self.clock() >= payload["expires_at"]:
                raise WorkbenchError("发布提案已过期", "RELEASE_EXPIRED")
            ...
            if payload["candidate_id"] != BASELINE_ID:
                ...
                if candidate["version"] != payload["candidate_version"] or candidate["status"] not in (
                    "shadow",
                    "approved",
                ):
                ...
                if payload["operation"] == "promote":
                    ...
                    if (
                        not shadow_row
                        or shadow_row["candidate_id"] != candidate["id"]
                        or shadow_row["evidence_hash"] != payload["shadow_hash"]
                    ):
                        raise WorkbenchError("影子证据绑定已变化", "SOURCE_CHANGED")
                    shadow = dict(shadow_row)
                    shadow["evidence"] = json.loads(shadow["evidence"])
                    if (
                        digest(shadow["evidence"]) != shadow["evidence_hash"]
                        or not self.quality_gate(candidate["descriptor"], shadow, payload["purpose"])["passed"]
                    ):
            ...
            if status == "applied":
                c.execute(
                    "INSERT OR REPLACE INTO model_pointers VALUES(?,?,?,?,?,?)",
                    (
                        actor.project_id,
                        payload["target"],
                        payload["purpose"],
                ...
                if payload["candidate_id"] != BASELINE_ID:
                    c.execute(
                        "UPDATE model_artifacts SET status='approved',version=version+1 WHERE id=?",
                        (payload["candidate_id"],),
                    )
                    self.event(c, payload["candidate_id"], actor.user_id, "approved", {"release_id": identifier})
            c.execute(
                "UPDATE model_release_proposals SET status=?,approval=? WHERE id=?",
                (status, dumps(approval), identifier),
            )
            self.event(
                c,
                identifier,
                actor.user_id,
                "release_" + status,
                {"pointer_version": pointer["version"] + (status == "applied")},
            )
        return self.proposal(actor, identifier)
    ...
    def reject(self, actor, identifier, decision):
        ...
        with self.connection() as c:
            ...
            if row["status"] == "pending":
                ...
                c.execute(
                    "UPDATE model_release_proposals SET status='rejected',approval=? WHERE id=?",
                    (dumps(approval), identifier),
                )
    ...
    def rollback(self, actor, request):
        ...
        if request.destination_release_id:
            ...
            if (
                destination["status"] != "applied"
                or destination["payload"]["target"] != request.target
                or destination["payload"]["purpose"] != "forecast"
            ):
    ...
    def retire(self, actor, identifier, expected_version):
        ...
        with self.connection() as c:
            ...
            if candidate["version"] != expected_version:
                raise WorkbenchError("工件版本已改变", "VERSION_CONFLICT")
    ...
    def pointer_at(self, project, target, cutoff, c):
        releases = c.execute(
            "SELECT * FROM model_release_proposals WHERE project_id=? AND status='applied' ORDER BY created_at",
            (project,),
        ).fetchall()
        selected = {
            "project_id": project,
        ...
        for row in releases:
            ...
            if (
                ...
                and payload["purpose"] == "forecast"
                and source_time(approval["at"]) <= cutoff
            ):
    ...
    def forecast(self, actor, request):
        ...
        with self.connection() as c:
            existing = c.execute(
                "SELECT * FROM release_predictions WHERE project_id=? AND owner_id=? AND request_key=?",
                (actor.project_id, actor.user_id, request.request_key),
            ).fetchone()
            if existing:
                if existing["fingerprint"] != fingerprint:
                    raise WorkbenchError("预测请求键冲突", "REQUEST_CONFLICT")
                return self.read_prediction_record(existing["result"])
            c.execute("BEGIN")
            cutoff = source_time(input_data["decision_at"])
            pointers = {
                t: self.pointer_at(actor.project_id, t, cutoff, c)
                if request.selection == "as_of_event"
                else self.pointer(actor.project_id, t, c=c)
                for t in ("cu", "as")
            }
        predictions = {}
        warnings = []
        for target, pointer in pointers.items():
            ...
            if value is None or value < 0:
                ...
                warnings.append(
                    {"target": target, "code": "CURRENT_RESULT_MISSING_OR_INVALID", "message": "当前化验结果缺失或无效"}
                )
            ...
            if pointer["candidate_id"] != BASELINE_ID:
                try:
                    with self.connection() as c:
                        candidate = self._artifact(c, pointer["candidate_id"])
                    ...
                    if descriptor["target"] != target:
                        raise WorkbenchError("发布目标不匹配", "RELEASE_SCOPE")
                    self.sources.verify(descriptor)
                    ...
                    if exc.code in ("MODEL_HASH_MISMATCH", "SOURCE_CHANGED", "MODEL_VERSION_MISMATCH"):
                        with self.connection() as c:
                            c.execute(
                                (
                                    "UPDATE model_artifacts SET status='quarantined',version=version+1 "
                                    "WHERE id=? AND status!='quarantined'"
                                ),
                                (pointer["candidate_id"],),
                            )
                            self.event(
                                c, pointer["candidate_id"], "runtime_verifier", "quarantined", {"reason": exc.code}
                            )
            predictions[target] = result
        candidate_regions = [
            predictions[target].get("model_evidence", {}).get("joint_prediction_region") for target in ("cu", "as")
        ]
        joint_region = (
            candidate_regions[0]
            if all(candidate_regions) and candidate_regions[0]["id"] == candidate_regions[1]["id"]
            else None
        )
        identifier = uuid.uuid4().hex
        result = {
            "schema_version": "release-prediction.g5c.v1",
            "id": identifier,
            "project_id": actor.project_id,
            "task_id": TASK_ID,
            "trace_id": uuid.uuid4().hex,
            "created_by": actor.user_id,
            "source_kind": "registered-model-research",
            "created_at": self.now(),
            "event_id": request.event_id,
            "snapshot_id": input_data["snapshot_id"],
            "input_decision_at": input_data["decision_at"],
            "computed_at": self.now(),
            "selection": request.selection,
            "evaluation_mode": "historical_replay",
            "online_prediction_claim": False,
            "selection_timing_note": (
                "Current research uses today's approved pointer; as_of_event uses o"
                "nly approvals already effective at the event time."
            ),
            "pointer_snapshot": pointers,
            "predictions": predictions,
            "joint_prediction_region": joint_region,
            "warnings": warnings,
        }
        ...
        with self.connection() as c:
            ...
            if existing:
                ...
                return self.read_prediction_record(existing["result"])
            c.execute(
                "INSERT INTO release_predictions VALUES(?,?,?,?,?,?,?)",
                (
                    identifier,
                    actor.project_id,
                    actor.user_id,
    ...
    def prediction(self, actor, identifier):
        ...
        if not row:
            raise WorkbenchError("预测记录不存在", "PREDICTION_NOT_FOUND")
        if row["project_id"] != actor.project_id or (row["owner_id"] != actor.user_id and actor.role != "owner"):
