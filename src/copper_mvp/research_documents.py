...
class ResearchDocuments:
    ...
    def consents(self, actor, doc_id, version):
        with self.knowledge.lock, self.knowledge.connection() as c:
            ...
            rows = c.execute(
                "SELECT * FROM document_disclosures WHERE doc_id=? AND version=? ORDER BY created_at", (doc_id, version)
            ).fetchall()
    ...
    def revoke(self, actor, doc_id, consent_id):
        ...
        with self.knowledge.lock:
            with self.knowledge.connection() as c:
                self.knowledge._document(c, actor, doc_id, manage=True)
                if not c.execute(
                    "SELECT 1 FROM document_disclosures WHERE id=? AND doc_id=?", (consent_id, doc_id)
                ).fetchone():
    ...
    def _read(self, actor, reference):
        ...
        if (
            ...
            or actual["parse_hash"] != reference["parse_hash"]
    ...
    def _consent(self, actor, reference, approval_id=None):
        ...
        with self.knowledge.connection() as c:
            rows = c.execute(
                (
                    "SELECT * FROM document_disclosures WHERE doc_id=? AND version=? AN"
                    "D document_hash=?\n"
                    "                              AND parse_hash=? AND provider=? AND "
                    "revoked=0 AND expires_at>? ORDER BY created_at DESC"
                ),
                (
                    citation["doc_id"],
                    citation["version"],
        ...
        with self.research.connection() as c:
            return [
                ...
                for row in c.execute(
                    "SELECT * FROM research_document_sources WHERE task_id=? ORDER BY created_at,chunk_id", (task_id,)
                )
            ]
        ...
        with self.knowledge.lock:
            ...
            for item in evidence:
                for field, fact in item["data"].get("local_facts", {}).items():
                    ...
                    with self.research.connection() as c:
                        c.execute(
                            "INSERT OR IGNORE INTO research_document_sources VALUES(?,?,?,?,?,?,?)",
                            (
                                task_id,
                                citation["doc_id"],
                                citation["version"],
    ...
    def invalidate(self, doc_id, *, force=False, approval_id=None):
        with self.knowledge.lock:
            with self.research.connection() as c:
                rows = c.execute(
                    """SELECT DISTINCT t.id,t.owner_id,t.request_json FROM research_tasks t
                    JOIN research_document_sources s ON s.task_id=t.id WHERE s.doc_id=?
                    AND (? IS NULL OR s.approval_id=?)""",
                    (doc_id, approval_id, approval_id),
                ).fetchall()
            for row in rows:
    ...
    def _redact(self, task_id, doc_id):
        ...
        with self.research.connection() as c:
            ...
            c.execute(
                (
                    "UPDATE research_tasks SET result_json=?,checkpoint_json='{}',error"
                    "_json=?,\n"
                    "                         status='needs_attention',version=version+"
                    "1,fence=fence+1,worker_id=NULL,lease_until=NULL WHERE id=?"
                ),
            ...
            c.execute(
                "INSERT INTO research_document_audit(task_id,doc_id,action,at) VALUES(?,?,?,?)",
