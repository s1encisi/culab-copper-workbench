...
def evaluate_calibration(data, root):
    ...
    for split in splits.values():
        ...
        for field, hash_field in (
            ("fit_ids", "fit_event_hash"),
            ("calibration_ids", "calibration_event_hash"),
            ("validation_ids", "validation_event_hash"),
            ("outer_ids", "outer_event_hash"),
        ):
        ...
        if split["fold_id"] != "DEVELOPMENT":
            ...
            if set(split["outer_ids"]) != set(data.training_ids(split["fold_id"])) or validation != set(
                cv.loc[cv.fold_role.eq("VALIDATION"), "origin_event_id"]
            ):
        ...
        wanted = max(
            ...
            math.ceil(len(ordered) * protocol["request"]["calibration_fraction"]),
        )
        ...
        if any(
            not source_time(split["base_fit_cutoff_at"])
            <= source_time(data.row(e).decision_at)
            < source_time(split["calibration_fit_cutoff_at"])
            for e in cal
        ):
            raise WorkbenchError("校准事件越界", "CALIBRATION_SPLIT")
        if any(
            source_time(data.row(e).decision_at) < source_time(split["calibration_fit_cutoff_at"]) for e in validation
        ):
    ...
    for method, seeds in methods.items():
        for seed in seeds:
            ...
            for event, row in part.iterrows():
                ...
                if row.fold_id != data.fold_for[event] or source_time(row.decision_at) != source_time(
                    data.row(event).decision_at
                ):
                    raise WorkbenchError("校准预测时间或归属不一致", "CALIBRATION_PREDICTIONS")
                if row.status == "completed" and source_time(row.calibration_fit_cutoff_at) != source_time(
                    split["calibration_fit_cutoff_at"]
                ):
            ...
            coverage.append(
                {
                    ...
                    "available": len(valid),
                    "missing_or_failed": len(expected) - len(valid),
    ...
    eligible = {
        ...
        for event in common
        if all(
            (event, t) in visible and visible[(event, t)].quality_eligible and visible[(event, t)].value is not None
            for t in TARGETS
        )
    }
    ...
    for artifact in manifest["artifacts"]:
        ...
        part = cal_frame[
            cal_frame.fold_id.eq(artifact["fold_id"])
            & cal_frame.method_id.eq(artifact["method_id"])
            & cal_frame.seed.eq(artifact["seed"])
        ].set_index("event_id")
        ...
        fitted = SplitC90.fit(
            part.loc[split["calibration_ids"], list(TARGETS)].to_numpy(float),
            actual,
            scale,
        ...
        if fitted.joint_radius is None:
            if saved["joint_score_radius"] is not None:
                raise WorkbenchError("联合区域状态不一致", "CALIBRATION_REPLAY")
        else:
        ...
        if artifact["fold_id"] != "DEVELOPMENT":
            expected_bounds = fitted.intervals(
                by_case[(artifact["method_id"], artifact["seed"])]
                .loc[split["validation_ids"], list(TARGETS)]
                .to_numpy(float)
            )
            ...
            for mode, bounds in expected_bounds.items():
                for j, target in enumerate(TARGETS):
                    np.testing.assert_allclose(
                        bounds[:, 0, j], valid[mode + "_" + target + "_lower"], rtol=1e-12, atol=1e-9
                    )
                    np.testing.assert_allclose(
                        bounds[:, 1, j], valid[mode + "_" + target + "_upper"], rtol=1e-12, atol=1e-9
                    )
    ...
    if events:
        ...
        for (method, seed), part in by_case.items():
            ...
            for mode in ("marginal_c90", "joint_c90", "native"):
                ...
                level = (
                    float(part.native_coverage.dropna().iloc[0])
                    if mode == "native" and "native_coverage" in part and part.native_coverage.notna().any()
                    else protocol["request"]["coverage"]
                )
                ...
                joint_metrics.append(
                    {
                        ...
                        "joint_picp_all_events": float(covered.mean()),
                ...
                for j, target in enumerate(TARGETS):
                    if not valid.any():
                        continue
                    point = (
                        part["native_" + target + "_center"].to_numpy(float)
                        if mode == "native"
                        else part[target].to_numpy(float)
                    )
                    values = interval_metrics(actual[valid, j], point[valid], lo[valid, j], hi[valid, j], level)
                    values["normalized_width"] = (
                        float(np.mean((hi[valid, j] - lo[valid, j]) / part[target + "_scale"].to_numpy(float)[valid]))
                        if (part[target + "_scale"].to_numpy(float)[valid] > 0).all()
                        else None
                    )
                    row = {
                        "method_id": method,
                    ...
                    if not valid.all():
                        row["wis"] = None
                        row["interval_score"] = None
                    row.update(numerical_metrics(actual[valid, j], point[valid]))
                    if (
                        mode == "native"
                        and "native_" + target + "_std" in part
                        and part["native_" + target + "_std"].notna().all()
                    ):
                        from copper_mvp.specialized_evaluation import gaussian_scores

                        row.update(
                            gaussian_scores(
                                actual[valid, j], point[valid], part["native_" + target + "_std"].to_numpy(float)[valid]
                            )
                        )
                    metrics.append(row)
                    alpha = 1 - level
                    ...
                    score = (
                        width
                        + (2 / alpha) * (lo[:, j] - actual[:, j]) * (actual[:, j] < lo[:, j])
                        + (2 / alpha) * (actual[:, j] - hi[:, j]) * (actual[:, j] > hi[:, j])
                    )
                    wis = (0.5 * np.abs(actual[:, j] - point) + (alpha / 2) * score) / 1.5
                    if valid.all():
                        wis_arrays[(method, seed, mode, target)] = wis
        ...
        for row in metrics:
            ...
            if key in wis_arrays and baseline in wis_arrays:
                ...
                row["paired_wis_ci95"] = paired_week_interval(
                    delta,
                    decisions,
                    {"minimum_blocks": 20, "replicates": 1000, "seed": protocol["request"]["seeds"][0]},
                )
    result = {
        "status": "completed" if events else "insufficient_common_coverage",
        "created_at": utc_now(),
        "common_events": len(events),
        "common_event_hash": digest(events),
        "coverage": coverage,
        "metrics": metrics,
        "joint_coverage": joint_metrics,
        "by_fold": groups,
        "split_audit": audit,
        "calibrator_replay": artifact_checks,
        "failed_artifacts": [v for v in manifest["artifacts"] if v["status"] != "completed"],
        "resources": [
            {
                k: v
                for k, v in a.items()
                ...
                in (
                    ...
                    "status",
                    "base_fit_ms",
                    "calibration_predict_ms",
                    "calibrator_fit_ms",
                    "validation_predict_ms",
                    "single_p50_ms",
                    "single_p95_ms",
                    "total_ms",
                )
        ...
        "empirical_only": True,
        "unconditional_coverage_guarantee": False,
