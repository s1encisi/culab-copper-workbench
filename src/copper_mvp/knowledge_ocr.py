...
def render_page(payload, page_number, dpi=180):
    ...
    with PDFIUM_LOCK, pdfium.PdfDocument(payload) as document:
        ...
        try:
            ...
            page.close()
    return stream.getvalue(), {"page": page_number, "rendered_size": size,
                               "pdf_size_points": [width, height], "dpi": dpi,
...
class LocalOCR:
    ...
    def _load(self):
        ...
        self.engine = RapidOCR(
            text_score=0.0, intra_op_num_threads=2, inter_op_num_threads=1,
        ...
        import cv2
        self.manifest = {**config, "onnxruntime": onnxruntime.__version__,
    ...
    def recognize(self, payload, page_number, dpi=180):
        with self.lock:
            ...
            for box, text, confidence in rows or []:
                lines.append({"text": text, "score": float(confidence),
                              "polygon": np.asarray(box, dtype=float).tolist()})
            content = "\n".join(row["text"] for row in lines)
            scores = [row["score"] for row in lines]
            return block("ocr_page", content, location, ocr={
                "engine_signature": self.signature, "engine": self.manifest,
                ...
                "lines": lines, "elapsed_ms": (perf_counter() - start) * 1000,
                "rendered_sha256": hashlib.sha256(rendered).hexdigest(),
                "review_required": True, "executable": False})
