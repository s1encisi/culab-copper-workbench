...
class TemporalInputs:
    ...
    def transform(self, data, event_ids):
        ...
        age = np.where(missing, 2.0, ages) / 2.0
        relative = np.broadcast_to((-np.array(OFFSETS) / 12.0)[None, :, None], (len(values), 7, 1))
        sequence = np.concatenate((scaled, missing.astype(float), age, gaps / 4.0, relative), axis=2)
        context = np.column_stack(
            (self.static_scaler.transform(static), np.eye(3)[categories].reshape(len(values), -1))
        )
