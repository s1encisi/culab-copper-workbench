...
def audit_scalarization(record, frame, prepared):
    ...
    if anchors is not None:
        ...
        if detail["status"] == "degenerate_anchor_geometry" and (record["optimizer_id"] not in ("NBI", "NNC") or np.all(span > 1e-10)):
    ...
    extra_evaluations = 0
    for job in jobs:
        if job["ledger_stop"]-job["ledger_start"] != job["actual_evaluations"] or job["actual_evaluations"] > job["quota"]:
            raise ValueError("Scalar job budget mismatch")
        if "returned" not in job:
            continue
        if job["solver_nfev"]+job["return_verification_evaluations"] != job["actual_evaluations"] or job["return_verification_evaluations"] != 1:
        ...
        if job["solver_converged"] and (job["solver_info"] not in (0, 1) or not job["subproblem_feasible"] or not job["solver_return_consistent"]):
            raise ValueError("Invalid scalar convergence claim")
    return {"jobs": len(jobs), "converged": sum(job["solver_converged"] for job in jobs),
            "extra_return_evaluations": extra_evaluations, "status": detail["status"]}
