...
class EnsembleSettings(BaseModel):
    ...
    inner_boundaries: tuple[float, ...] = (0.4, 0.6, 0.8, 1.0)
    base_subsets: tuple[tuple[int, ...], ...] = ((0, 2), (0, 1, 2))
    ridge_alphas: tuple[float, ...] = (0.1, 1.0, 10.0)
    convex_penalties: tuple[float, ...] = (0.0, 0.01, 0.1, 1.0)
...
def simplex_weights(predictions, actual, scale, penalty=0.0, prior=None):
    ...
    m = P.shape[1]
    prior = np.asarray(prior if prior is not None else [1.0, *([0.0] * (m - 1))])
    A = (P - P[:, [0]]) / scale
    b = (y - P[:, 0]) / scale
    ...
    for size in range(1, m + 1):
        for subset in combinations(range(m), size):
            ids = np.array(subset)
            K = np.block([[Q[np.ix_(ids, ids)], np.ones((size, 1))], [np.ones((1, size)), np.zeros((1, 1))]])
            solution = np.linalg.lstsq(K, np.r_[q[ids], 1.0], rcond=None)[0][:size]
            if np.min(solution) < -1e-8 or abs(solution.sum() - 1) > 1e-7:
                continue
            w = np.zeros(m)
            w[ids] = np.maximum(0.0, solution)
            w /= w.sum()
...
def make_meta(alpha):
    return TransformedTargetRegressor(
        regressor=Pipeline([("scale", StandardScaler()), ("ridge", Ridge(alpha=alpha))]), transformer=StandardScaler()
    )
...
class StackedPredictor:
    ...
    def predict(self, X):
        ...
        output = []
        for target, (meta, subset) in enumerate(zip(self.meta_models, self.subsets, strict=True)):
