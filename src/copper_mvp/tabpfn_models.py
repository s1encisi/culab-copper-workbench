...
from copper_mvp.common import PROJECT_ROOT, WorkbenchError, file_hash
...
class TabPFNModel:
    ...
    def fit(self, X, y, sample_weight=None, max_wall_seconds=None):
        ...
        self.preprocessor = Pipeline(
            [
                ("columns", classical_preprocess("TabPFN")),
        ...
        self.context_id = hashlib.sha256(
            self.train_X.tobytes()
            + self.train_y.tobytes()
            + str(self.seed).encode()
            + WEIGHT_SHA256.encode()
            + str(self.settings["n_estimators"]).encode()
        ).hexdigest()
        ...
        self.fit_metadata = {
            "training_rows": len(X),
            "projected_features": self.train_X.shape[1],
            ...
            "cpu_context_limit_explicitly_enabled": len(X) > 1000,
            "fit_seconds": result.get("fit_seconds", 0.0),
        }
        return self
    ...
    def _ensure_context(self, timeout=1800):
        ...
        path = root / "train.npz"
        if not path.exists():
            np.savez(path, X=self.train_X, y=self.train_y)
    ...
    def _predict(self, X):
        ...
        query = root / (identifier + "_input.npz")
        result = root / (identifier + "_output.npz")
    ...
    def __getstate__(self):
        ...
        state.pop("_last_query_key", None)
        state.pop("_last_result", None)
        return state
    ...
    def uncertainty(self, X):
        return {
            ...
            "levels": [0.1, 0.5, 0.9],
            "values": self._predict(X)[1],
            "distribution": "pretrained_TabPFN_conditional",
            "calibrated": False,
            "joint_region": False,
        }
