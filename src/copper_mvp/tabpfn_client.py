...
import atexit,json,os,queue,subprocess,sys,threading,uuid
...
def read_responses(stream,responses):
    try:
        for line in iter(stream.readline,b""):
            ...
            if text.startswith("TABPFN_JSON "):responses.put(json.loads(text[len("TABPFN_JSON "):]))
    finally:responses.put(None)


class LocalWorker:
    def __init__(self):
        root=PROJECT_ROOT/"runs/mvp/tabpfn_worker_logs";root.mkdir(parents=True,exist_ok=True)
        self.stderr=(root/(uuid.uuid4().hex+".log")).open("wb")
        flags={"creationflags":subprocess.CREATE_NO_WINDOW} if os.name=="nt" else {}
        self.process=subprocess.Popen([sys.executable,"-X","utf8","-B",str(PROJECT_ROOT/"scripts/run_tabpfn_worker.py")],
            cwd=PROJECT_ROOT,env=os.environ.copy(),stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=self.stderr,**flags)
