...
class StatisticalRegressor:
    ...
    def fit(self,X,y):
        ...
        if len(X)<30:raise WorkbenchError("统计回归训练样本不足","MODEL_TRAINING_SUPPORT")
        ...
        with warnings.catch_warnings(record=True) as caught:
            ...
            if self.method_id=="LocalLinearKernel":
                ...
                self.models=[KernelReg(target[:,t],A,var_type="cc",reg_type="ll",bw=[.5,.5]) for t in (0,1)]
            else:
                from statsmodels.regression.linear_model import OLS
                self.splines=SplineTransformer(n_knots=5,degree=3,knots="quantile",include_bias=False,extrapolation="linear")
        ...
        self.fit_metadata={"input_columns":[0,1],"preprocessing":"training_only_current_result_pair",
                           "penalty_rows_are_observations":False,"training_events":len(X)}
        return self
...
def event_arrays(data,with_exog):
    ...
    quality=data.frame.loc[ids,"origin_result_quality_eligible"].astype(str).str.lower().isin(("true","1")).to_numpy()
    Y[~quality]=np.nan
...
def forward_fill(values,initial):
    result=np.asarray(values,float).copy();last=np.asarray(initial,float).copy()
    for i,row in enumerate(result):
        good=np.isfinite(row);last[good]=row[good];result[i]=last
    return result
...
class EventSequenceModel:
    ...
    def _prefix_hash(self,ids,times,Y,exog,end):
        return digest({"ids":ids[:end],"times":[t.isoformat() for t in times[:end]],"Y":Y[:end].tolist(),
    ...
    def fit_context(self,data,train_ids,cutoff):
        ...
        self.scale_=np.where(self.scale_>0,self.scale_,1.)
        normalized=(Y[:count]-self.mean_)/self.scale_
        self.models=[];diagnostics=[]
        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter("always")
            if self.method_id=="VAR":
                ...
                diagnostics.append({"aic":float(result.aic),"bic":float(result.bic),"stable":bool(result.is_stable()),"lags":result.k_ar})
                ...
                for target in (0,1):
                    result=SARIMAX(normalized[:,target],exog=regressors,order=(1,0,1),trend="n",
                        enforce_stationarity=True,enforce_invertibility=True).fit(disp=False,maxiter=200)
                    self.models.append(result)
                    diagnostics.append({"target":target,"aic":float(result.aic),"bic":float(result.bic),
                                        "converged":bool(result.mle_retvals.get("converged",False))})
            else:
                from statsmodels.tsa.statespace.exponential_smoothing import ExponentialSmoothing
                for target in (0,1):
                    model=ExponentialSmoothing(normalized[:,target],trend=True,damped_trend=True,seasonal=None,
                        initialization_method="estimated",concentrate_scale=False)
                    initial_values=forward_fill(normalized[:,target:target+1],np.zeros(1))[:,0]
                    initializer=ExponentialSmoothing(initial_values,trend=True,damped_trend=True,seasonal=None,
                        initialization_method="estimated",concentrate_scale=False)
                    ...
                    diagnostics.append({"target":target,"aic":float(result.aic),"bic":float(result.bic),
                                        "converged":bool(result.mle_retvals.get("converged",False))})
        self.fit_warnings=[{"category":w.category.__name__,"message":str(w.message)[:400]} for w in caught]
        self.fit_metadata={"fit_cutoff_at":self.fit_cutoff_at,"training_event_ids_hash":digest(self.training_event_ids),
            "training_prefix_hash":self.training_prefix_hash,"raw_series_records":count,
            "supervised_primary_event_count":len(train_ids),"supervised_primary_event_hash":digest(train_ids),
            "observed_per_target":np.isfinite(Y[:count]).sum(axis=0).tolist(),
            "missing_per_target":np.isnan(Y[:count]).sum(axis=0).tolist(),
            "original_event_index_preserved":True,"step_basis":"recorded_events_not_fixed_hours",
            "parameter_training_scope":"all quality-eligible current assay observations available at the fit cutoff",
            "missing_policy":"causal_forward_fill_with_training_mean_initialization" if self.method_id=="VAR" else "missing_observation_state_filter",
            "diagnostics":diagnostics,"parameter_updates_during_prediction":False,
            "prediction_state_method":"fixed_parameter_forward_filter_predicted_information" if self.method_id!="VAR" else "fixed_VAR_coefficients_causal_lag_update",
            "ETS_initialization":"causal fill for optimizer starting states; likelihood retains missing observations" if self.method_id=="ETS" else None}
        return self
    ...
    def forecast_context(self,data,events):
        ...
        if digest(data.feature_columns)!=self.feature_spec_hash or self._prefix_hash(ids,times,Y,exog,self.train_count)!=self.training_prefix_hash:
        ...
        means={};deviations={}
        if self.method_id=="VAR":
            history=self.var_history.copy();last=self.last_observed.copy()
            wanted=set(query)
            ...
            for index in range(self.last_training_index,max(query)+1):
                if index>self.last_training_index:
                    ...
                    good=np.isfinite(observation);last[good]=observation[good]
            ...
            end=max(query)+1;first=min(query)+1
            ...
            if self.method_id=="SARIMAX":
                ...
                regressors=np.column_stack((np.ones(end),self.exog_scale.transform(self.exog_imputer.transform(lagged))))
                extra["exog"]=np.column_stack((np.ones(1),self.exog_scale.transform(self.exog_imputer.transform(exog[end-1:end]))))
            target_means=[];target_std=[]
            for target,result in enumerate(self.models):
                model=result.model.clone(normalized[:,target],exog=regressors) if regressors is not None else result.model.clone(normalized[:,target])
                ...
                target_std.append(np.sqrt(np.maximum(0.,np.asarray(prediction.var_pred_mean))))
        ...
        output={"mean":np.vstack([means[i] for i in query]),"std":np.vstack([deviations[i] for i in query]),
                "kind":"marginal_standard_deviation","calibrated":False,"joint_region":False,
                "state_updates_only":True,"event_steps_preserved":True}
