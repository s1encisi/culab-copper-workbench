...
from copper_mvp.diagnostic_tools import DiagnosticTools, TOOL_ARGUMENTS, TOOL_DESCRIPTIONS

AGENT_VERSION = "optimization-diagnosis-v1.3"
ENDPOINT = "https://api.deepseek.com/chat/completions"
NextCheck = Literal["refine_original_grid", "inspect_tree_partitions", "review_training_support", "review_objective_definition"]
NEXT_CHECK_LABELS = {
...
SYSTEM_PROMPT = """你是铜电积离线研究助手，诊断一份已经完成的优化结果。你可以自主选择本地工具和探测密度，根据工具结果决定下一步，最后调用finish_diagnosis交付中文报告。
本任务的决策变量是三段/四段电流I3/I4（A）；电压固定。ranges_relative_to_reference描述的是电流的相对范围。准确使用工具中的quantity与unit，不得把电流扫描写成电压扫描。摘要直接说明诊断发现，基础配置由inspect_run证据提供，无需在摘要重复整套配置。
先了解原运行，再选择有区分力的数值检查。工具的purpose用一句中文解释本次检查的目的。至少获取inspect_run和一项数值探测证据；不必机械执行所有工具。根据用户问题和已返回证据选择工具。
结果里evidence_id是可引用的证据编号。每条finding必须引用实际取得的编号。finish_diagnosis必须单独调用，并且只能在读到工具结果后调用。不要在正文直接返回报告。
区分original_saved_run和new_uniform_grid_probe；后者不是原NSGA-II候选拒绝轨迹。不得声称已经证明全局最优或收敛。不得把条件模型响应说成实际工艺的因果效应。单个Pareto点可能是合理的支配结果。
准确使用变量：as_allowance_mg_l是允许的As预测增量epsilon_As。epsilon_As=0意味着候选As预测不高于参考预测，绝不是As浓度为0。
准确区分集合：可行点数多但非支配点只有1个，表示探测前沿只有1点，可行域并未塌缩。优先根据同一点的两个目标支配关系解释单点前沿；预测值种类较少只是模型响应特征，不能单独证明平台是主因。判断始终限定在已探测的原范围内。
分辨率对照只支持已检查的9/17/33网格结果一致，不足以排除所有采样或搜索不足；未检查更宽范围，不能排除范围选择的影响。没有对应对照的判断使用inconclusive。
表述分辨率时写“已检查的三种密度均返回1点”；不要写“不是采样伪影”或“搜索分辨率不是原因”这类已经完全排除的结论。
后续检查围绕原范围内的证据缺口。不要为了制造冲突而更换优化目标，不建议直接扩到30%或50%的范围，不建议对分段常数的DeltaHGB使用普通梯度优化。诊断结论不需要把所有因素都强行归为确定主因。
finish_diagnosis的next_checks从给定动作编号中选择，也可为空；它们只对应原范围网格加密、当前模型分区检查、原训练折覆盖审查及当前目标定义复核。
固定已有模型、历史背景、范围和As约束；没有训练、改约束或设备控制工具。发现证据不足时明确说明缺少的检查，不编造现场操作原因。数值和结论只依据工具返回。报告保持简洁，说明主因判断、依据和仍不确定的部分。"""
...
class AgentSettings(BaseModel):
    ...
    @classmethod
    def load(cls):
        return cls.model_validate(yaml.safe_load((PROJECT_ROOT / "configs/llm/diagnostic_agent.yaml").read_text(encoding="utf-8")))
...
def tool_definitions() -> list[dict]:
    tools = [{"type": "function", "function": {"name": name, "description": TOOL_DESCRIPTIONS[name], "parameters": model.model_json_schema()}} for name, model in TOOL_ARGUMENTS.items()]
    tools.append({"type": "function", "function": {"name": "finish_diagnosis", "description": "在读到证据后单独提交最终报告。每项判断引用工具返回的evidence_id，证据不足标inconclusive。", "parameters": DiagnosisReport.model_json_schema()}})
    return tools
...
def estimate_usage(usage: dict, pricing: Pricing, requested_at: datetime) -> dict:
    ...
    cost = factor * (hit * pricing.cache_hit_per_million_cny + (prompt - hit) * pricing.cache_miss_per_million_cny + completion * pricing.output_per_million_cny) / 1_000_000
    return {"input_tokens": prompt, "output_tokens": completion, "cache_hit_tokens": hit, "estimated_cost_cny": cost, "price_period": "peak" if peak else "off_peak"}
...
class DiagnosticAgentService:
    ...
    def availability(self) -> dict:
        return {"configured": read_deepseek_key() is not None, "version": AGENT_VERSION, **self.settings.model_dump(exclude={"pricing"}), "questions": QUESTIONS}
    ...
    def execute(self, run_id: str):
        ...
        record = {"kind": "agent_diagnosis", "source_run_id": job["request"]["source_run_id"], "agent_version": AGENT_VERSION, "model": settings.model, "configuration": settings.model_dump(exclude={"pricing"}), "question": job["request"]["question"], "report": None, "evidence": [], "steps": [], "calls": [], "usage": {"api_calls": 0, "tool_calls": 0, "tool_cache_hits": 0, "input_tokens": 0, "output_tokens": 0, "cache_hit_tokens": 0, "estimated_cost_cny": 0.0, "unsettled_reservation_cny": 0.0, "retries": 0}, "outbound_policy": "DERIVED_SUMMARIES_ONLY", "pricing": settings.pricing.model_dump(), "concurrency": 1}
        try:
            ...
            messages = [{"role": "system", "content": SYSTEM_PROMPT}, {"role": "user", "content": QUESTIONS[record["question"]]}]
            cache = {}
            for round_index in range(settings.max_calls):
                ...
                payload = {"model": settings.model, "messages": messages, "tools": tool_definitions(), "tool_choice": "auto", "max_tokens": settings.max_output_tokens, "thinking": {"type": settings.thinking}, "stream": False}
                if settings.thinking == "enabled":
                    payload["reasoning_effort"] = settings.reasoning_effort
                else:
                    payload["temperature"] = 0
                input_upper = len(dumps(payload).encode("utf-8"))
                reservation = (input_upper * settings.pricing.cache_miss_per_million_cny + settings.max_output_tokens * settings.pricing.output_per_million_cny) / 1_000_000
                ...
                self.store.trace(run_id, f"DeepSeek · 第{round_index + 1}轮", "running", utc_now(), 0, {"purpose": "根据问题和已取得证据选择下一项工具"})
                try:
                    with httpx.Client(transport=self.transport, timeout=min(settings.request_timeout_seconds, remaining), follow_redirects=False) as client:
                        response = client.post(ENDPOINT, headers={"Authorization": "Bearer " + key.get_secret_value()}, json=payload)
                        response.raise_for_status()
                        reply = response.json()
                except httpx.HTTPStatusError as exc:
                    raise WorkbenchError(f"DeepSeek返回HTTP {exc.response.status_code}，本次未自动重试", "LLM_HTTP_ERROR") from None
                except (httpx.RequestError, ValueError):
                    raise WorkbenchError("DeepSeek请求失败或返回格式错误，本次未自动重试", "LLM_REQUEST_ERROR") from None
                ...
                record["calls"].append({"round": round_index + 1, "response_model": reply.get("model"), "duration_ms": (perf_counter() - call_start) * 1000, **measure})
                self.store.trace(run_id, f"DeepSeek · 第{round_index + 1}轮", "completed", requested_at.isoformat(), record["calls"][-1]["duration_ms"], measure)
                ...
                if not calls:
                    messages.append({"role": "user", "content": "请通过工具取得证据，并调用finish_diagnosis提交报告。不要只返回文字。"})
                for call in calls:
                    ...
                    try:
                        ...
                        if name == "finish_diagnosis":
                            if len(calls) != 1:
                                raise WorkbenchError("请读到工具结果后，单独调用finish_diagnosis", "REPORT_REQUIRES_FEEDBACK")
                            ...
                            record["report"] = {**report.model_dump(), "next_check_codes": report.next_checks, "next_checks": [NEXT_CHECK_LABELS[key] for key in report.next_checks]}
                            self.store.trace(run_id, "完成有证据的诊断", "completed", utc_now(), 0, {"evidence_count": len(evidence_ids)})
                        ...
                        cache_key = digest({"name": name, "arguments": {k: v for k, v in arguments.items() if k != "purpose"}})
                        if cache_key in cache:
                            ...
                            cached = False
                        step = {"tool": name, "purpose": purpose, "evidence_id": result["evidence_id"], "cached": cached, "status": "completed", "duration_ms": (perf_counter() - clock) * 1000}
                        ...
                        message_text = str(exc) if isinstance(exc, WorkbenchError) else "工具参数不符合结构，请根据工具定义修正"
                        result = {"error": {"code": code, "message": message_text}}
                        if isinstance(exc, ValidationError):
                            result["error"]["fields"] = [{"field": ".".join(map(str, e["loc"])), "message": e["msg"]} for e in exc.errors(include_input=False, include_context=False, include_url=False)]
                        step = {"tool": name, "purpose": purpose, "status": "failed", "error": result["error"], "duration_ms": (perf_counter() - clock) * 1000}
            ...
            message = str(exc) if isinstance(exc, (WorkbenchError, ExternalLLMBudgetExceeded)) else "诊断执行失败，已保留证据和用量记录"
