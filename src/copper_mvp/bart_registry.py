...
BART_SOURCE_FILES = tuple("src/copper_mvp/"+name for name in
    ("bart_registry.py", "bart_models.py", "bart_runtime.py", "bart_sampling.py", "bart_trees.py")) + (
...
BART_PACKAGES = {"pymc-bart": "0.7.0", "pymc": "5.16.2", "pytensor": "2.25.5",
                 "arviz": "0.20.0", "numba": "0.61.2", "llvmlite": "0.44.0"}


def bart_spec(seed):
    spec = {
        "schema_version": "bart-registry.g6h.v1", "method_id": "BART",
        "method_version": BART_VERSION, "implementation": "PyMC-BART PGBART posterior tree ensembles",
        "package_version": "0.7.0", "training_dependencies": dict(BART_PACKAGES),
        "dependencies": {"numba": "0.61.2", "llvmlite": "0.44.0"},
        "requires_fit": True, "status": "registered", "feature_count": 114,
        "multi_output": "two_independent_posterior_models", "target_transform": "delta_from_current",
        "target_scaling": "training_only_standard_delta", "seed": seed,
        "preprocessing": "training_only_impute_stable_scale_fixed_one_hot_PCA8",
        "preset_parameters": dict(BART_PARAMETERS), "sample_weight": "unsupported",
        "capabilities": {"sample_weight": False, "uncertainty": "raw_quantiles"},
        "uncertainty": "posterior_normal_mixture_quantiles", "internal_validation": "disabled",
        ...
        "causal_control": False, "optimization_proxy_approval": "requires_independent_qualification",
    }
