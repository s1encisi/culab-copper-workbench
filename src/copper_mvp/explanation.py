...
def template(run: dict, question: str) -> dict:
    ...
    if kind == "prediction":
        ...
        if question == "mode":
            ...
            parts = [
                "工况记录：" + "，".join(f"{n} {s}" for n, s in zip(names, result["mode"], strict=True)) + "。",
                "状态来自最近可用电流，电压、流量或槽数用于补充证据。UNKNOWN 表示缺少可用电流。",
            ]
        elif question == "model":
            parts.append(
                "Persistence 延续当前浓度；变化量模型利用当前和历史过程字段。模型实"
                "验页保留所有五折结果，可比较 MAE、RMSE 与逐折表现。"
            )
        else:
        ...
        if result.get("warnings"):
            ...
            for warning in result["warnings"]:
                ...
                for code, label in (
                    ...
                    message = (
                        message.replace(code + "_CURRENT_MISSING", label + "缺少可用电流")
                        .replace(code + "_SIGNAL_CONFLICT", label + "的电流与辅助信号存在冲突")
                        .replace(code + "_COMPANION_MISSING", label + "缺少辅助工况证据")
                    )
                message = {
                    "CURRENT_RESULT_QUALITY_WARNING": "当前化验带有来源质量提示",
                    "MODEL_NOT_READY": "本次使用 Persistence 参照",
                    "NO_OOF_MODEL": "该事件未配置折外模型，本次使用 Persistence",
                }.get(message, message)
        ...
        if not result["candidates"]:
            parts = [
                "本次预算内没有找到满足配置的候选，参考方案和约束信息已保留。可检查局部范围与 As 容许变化后重新实验。"
            ]
        else:
            ...
            if result["mode"] == "plant":
                parts = [
                    ...
                    (
                        "相对同代理参考点，Cu 降低 "
                        f"{candidate['delta_cu']:.3f}"
                        " g/L，功率代理降低 "
                        f"{candidate['delta_power']:.2f}"
                        " kW；As 约束裕度为 "
                        f"{candidate['as_margin']:.3f}"
                        " mg/L。"
                    ),
                    "候选使用固定电压与历史上下文，呈现模型空间中的质量—功率权衡。",
                ]
                if question == "constraints":
                    parts.append(
                        "As 比较基准是同一个响应模型对参考电流的预测；搜索范围由训练期分位数与局部变化比例取交集。"
                    )
            else:
                parts = [
                    (
                    ...
                    "函数为两组平方距离，约束 u₁+u₂≤3；结果为无量纲求解验证。",
                ]
            parts.append("Cu/目标一优先、功率/目标二优先与平衡方案使用不同偏好，从同一已审计候选集中选取。")
    elif kind == "training":
        parts = [
            f"完成 {result['train_count']} 条主样本的模型实验，OOF 覆盖 {result['oof_events']} 个事件。",
            (
                "每个目标比较 Persistence、DeltaRidge 与 DeltaHGB，保存五折模型和全"
                "开发期模型。历史回放默认匹配对应时间折。"
            ),
        ]
    else:
        ...
        with self.lock:
            ...
            if use_llm and self.enabled:
                try:
                    ...
                    if not all(
                        math.isfinite(v) and v > 0 for v in (monthly, cap, input_price, output_price)
                    ) or not os.environ.get("DEEPSEEK_MODEL"):
                    ...
                    runtime = resolve_provider_runtime(registry, "deepseek_executor", environ=env).model_copy(
                        update={
                            "input_cny_per_million_tokens": input_price,
                            "output_cny_per_million_tokens": output_price,
                        }
                    )
                    values = run["result"]
                    reasons = [
                        "RUN_COMPLETED",
                        "HAS_WARNINGS" if values.get("warnings") else "NO_WARNINGS",
                        "QUESTION_" + question.upper(),
                    ]
                    task = build_deepseek_explanation_task(
                        artifact_ids=(run_id,),
                        ...
                        explanation_scope="A5_AUDIT_SUMMARY",
                    )
                    month = datetime.now(timezone(timedelta(hours=8))).strftime("%Y-%m")
                    self.store.reserve_cost(key, month, cap, monthly)
                    called = call_structured(
                        runtime=runtime,
                        task=task,
                        output_model=ExplanationCardV1,
                        budget=CallBudget(max_calls=1, max_cost_cny=cap),
                        timeout_seconds=20,
                        transport=transport,
                    )
                    ...
                    result.update(
                        {
                            "mode": "llm",
                            "text": result["text"] + "\n\n语言说明：" + summary,
                            "input_tokens": called.input_tokens,
                            "output_tokens": called.output_tokens,
                            "estimated_cost_cny": called.estimated_cost_cny,
                            "provider_model": called.model,
                        }
                    )
