...
from copper_mvp.tabular_registry import ARCHITECTURES, training_parameters
...
class TabularModel:
    ...
    def fit(self, X, y, sample_weight=None, max_wall_seconds=None):
            ...
            for epoch in range(self.training["epochs"]):
                ...
                for indices in batches(torch.randperm(len(X), generator=generator), self.training["batch_size"]):
                    ...
                    if not torch.isfinite(objective):
                        raise WorkbenchError("神经模型损失非有限", "NEURAL_LOSS")
                    objective.backward()
        ...
        self.fit_metadata = {
            ...
            "parameter_budget_relative_difference": abs(sum(p.numel() for p in network.parameters()) - parameter_budget)
            / parameter_budget,
        }
        return self
    ...
    def predict(self, X):
        ...
        with torch.inference_mode():
            for start in range(0, len(X), 128):
                values, _ = network(
                    torch.from_numpy(numerical[start : start + 128]), torch.from_numpy(categories[start : start + 128])
                )
