...
class SplitC90:
    ...
    def fit(cls,predictions,actual,scale,coverage=0.9,minimum=60):
        point=np.asarray(predictions,float);y=np.asarray(actual,float);scale=np.asarray(scale,float)
    ...
    def manifest(self):
        return {"schema_version":CALIBRATION_VERSION,"nominal_coverage":self.coverage,
                "calibration_count":self.calibration_count,"marginal_radius":self.marginal_radius.tolist(),
                "training_delta_std":self.scale.tolist(),"joint_score_radius":self.joint_radius,
                ...
                "marginal_is_joint":False,"interval_support":"unclipped_real_line",
    ...
    def from_manifest(cls,value):
        ...
        return cls(value["nominal_coverage"],value["calibration_count"],np.asarray(value["marginal_radius"]),
                   np.asarray(value["training_delta_std"]),value["joint_score_radius"])


def interval_metrics(actual,point,lower,upper,coverage=0.9,scale=None):
    y,p,lo,hi=map(lambda value:np.asarray(value,float),(actual,point,lower,upper))
    ...
    result={"n":len(y),"nominal_coverage":coverage,
            "picp":float(np.mean((y>=lo)&(y<=hi))),"mean_width":float(width.mean()),
            "interval_score":float(score.mean()),"wis":float(wis.mean()),
