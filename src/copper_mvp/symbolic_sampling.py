...
from copper_mvp.symbolic_expression import (
    encode_expression,
    evaluate_expression,
    expression_features,
...
def fit_symbolic_models(X, y, settings, seed, directory, runtime_lock):
    ...
    for target, name in enumerate(("cu", "as")):
        write_json(
            ...
            {"status": "searching", "target": name, "elapsed_seconds": time.perf_counter() - started},
        )
        ...
        with warnings.catch_warnings(record=True) as captured:
            ...
            model = PySRRegressor(
                **parameters,
                random_state=(seed + target) % 2**31,
                extra_sympy_mappings={"safe_div": lambda x, y: x / (sp.Abs(y) + 1)},
                output_directory=str(directory / "search"),
                run_id=name,
            )
        ...
        item = {
            ...
            "julia_equation": str(best["equation"]),
            "ast": encoded,
            "complexity": int(best["complexity"]),
            "training_mse": loss,
            "native_training_loss": float(best["loss"]),
