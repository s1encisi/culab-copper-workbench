...
def scalarization_spec(name):
    designs = {"Epsilon-Constraint": "O17", "Augmented-Chebyshev": "O18", "Weighted-Sum": "O19", "NBI": "O20", "NNC": "O21"}
    equations = {
    ...
    return {"optimizer_id": name, "design_id": designs[name], "method_version": "g6e.scalar.v3",
            "mechanism": equations[name], "package": "scipy", "package_version": "1.17.1",
            "inner_solver": "PRIMA COBYLA combined calcfc", "variables": "continuous", "objectives": 2,
            "inequality_constraints": True, "constraint_handling": "explicit_physical_and_preference_constraints",
            "parameters": {"anchor_fraction": 0.2, "anchor_targets": 2, "starts_per_subproblem": 2,
                "interior_preferences_max": 9, "preference_grid": "equally_spaced_inside_(0,1)",
                "normalization": "frozen_best_feasible_anchor_ideal_and_payoff_spans", "rhobeg": 0.2, "rhoend": 1e-6,
                "physical_constraint_tolerance": TOLERANCE, "nbi_equality_tolerance": GEOMETRY_TOLERANCE,
                "anchor_span_tolerance": SPAN_TOLERANCE, "decision_scaling": "unit_box",
            ...
            "smooth_proxy_preferred": name in ("NBI", "NNC"), "status": "registered", "execution_authorized": False}
...
def scalar_value_constraints(name, y, parameter, auxiliary=0.0):
    ...
    if name == "NNC":
        return float(y[1]), np.array([np.dot([1., -1.], y-base)])
    if name == "NBI":
...
class ScalarizationAdapter:
    ...
    def _finish_anchors(self):
        ...
        self.anchor_record = {"variables": X[indices].tolist(), "raw_objectives": (anchors*self.metric_scale).tolist(),
            "scaled_ideal": self.ideal.tolist(), "payoff_spans": span.tolist(), "normalization": self.normalization.tolist(),
        ...
        self.preference_jobs = [(parameter, restart, remaining//len(jobs)+int(i < remaining%len(jobs)))
    ...
    def _solve(self, phase, parameter, restart, quota):
        ...
        extra_count = 4 if phase == "preference" and self.name == "NBI" else int(phase == "preference" and self.name in ("NNC", "Epsilon-Constraint"))
        ...
        def calcfc(coordinates):
            ...
            last_record = {"variables": physical.tolist(), "objectives": F[0].tolist(),
                "constraints": G[0].tolist(), "preference_constraints": extra.tolist(), "scalar_objective": value,
                "solver_coordinates": coordinates.tolist()}
            return value, constraints
        summary = {"stage": phase, "parameter": parameter, "restart": restart, "quota": quota,
                   "ledger_start": before, "start_coordinates": x0.tolist()}
        try:
            result = cobyla(calcfc, self.n_constraints+extra_count, x0, xl=lower, xu=upper,
                            rhobeg=0.2, rhoend=1e-6, ctol=TOLERANCE, maxfun=quota-1, maxhist=quota, iprint=0)
            ...
            summary.update(solver_info=int(result.info), solver_reason=get_info_string("COBYLA", result.info),
                solver_nfev=int(result.nf), return_verification_evaluations=1,
                subproblem_feasible=bool(maxcv <= TOLERANCE), solver_reported_maxcv=float(result.cstrv),
                solver_return_value_difference=difference, solver_return_consistent=consistent,
                solver_converged=bool(maxcv <= TOLERANCE and consistent and result.info in (0, 1)), maxcv=maxcv, returned=last_record)
        ...
        summary.update(actual_evaluations=self.evaluator.used-before, ledger_stop=self.evaluator.used, clipped_probes=clipped)
    ...
    def details(self):
        return safe({"status": self.stop_reason, "anchors": self.anchor_record, "jobs": self.trace,
            "anchor_evaluations": sum(r["actual_evaluations"] for r in self.trace if r["stage"] == "anchor"),
            "preference_evaluations": sum(r["actual_evaluations"] for r in self.trace if r["stage"] == "preference"),
