...
TEMPORAL_FILES = tuple(
    "src/copper_mvp/" + name
    for name in (
        "temporal_registry.py",
        "temporal_inputs.py",
...
def temporal_spec(method, seed):
    record = {
        "schema_version": "temporal-registry.g6k.v1",
        "method_id": method,
        "method_version": TEMPORAL_VERSION,
        "implementation": {
            "GRU": "torch GRU on causal process anchors",
            "LSTM": "torch LSTM on causal process anchors",
            "CausalTCN": "left-padded dilated residual temporal convolutions",
        }[method],
        ...
        "status": "registered",
        "feature_count": 114,
        "input_kind": "anchored_process_sequence",
        "multi_output": "joint_two_target_head",
        "seed": seed,
        "target_transform": "delta_from_current",
        "target_scaling": "training_only_standard_delta",
        "preprocessing": (
            "train-only scaling of 27 signals and current results; missing, age and observation-gap channels"
        ),
        "sequence": {
            "steps": 7,
            "order": "oldest_to_current",
            "offsets_hours": [12, 10, 8, 6, 4, 2, 0],
            "observation_tolerance_hours": 2,
            "hidden_state": "reset_for_each_window",
            "labels_in_sequence": False,
        },
        "preset_parameters": {
            "training": dict(TEMPORAL_TRAINING),
            "architecture": dict(TEMPORAL_ARCHITECTURES[method]),
        },
