...
class PreparedProblem:
    ...
    def specification(self):
        ...
        return safe(
            {
                "schema_version": "problem-spec.g2b.v1",
                ...
                "variables": [
                    {"name": f"stage{s}_current_a", "unit": "A", "lower": float(lo), "upper": float(hi)}
                    for s, lo, hi in zip(self.variable_stages, self.lower, self.upper, strict=True)
                ]
                if self.context
                else [
                ...
                "objectives": [
                    {"name": name, "unit": unit, "direction": "minimize"}
                    for name, unit in zip(self.labels, self.units, strict=True)
                ],
                "constraints": {
                    "kind": "as_allowance_and_nonnegative_predictions" if self.context else "u1+u2<=3",
                    "epsilon_as_mg_l": self.request.epsilon_as if self.context else None,
                    "scales": self.scale_y.tolist() if self.context else None,
                    "normalized_tolerance": 1e-8,
                },
                "fixed_context": {
                    "stages": self.stages,
                    "reference_currents": self.base_currents.tolist(),
                    "fixed_voltages": self.voltages.tolist(),
                }
                if self.context
                else {},
                "reference": {
                    "x": self.reference_x[0].tolist(),
                    "F": self.ref_F[0].tolist(),
                    "G": self.ref_G[0].tolist(),
...
def build_problem(data, models, request: RunRequest):
    ...
    if request.mode == "benchmark":
        ...
        def evaluate(X):
            return (
                np.column_stack(((X * X).sum(axis=1), ((X - 2) ** 2).sum(axis=1))),
                (X.sum(axis=1) - 3)[:, None],
                {"variables": X},
            )
        ...
        for j, stage in enumerate(stages):
            ...
            ranges.append(
                {
                    "stage": stage,
                    "current": base_currents[j],
                    "lower": lo if varies else base_currents[j],
                    "upper": hi if varies else base_currents[j],
                    "historical_p5": p5,
                    "historical_p95": p95,
                    "historical_n": len(values),
                    "varies": varies,
                    "source": "same_mode" if len(same) >= 30 else "training_period",
                }
            )
            if varies:
                variable_stages.append(stage)
                active_positions.append(j)
                lower_list.append(lo)
                upper_list.append(hi)
        lower = np.asarray(lower_list)
        upper = np.asarray(upper_list)
        ...
        historical_currents = data.frame.loc[history_ids, [f"stage{s}_current_a__t_minus_0h" for s in stages]].to_numpy(
            float
        )
        historical_currents = historical_currents[
            np.isfinite(historical_currents).all(axis=1) & (historical_currents > 0).all(axis=1)
        ]
        ...
        def evaluate(X):
            ...
            G = np.column_stack(
                ((y[:, 1] - ref_y[1] - epsilon_as) / scale_y[1], -y[:, 0] / scale_y[0], -y[:, 1] / scale_y[1])
            )
