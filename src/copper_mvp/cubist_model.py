...
class CubistModel:
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        self.fit_metadata = {"training_rows":len(X), "preprocessed_columns":inputs.shape[1],
                             "target_delta_mean":self.target_scaler.mean_.tolist(), "target_delta_scale":self.target_scaler.scale_.tolist(),
                             "rule_linear_models":True, "internal_cv":False, "sample_weight":"unsupported",
                             "undefined_committee_reduction": [not np.isfinite(m.committee_error_reduction_) if m.committee_error_reduction_ is not None else True for m in self.models]}
        return self

    def predict(self, X):
        X = np.asarray(X, float)
        inputs = np.ascontiguousarray(self.preprocessor.transform(X))
        prediction = np.column_stack([model.predict(pd.DataFrame(inputs, columns=model.feature_names_in_)) for model in self.models])
        return X[:,:2]+self.target_scaler.inverse_transform(prediction)

    def rules(self):
        return {"feature_names":self.preprocessor.get_feature_names_out().tolist(),
                "target_delta_mean":self.target_scaler.mean_.tolist(),"target_delta_scale":self.target_scaler.scale_.tolist(),
                "models":[model.model_ for model in self.models], "scope":"rules in transformed feature space; model associations"}
