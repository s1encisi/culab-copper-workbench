...
class BartModel:
    ...
    def fit(self, X, y, sample_weight=None, max_wall_seconds=None):
        ...
        write_json(directory/"request.json", {"settings": self.settings, "seed": self.seed,
                   "input_sha256": file_hash(directory/"input.npz"), "code_hashes": bart_source_hashes()})
        ...
        if max_wall_seconds is not None:
            timeout = min(timeout, max(1., max_wall_seconds))
        kwargs = {"creationflags": subprocess.CREATE_NO_WINDOW} if os.name == "nt" else {"start_new_session": True}
        with (directory/"stdout.log").open("wb") as out, (directory/"stderr.log").open("wb") as err:
            process = subprocess.Popen([sys.executable, "-X", "utf8", "-B",
                str(PROJECT_ROOT/"scripts/run_bart_fit.py"), "--directory", str(directory)],
                cwd=PROJECT_ROOT, env=environment, stdout=out, stderr=err, **kwargs)
            try:
        ...
        self.fit_metadata = {"training_rows": len(X), "projected_features": inputs.shape[1],
            ...
            "chains_per_target": self.settings["chains"], "draws_per_chain": self.settings["draws"],
    ...
    def predict(self, X):
        ...
        for target in (0, 1):
            for start in range(0, len(X), 128):
                result[start:start+128, target] = self._posterior_means(inputs[start:start+128], target).mean(axis=0)
    ...
    def uncertainty(self, X):
        ...
        for target in (0, 1):
            ...
            for start in range(0, len(X), 32):
                ...
                result[start:start+32, :, target] = (X[start:start+32, target, None]
                    +self.target_scaler.mean_[target]+self.target_scaler.scale_[target]*quantiles)
        return {"kind": "raw_marginal_quantiles", "levels": [.1, .5, .9], "values": result,
                "distribution": "posterior_normal_mixture", "calibrated": False, "joint_region": False}
