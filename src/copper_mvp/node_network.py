...
class ObliviousLayer(torch.nn.Module):
    ...
    def initialize(self,X):
        with torch.no_grad():
            ...
            low=positions.floor().long();high=positions.ceil().long()
            column=torch.arange(flat.shape[1])
            fraction=positions-low
            thresholds=(flat[low,column]*(1-fraction)+flat[high,column]*fraction).view(self.trees,self.depth)
...
class NodeNetwork(torch.nn.Module):
    ...
    def forward(self,X):
        outputs=[]
        for layer in self.layers:
            value=layer(X);outputs.append(value)
