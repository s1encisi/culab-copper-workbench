...
from copper_mvp.data import DataRepository, TARGETS, finite
...
def make_model(family: str):
    if family == "DeltaRidge":
        preprocessing = ColumnTransformer([
            ("numeric", Pipeline([("impute", SimpleImputer(strategy="median", keep_empty_features=True)), ("scale", StandardScaler())]), list(range(110))),
            ("modes", OneHotEncoder(categories=[[0.0, 1.0, 2.0]] * 4, handle_unknown="ignore", sparse_output=False), list(range(110, 114))),
        ])
        return Pipeline([("preprocess", preprocessing), ("regressor", Ridge(alpha=1000.0))])
    if family == "DeltaHGB":
        return HistGradientBoostingRegressor(loss="squared_error", learning_rate=0.05, max_iter=200, max_leaf_nodes=15, min_samples_leaf=20, l2_regularization=1.0, early_stopping=False, categorical_features=[False] * 110 + [True] * 4, random_state=20260905)
...
def metrics(y: np.ndarray, predicted: np.ndarray) -> dict:
    ...
    return {"n": len(y), "mae": float(mean_absolute_error(y, predicted)), "rmse": float(np.sqrt(mean_squared_error(y, predicted))), "r2": float(r2_score(y, predicted)) if len(y) > 1 else None, "bias": float(np.mean(predicted - y))}
...
class ModelManager:
    ...
    def manifest(self, bundle_id: str | None = None) -> dict:
        ...
        bundle = next((b for b in bundles if b["bundle_id"] == bundle_id), None) if bundle_id else (bundles[0] if bundles else None)
        if bundle is None:
    ...
    def train(self, bundle_id: str, progress: Callable[[str, dict, float], None]) -> dict:
        ...
        outputs = []; fold_metrics = []; artifacts = {}
        for fold_id in sorted(self.data.cv.fold_id.unique()):
            ...
            for target_index, target in enumerate(TARGETS):
                ...
                fold_metrics.append({"fold_id": fold_id, "model": "Persistence", "target": target, **metrics(valid_y, current)})
                for event, actual, predicted in zip(valid_ids, valid_y, current):
                    outputs.append({"event_id": event, "fold_id": fold_id, "target": target, "model": "Persistence", "actual": actual, "predicted": predicted})
                ...
                for family in FAMILIES:
                    ...
                    artifacts[f"{fold_id}:{family}:{target}"] = {"file": filename, "sha256": file_hash(folder / filename), "fit_cutoff_at": str(rows.fold_fit_cutoff_at.iloc[0]), "train_count": len(train_ids)}
                    fold_metrics.append({"fold_id": fold_id, "model": family, "target": target, **measure})
                    for event, actual, predicted in zip(valid_ids, valid_y, prediction):
                        outputs.append({"event_id": event, "fold_id": fold_id, "target": target, "model": family, "actual": actual, "predicted": predicted})
        ...
        for (family, target), group in oof.groupby(["model", "target"]):
            metric_rows.append({"model": family, "target": target, **metrics(group.actual.to_numpy(), group.predicted.to_numpy())})
        for row in metric_rows:
            baseline = next(r["mae"] for r in metric_rows if r["model"] == "Persistence" and r["target"] == row["target"])
            row["skill_mae"] = 1 - row["mae"] / baseline if baseline else 0
        defaults = {t: min((r for r in metric_rows if r["target"] == t and r["model"] in FAMILIES), key=lambda r: (round(r["mae"], 6), 0 if r["model"] == "DeltaRidge" else 1))["model"] for t in TARGETS}
        ...
        for index, target in enumerate(TARGETS):
            for family in FAMILIES:
                ...
                joblib.dump(model, folder / filename, compress=3)
                artifacts[f"DEVELOPMENT:{family}:{target}"] = {"file": filename, "sha256": file_hash(folder / filename), "fit_cutoff_at": self.data.frame.decision_at.max().isoformat(), "train_count": len(X)}
                progress(f"A3 · 全开发期 · {target.upper()} · {family}", {"train_count": len(X)}, (perf_counter() - start) * 1000)
        oof["decision_at"] = oof.event_id.map(self.data.frame.decision_at).astype(str)
        oof.to_csv(folder / "oof_predictions.csv", index=False)
        manifest = safe({"bundle_id": bundle_id, "status": "completed", "created_at": utc_now(), "dataset_version": self.data.dataset_version, "model_version": MODEL_VERSION, "feature_columns": self.data.feature_columns, "numeric_features": 110, "categorical_features": 4, "train_count": len(X), "oof_events": oof.event_id.nunique(), "default_response": defaults, "metrics": metric_rows, "fold_metrics": fold_metrics, "artifacts": artifacts, "oof_predictions_sha256": file_hash(folder / "oof_predictions.csv"), "model_configs": {"DeltaRidge": {"alpha": 1000}, "DeltaHGB": {"learning_rate": 0.05, "max_iter": 200, "max_leaf_nodes": 15, "min_samples_leaf": 20, "l2_regularization": 1, "early_stopping": False}}, "scopes": ["oof_replay", "development_analysis"]})
    ...
    def predict(self, event_id: str, profile: str = "auto", scope: str = "oof_replay", bundle_id: str | None = None) -> dict:
        ...
        for i, target in enumerate(TARGETS):
            ...
            if current is None:
                predictions[target] = {"current": None, "value": None, "delta": None, "model": None, "unit": "g/L" if target == "cu" else "mg/L"}
            ...
            predictions[target] = {"current": current, "value": value, "delta": delta, "model": resolved["names"][target] if resolved else "Persistence", "unit": "g/L" if target == "cu" else "mg/L"}
        return safe({"kind": "prediction", "event_id": event_id, "decision_at": context["decision_at"], "predictions": predictions, "mode": context["mode"], "mode_evidence": context["mode_evidence"], "warnings": sorted(set(warnings)), "model_scope": scope if resolved else "reference", "fold_id": resolved["fold_id"] if resolved else context["fold_id"], "fit_cutoff_at": resolved["fit_cutoff_at"] if resolved else None, "bundle_id": resolved["manifest"]["bundle_id"] if resolved else None, "validation_metrics": resolved["manifest"]["metrics"] if resolved else [], "dataset_version": self.data.dataset_version})
    ...
    def series(self, bundle_id: str, target: str = "cu") -> list[dict]:
        ...
        pivot = frame.pivot(index=["event_id", "decision_at", "actual"], columns="model", values="predicted").reset_index().sort_values("decision_at")
