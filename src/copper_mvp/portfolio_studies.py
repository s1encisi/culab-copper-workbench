...
from copper_mvp.portfolio_comparison import PORTFOLIO_FILES, PortfolioRequest, compare_portfolios, source_record


class PortfolioStudies:
    def __init__(self, root, data, models):
        self.root = Path(root) / "optimizer_portfolios"
        ...
        with self.lock:
            if root.exists():
                ...
                if state["fingerprint"] != fingerprint:
                    raise WorkbenchError("请求键已用于不同配置或来源", "REQUEST_CONFLICT")
    ...
    def execute(self, identifier, request):
        ...
        try:
            ...
            update(
                ...
                finished_at=utc_now(),
                comparison_sha256=file_hash(root / "comparison.json"),
                runs=result["runs"],
            )
    ...
    def get(self, actor, identifier, result=True):
        ...
        if result and state["status"] in ("completed", "completed_with_failures"):
            if file_hash(root / "comparison.json") != state["comparison_sha256"]:
                raise WorkbenchError("比较结果哈希不符", "SOURCE_CHANGED")
            value = json.loads((root / "comparison.json").read_text(encoding="utf-8"))
            if file_hash(root / "protocol.json") != value["protocol_sha256"]:
                raise WorkbenchError("比较协议哈希不符", "SOURCE_CHANGED")
    ...
    def list(self, actor):
        ...
        for path in self.root.glob("*/state.json"):
            ...
            if state["project_id"] == actor.project_id and (
                state["owner_id"] == actor.user_id or actor.role == "owner"
            ):
    ...
    def resume(self, actor, identifier, executor=None):
        ...
        with self.lock:
            ...
            if state["status"] == "completed_with_failures":
                ...
                if all(r["status"] != "failed" for r in completed["result"]["results"]):
                    return {**completed, "reused": True}
            request = PortfolioRequest.model_validate(state["request"])
            if state["fingerprint"] != self.fingerprint(request):
                raise WorkbenchError("代码、数据或参数已变化", "SOURCE_CHANGED")
