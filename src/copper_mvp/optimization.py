...
class CandidateProblem(Problem):
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        self.all_x.append(np.array(X, copy=True))
        self.all_f.append(F.copy())
        self.all_g.append(G.copy())
        out["F"] = F
        out["G"] = G
...
def solve(data: DataRepository, models: ModelManager, request: RunRequest, output: Path, progress: Callable) -> dict:
    ...
    if len(lower):
        ...
        algorithm = NSGA2(
            ...
            mutation=PM(prob=1.0, prob_var=1 / len(lower), eta=20),
        ...
        while algorithm.has_next():
            ...
            algorithm.next()
            progress(
                "候选搜索", {"evaluations": int(algorithm.evaluator.n_eval), "budget": request.evaluation_budget}, 0
            )
            if perf_counter() - started > 30:
                stop = "time_budget"
                break
        all_x = np.vstack(problem.all_x)
        all_f = np.vstack(problem.all_f)
        all_g = np.vstack(problem.all_g)
    else:
        all_x = np.empty((1, 0))
        all_f, all_g, _ = evaluate(all_x)
    ...
    if len(feasible):
        ...
        if (
            not np.allclose(audited_f, all_f[chosen], rtol=1e-9, atol=1e-8)
            or not np.all(audited_g <= 1e-8)
            or not np.isfinite(audited_f).all()
        ):
        ...
        final_x = np.empty((0, len(lower)))
        audited_f = np.empty((0, 2))
        audited_g = np.empty((0, constraints))
        details = {}
    candidates = []
    for i, (point, objectives, constraints_row) in enumerate(zip(final_x, audited_f, audited_g, strict=True)):
        candidate = {
            "id": "candidate_" + digest(point.tolist())[:16],
            "f1": objectives[0],
            "f2": objectives[1],
            "constraint_violation": float(max(0, constraints_row.max())),
            "delta_cu": ref_F[0, 0] - objectives[0],
            "delta_power": ref_F[0, 1] - objectives[1],
            "audit_passed": True,
        }
        if request.mode == "benchmark":
            candidate.update(
                {
                    "variables": {"u1": point[0], "u2": point[1]},
                    "as": None,
                    "as_margin": None,
                    "support_distance": None,
                    "improvement_f1": candidate["delta_cu"],
                    "improvement_f2": candidate["delta_power"],
                    "delta_cu": None,
                    "delta_power": None,
                }
            )
            ...
            distance = (
                np.min(np.linalg.norm((historical_currents - currents) / current_scale, axis=1))
                if len(historical_currents)
                else None
            )
            candidate.update(
                {
                    "variables": {f"stage{s}_current_a": currents[j] for j, s in enumerate(stages)},
                    "as": details["y"][i, 1],
                    "as_margin": ref_y[1] + request.epsilon_as - details["y"][i, 1],
    ...
    representatives = {}
    if candidates:
        normalized = (audited_f - audited_f.min(axis=0)) / np.where(
            np.ptp(audited_f, axis=0) > 0, np.ptp(audited_f, axis=0), 1
        )
        balanced = np.max(0.5 * normalized, axis=1).argmin()
        indices = {
            "quality": int(audited_f[:, 0].argmin()),
            "power": int(audited_f[:, 1].argmin()),
            "balanced": int(balanced),
        }
        representatives = {k: candidates[i]["id"] for k, i in indices.items()}
        retain = sorted(
            set(
                np.linspace(0, len(candidates) - 1, min(128, len(candidates))).astype(int).tolist()
                + list(indices.values())
            )
        )
        pd.DataFrame(
            [
                {"mode": request.mode, **{k: v for k, v in c.items() if k != "variables"}, **c["variables"]}
                for c in candidates
            ]
        ).to_csv(output / "pareto_candidates.csv", index=False)
    ...
    if request.mode == "plant":
        reference.update(
            {"as": ref_y[1], "variables": {f"stage{s}_current_a": base_currents[j] for j, s in enumerate(stages)}}
        )
    else:
    ...
    return safe(
        {
            "kind": "optimization",
            "mode": request.mode,
            "event_id": request.event_id if context else None,
            "decision_at": context["decision_at"] if context else None,
            "model_scope": request.model_scope if context else "mathematical_test",
            "fold_id": resolved["fold_id"] if resolved else None,
            "bundle_id": resolved["manifest"]["bundle_id"] if resolved else None,
            "models": resolved["names"] if resolved else {"function": "constrained_quadratic"},
            ...
            "total_front_points": len(candidates),
            "representatives": representatives,
            "search_evaluations": len(all_x),
            "audit_evaluations": len(final_x),
            "reference_evaluations": 1,
            "feasible_evaluations": len(feasible),
            "feasible_rate": len(feasible) / len(all_x),
            "elapsed_ms": (perf_counter() - started) * 1000,
            "stop_reason": stop,
            "seed": request.seed,
            "epsilon_as": request.epsilon_as,
            "warnings": warnings,
            "solution_status": "feasible_set" if candidates else "no_feasible_candidate_found",
            "audit": {
                "passed": True,
                "checks": ["目标与约束已复算", "变量在本次范围内", "候选为有限数值", "返回非支配候选"],
            },
            "note": "当前电压固定的局部功率代理；候选用于同工况状态探索"
            if context
            else "无量纲数学测试；不代表工厂结果",
        }
    )
