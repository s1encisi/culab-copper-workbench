    ...
    def __init__(self,method,architecture,sequence_features=109,context_features=14):
        ...
        if method in ("GRU","LSTM"):
            ...
            self.encoder=torch.nn.Sequential(*[
                CausalBlock(sequence_features if i==0 else hidden,hidden,architecture["kernel_size"],
                            dilation,architecture["dropout"]) for i,dilation in enumerate(architecture["dilations"])])
        self.head=torch.nn.Sequential(torch.nn.Linear(hidden+context_features,32),
