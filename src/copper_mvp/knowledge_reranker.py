...
class LocalReranker:
    def __init__(self, model_dir=None):
        self.directory = Path(
            model_dir
            or os.environ.get(
                "COPPER_KNOWLEDGE_RERANKER_DIR", str(PROJECT_ROOT / "runs/dependencies/mmarco-minilm-reranker")
            )
        )
    ...
    def _load(self):
        ...
        self.session = ort.InferenceSession(
            str(self.directory / self.manifest["model_file"]), sess_options=options, providers=["CPUExecutionProvider"]
        )
        self.identity = {
            **self.manifest,
            ...
            "score_kind": "uncalibrated_relevance_logit",
        }
    ...
    def cache_key(self):
        return digest(
            {
                "manifest": file_hash(PROJECT_ROOT / "configs/runtime/knowledge_reranker.json"),
                "adapter": "g6m.cross-encoder.v1",
            }
        )
    ...
    def rank(self, query, passages):
        with self.lock:
            ...
            for offset in range(0, len(passages), 4):
                ...
                fields = {
                    "input_ids": [v.ids for v in batch],
                    "attention_mask": [v.attention_mask for v in batch],
            ...
            return {
                "scores": scores,
                "pairs": len(passages),
                "truncated_pairs": sum(n > self.manifest["max_length"] for n in lengths),
