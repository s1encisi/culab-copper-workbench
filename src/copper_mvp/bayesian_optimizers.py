...
def bayesian_spec(name):
    ...
    return {
        ...
        "constraint_handling": "native_GP_constraint_probability"
        if name not in ENTROPY_OPTIMIZERS
        else "unconstrained_mathematical_only",
        "parameters": {
            "device": "cpu",
            "dtype": "float64",
            "batch_size": 4,
            "initial_samples": 64,
            "gp": "independent-output SingleTaskGP with native RBF priors and Standardize",
            "gp_max_iterations": 25,
            "gp_fit_timeout_seconds": 15,
            "gp_training_limit": 256,
            "training_window": "first_64_plus_latest_192",
            "likelihood_noise": "learned_internal_surrogate_noise",
            "objective_sign": "negated_shared_scaled_minimization_objectives",
            "mc_samples": 32,
            "acquisition_restarts": 4,
            "acquisition_raw_samples": 64,
            "acquisition_max_iterations": 20,
            "pareto_function_samples": 4,
            "pareto_candidate_pool": 256,
            "pareto_points_max": 8,
            "pareto_points_policy": "minimum_available_count_across_function_draws_without_duplicate_padding",
            "entropy_estimator": "LB",
            "fixed_maximization_reference": [-1.1, -1.1],
        },
        "status": "registered",
...
class BayesianAdapter:
    ...
    def _training_data(self):
        ...
        return (
            torch.as_tensor((X[indices] - self.lower) / self.span, dtype=torch.double),
            torch.as_tensor(target[indices], dtype=torch.double),
            indices,
        )
    ...
    def _acquisition(self, model, train_x, bounds, iteration):
        ...
        return acq, {
            "surrogate_function_points": evaluations,
            "pareto_sets": pareto_x.tolist(),
            "pareto_fronts": pareto_y.tolist(),
            "hypercell_count": boxes.shape[-2],
        }
    ...
    def next(self):
        ...
        detail = {
            "iteration": iteration,
            "physical_evaluations_before": self.evaluator.used,
            "acquisition_calls": 0,
            "acquisition_candidate_points": 0,
        }
        started = perf_counter()
        notices = []
        try:
            with (
                tensor_random_scope(torch, self.seed + 104729 * iteration),
                ...
                detail.update(
                    gp_class=type(model).__name__,
                    gp_kernel=type(model.covar_module).__name__,
                    modeled_constraint_outputs=self.n_constraints,
                )
                mll = ExactMarginalLogLikelihood(model.likelihood, model)
                fit_start = perf_counter()
                fit = fit_gpytorch_mll_scipy(
                    mll, options={"maxiter": 25}, timeout_sec=min(15.0, self._remaining_time())
                )
                detail.update(
                    gp_fit_ms=(perf_counter() - fit_start) * 1000,
                    gp_fit_status=str(fit.status),
                    gp_fit_steps=int(fit.step),
                    gp_fit_value=float(fit.fval),
                )
                ...
                bounds = torch.stack(
                    [torch.zeros(self.n_var, dtype=torch.double), torch.ones(self.n_var, dtype=torch.double)]
                )
                ...
                try:
                    candidate, value = optimize_acqf(
                        ...
                        timeout_sec=self._remaining_time(),
                ...
                with torch.no_grad():
                    ...
                    detail.update(
                        candidate_unit=candidate.tolist(),
                        acquisition_value=float(checked_value),
                        optimizer_reported_acquisition_value=float(value),
                        posterior_mean=prediction.mean.tolist(),
                        posterior_variance=prediction.variance.tolist(),
                    )
                    ...
                    torch.save(
                        {
                            ...
                            "iteration_seed": self.seed + 104729 * iteration,
                            "acquisition_rng_state": acquisition_rng_state,
                        },
                        target,
                    )
                    detail.update(
                        gp_artifact=str(target.relative_to(self.artifact_dir)).replace("\\", "/"),
                        gp_artifact_sha256=file_hash(target),
                    )
                physical = self.lower + candidate.cpu().numpy() * self.span
                F, G, _ = self.evaluator.evaluate(physical, "search")
                detail.update(
                    observed_objectives=F.tolist(),
                    observed_constraints=G.tolist(),
                    evaluated_candidates=len(physical),
                    status="evaluated",
                )
                detail["warnings"] = [
                    {"category": type(w.message).__name__, "message": str(w.message)} for w in notices
                ]
    ...
    def details(self):
        return safe(
            {
                "status": self.stop_reason or "total_budget",
                "iterations": self.trace,
