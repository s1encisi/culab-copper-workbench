...
def uncertainty_metrics(path, labels, common_events):
    ...
    for (method, target), part in frame.groupby(["method_id", "target"]):
        ...
        if part.kind.eq("marginal_standard_deviation").all():
            ...
            lower = point - 1.6448536269514722 * std
            upper = point + 1.6448536269514722 * std
            result.update(
                kind="Gaussian_model_assumption",
                nominal_coverage=0.9,
                empirical_coverage=float(((actual >= lower) & (actual <= upper)).mean()),
                mean_width=float((upper - lower).mean()),
                gaussian_nll=float(
                    (0.5 * np.log(2 * np.pi) + np.log(std) + 0.5 * ((actual - point) / std) ** 2).mean()
                ),
            ...
            if not np.isfinite(values).all():
                raise WorkbenchError("分位数输出含非法值", "UNCERTAINTY_VALUES")
            ...
            result.update(
                ...
                crossing_events=int((~valid).sum()),
                valid_interval_fraction=float(valid.mean()),
                empirical_coverage_all_events=float(covered.mean()),
                pinball_mean=pinball.mean(axis=0).tolist(),
                mean_valid_width=float((values[valid, 2] - values[valid, 0]).mean()) if valid.any() else None,
                interval_scoring_note=(
                    "Crossed intervals are counted as uncovered; raw quantiles are not silently reordered."
                ),
            )
        else:
