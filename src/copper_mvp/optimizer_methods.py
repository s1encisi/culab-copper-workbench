...
def optimizer_method_spec(name):
    descriptions = {
        ...
        "C-TAEA": ("O08", "convergence and diversity archives with restricted mating", "native_two_archive_constraint_handling"),
        "GDE3": ("O10", "differential evolution, parent-trial dominance and crowding truncation", "native_constraint_dominance"),
        "Omni-Optimizer": ("O16", "loose dominance, objective/decision crowding and neighbor mating", "native_feasibility_first"),
    }
    design_id, mechanism, constraint_handling = descriptions[name]
    parameters = {"population": 64, "objective_scaling": "fixed_shared_metric_scale", "variables": "continuous", "objectives": 2,
                  "sample_reuse": "same_charged_initial_population", "gradient": False}
    if name == "GDE3":
        parameters.update(variant="DE/rand/1/bin", CR=0.5, F=0.5, gamma=1e-4, repair="bounce-back",
                          last_batch="sample_parent_indices_without_replacement_keep_unvisited_parents")
    else:
        parameters.update(crossover="SBX", crossover_probability=0.9, crossover_eta=15,
    ...
    if name == "MOEA-D":
        parameters.update(neighbors=20, neighbor_mating_probability=0.9, decomposition="Tchebicheff",
                          ideal_point="feasible_observations_only_after_first_feasible_point")
    if name == "RVEA":
        parameters.update(alpha=2.0, adaptation_frequency=0.1, progress="initial_generation_plus_ceil_search_budget_over_64",
    ...
    if name == "AGE-MOEA":
        ...
        for package, version in (("numba", "0.61.2"), ("llvmlite", "0.44.0")):
            try:
                ...
                raise WorkbenchError("AGE-MOEA 依赖未安装，请运行 scripts/setup_optimizer_methods.ps1", "OPTIMIZER_DEPENDENCY") from None
            if actual != version:
...
class FeasibleMOEAD(MOEAD):
    ...
    def _replace(self, k, off):
        ...
        if off_cv <= 0:
            self.feasible_ideal = off.F.copy() if self.feasible_ideal is None else np.minimum(self.feasible_ideal, off.F)
        if self.feasible_ideal is not None:
            self.ideal = self.feasible_ideal.copy()
        neighbors = self.neighbors[k]
        parent_cv = self.pop[neighbors].get("CV")[:, 0]
        parent_f = self.decomposition.do(self.pop[neighbors].get("F"), weights=self.ref_dirs[neighbors], ideal_point=self.ideal)
...
class BudgetedGDE3(GDE3):
    ...
    def _advance(self, infills=None, **kwargs):
        ...
        for parent_index, off in zip(self.trial_parent_indices, infills):
        ...
        self.pop = self.survival.do(self.problem, Population.create(*survivors), n_survive=self.pop_size,
...
def make_extended_optimizer(name, population, n_var):
    ...
    if name == "MOEA-D":
        return FeasibleMOEAD(ref_dirs=directions, n_neighbors=20, prob_neighbor_mating=0.9,
                             decomposition=Tchebicheff(), **common)
    if name == "RVEA":
        return RVEA(ref_dirs=directions, pop_size=64, alpha=2.0, adapt_freq=0.1,
                    survival=StableAPDSurvival(directions, alpha=2.0), eliminate_duplicates=True, **common)
    if name == "AGE-MOEA":
    ...
    if name == "GDE3":
        return BudgetedGDE3(pop_size=64, sampling=population, variant="DE/rand/1/bin", CR=0.5, F=0.5,
                            gamma=1e-4, de_repair="bounce-back")
    if name == "Omni-Optimizer":
        return OmniOptimizer(pop_size=64, delta=0.001, obj_crowding=True, var_crowding=True,
                             eliminate_duplicates=True, **common)
