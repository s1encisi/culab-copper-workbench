...
def portfolio_router():
    router = APIRouter(prefix="/api/v2/optimizer-portfolios")
    lock = threading.Lock()
    ...
    def run(request: Request, identifier: str, case: int, seed: int, strategy: str):
        ...
        selected = next(
            (r for r in rows if r["case"] == case and r["seed"] == seed and r["strategy"] == strategy), None
        )
        if selected is None:
            raise WorkbenchError("运行记录不存在", "RUN_NOT_FOUND")
        root = current.directory(identifier)
        path = (root / selected["result_path"]).resolve()
        if not path.is_relative_to(root.resolve()):
            raise WorkbenchError("工件路径无效", "SOURCE_CHANGED")
