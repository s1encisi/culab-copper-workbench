...
DEFAULT_CALIBRATION_METHODS=["Persistence","DeltaRidge","BayesianRidge","MultiTaskElasticNet","Quantile","NGBoost"]
...
class CalibrationRequest(BaseModel):
    ...
    seeds:list[int]=Field(default_factory=lambda:[20260911,20260912,20260913,20260914,20260915],min_length=1,max_length=10)
    coverage:Literal[0.9]=0.9
    calibration_fraction:float=Field(default=.2,ge=.1,le=.4)
    ...
    def registered(self):
        ...
        for method in self.methods:method_spec(method,self.seeds[0])
        return self
