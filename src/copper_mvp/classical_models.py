...
from copper_mvp.classical_registry import JOINT, PARAMETERS, REDUCTION, WEIGHTED
...
class StableNumericScale(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        self.tolerance_ = 64 * np.finfo(float).eps * np.maximum(1.0, np.abs(X).max(axis=0))
        self.active_ = self.var_ > self.tolerance_**2
        self.scale_ = np.where(self.active_, np.sqrt(self.var_), 1.0)
        return self

    def transform(self, X):
        X = np.asarray(X, float)
        result = (X - self.mean_) / self.scale_
        result[:, ~self.active_] = 0.0
        return result

    def get_feature_names_out(self, input_features=None):
        return np.asarray(
            input_features if input_features is not None else [f"x{i}" for i in range(self.n_features_in_)],
            dtype=object,
        )
...
def estimator(method, seed, quantile=None):
    ...
    if method == "ARD":
        return ARDRegression(**p)
    if method == "RANSAC":
        p.pop("estimator")
        return RANSACRegressor(estimator=Ridge(alpha=1.0), random_state=seed, **p)
    if method == "TheilSen":
        return TheilSenRegressor(random_state=seed, **p)
    if method == "Quantile":
        p.pop("quantiles")
        return QuantileRegressor(quantile=quantile if quantile is not None else 0.5, **p)
    if method == "MultiTaskElasticNet":
    ...
    if method == "GaussianProcess":
        p.pop("kernel")
        return GaussianProcessRegressor(kernel=ConstantKernel(1.0, "fixed") * RBF(1.0, "fixed"), random_state=seed, **p)
    ...
    if method == "AdaBoost":
        ...
        return AdaBoostRegressor(
            estimator=DecisionTreeRegressor(max_depth=3, min_samples_leaf=5, random_state=seed), random_state=seed, **p
        )
    if method == "MLP":
...
def wrapped_model(method, seed, quantile=None):
    return TransformedTargetRegressor(
        regressor=Pipeline(
            [("preprocess", classical_preprocess(method)), ("regressor", estimator(method, seed, quantile))]
        ),
...
class ClassicalModel:
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        with warnings.catch_warnings(record=True) as captured:
            ...
            if self.method_id == "Quantile":
                ...
                self.models = self.quantile_models[0.5]
    ...
    def uncertainty(self, X):
        ...
        if self.method_id == "Quantile":
            values = np.stack(
                [current + np.column_stack([m.predict(X) for m in self.quantile_models[q]]) for q in (0.1, 0.5, 0.9)],
                axis=1,
            )
        ...
        if self.method_id == "GaussianProcess":
            ...
            std = (
                np.sqrt(np.asarray(std).reshape(len(X), 2) ** 2 + PARAMETERS["GaussianProcess"]["alpha"])
                * model.transformer_.scale_
            )
        else:
            means = []
            stds = []
