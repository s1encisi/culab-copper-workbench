...
class TabularNetwork(torch.nn.Module):
    def __init__(self,method,architecture,numeric_features=220):
        ...
        if method=="TabNet":
            ...
            self.core=TabNet(input_dim=total,output_dim=2,
                cat_idxs=list(range(numeric_features,total)),cat_dims=[3]*4,
                group_attention_matrix=torch.eye(total),**options)
        elif method=="FTTransformer":
            from rtdl_revisiting_models import FTTransformer
            self.core=FTTransformer(n_cont_features=numeric_features,cat_cardinalities=[3]*4,
                d_out=2,ffn_d_hidden_multiplier=None,**options)
        elif method=="NODE":
    ...
    def initialize(self,numerical,categories):
        if self.method=="NODE":self.core.initialize(self.node_input(numerical,categories))
