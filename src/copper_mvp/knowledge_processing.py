...
class DocumentProcessing:
    ...
    def archive(self, c, actor_id, doc_id, version, parsed, operation):
        revision = c.execute(
            "SELECT COALESCE(MAX(revision),0)+1 FROM document_parses WHERE doc_id=? AND version=?", (doc_id, version)
        ).fetchone()[0]
        parsed_hash = digest(parsed)
        c.execute(
            "INSERT INTO document_parses VALUES(?,?,?,?,?,?,?,?)",
    ...
    def initialize_history(self, c):
        rows = c.execute(
            "SELECT doc_id,version,parsed_json,index_id,index_parse_hash FROM d"
            "ocument_versions WHERE parsed_json IS NOT NULL"
        ).fetchall()
        for row in rows:
            if not c.execute(
                "SELECT 1 FROM document_parses WHERE doc_id=? AND version=?", (row["doc_id"], row["version"])
            ).fetchone():
                self.archive(
                    c,
                    "legacy_migration",
            ...
            if row["index_id"] and row["index_parse_hash"] is None:
                c.execute(
                    "UPDATE document_versions SET index_parse_hash=? WHERE doc_id=? AND version=?",
        ...
        with self.store.lock, self.store.connection() as c:
            ...
            return [
                ...
                for row in c.execute(
                    (
                        "SELECT revision,parse_hash,actor_id,operation,at FROM document_par"
                        "ses WHERE doc_id=? AND version=? ORDER BY revision"
                    ),
                    (doc_id, version),
                )
            ]
    ...
    def historical_parse(self, actor, doc_id, version, revision):
        with self.store.lock, self.store.connection() as c:
            ...
            row = c.execute(
                (
                    "SELECT parse_hash,parsed_json,actor_id,operation,at FROM document_"
                    "parses WHERE doc_id=? AND version=? AND revision=?"
                ),
                (doc_id, version, revision),
            ).fetchone()
            if row is None:
    ...
    def inventory(self, actor):
        ...
        with self.store.lock, self.store.connection() as c:
            rows = c.execute(
                """SELECT v.doc_id,v.version,v.metadata_json,v.status,v.index_id,d.revoked,d.acl_json
            ...
            return [
                {
                    **{k: row[k] for k in ("doc_id", "version", "status", "index_id", "revoked")},
                    "metadata": json.loads(row["metadata_json"]),
                    "access": json.loads(row["acl_json"]),
                }
                for row in rows
            ]
    ...
    def reparse(self, actor, doc_id, version, expected_parse_hash):
        with self.store.lock, self.store.connection() as c:
            ...
            c.execute(
                (
                    "UPDATE document_versions SET parsed_json=?,status='needs_review',r"
                    "eview_json=NULL WHERE doc_id=? AND version=?"
                ),
    ...
    def run_ocr(self, actor, doc_id, version, expected_parse_hash, pages=None, dpi=180):
        with self.store.lock, self.store.connection() as c:
            ...
            selected = pages or sorted(
                {v["location"]["page"] for v in parsed["issues"] if v["reason"] == "ocr_required"}
            )
            if not selected:
        ...
        for page, item in replacements.items():
            revised["issues"].append(
                {"location": item["location"], "reason": "ocr_review" if item["text"].strip() else "ocr_no_text"}
            )
    ...
    def correct(self, actor, doc_id, version, request):
        with self.store.lock, self.store.connection() as c:
            ...
            if len(corrections) != len(request.corrections) or set(corrections) - {
                v["block_id"] for v in parsed["blocks"]
            }:
            ...
            for original in revised["blocks"]:
                ...
                for region in correction.regions:
                    ...
                    details = {
                        ...
                        "original_parse_hash": request.expected_parse_hash,
                        "reason": request.note,
                        "executable": False,
                    }
                    text = region.text
                    ...
                    output.append(
                        {
                            ...
                            "text": text,
                            "location": location,
                            "heading": region.heading or original["heading"],
                            "details": details,
