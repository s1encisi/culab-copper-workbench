...
from copper_mvp.bayesian_optimizers import BAYESIAN_OPTIMIZERS, BayesianAdapter, bayesian_spec
...
class EvaluationService:
    ...
    def __init__(self, prepared, budget):
        ...
        self.groups = [
            {
                "phase": "reference",
                "X": prepared.reference_x.copy(),
                "F": prepared.ref_F.copy(),
                "G": prepared.ref_G.copy(),
            }
        ]
        self.counts = {"reference": 1, "pilot": 0, "initial": 0, "search": 0, "verification": 0}
        self.started = perf_counter()
        self.first_feasible_ms = (
            0.0
            if (
                np.all(prepared.ref_G <= TOLERANCE)
                and np.all(prepared.reference_x >= prepared.lower)
                and np.all(prepared.reference_x <= prepared.upper)
            )
            else None
        )
        ...
        for group in self.groups:
            for X, F, G in zip(group["X"], group["F"], group["G"], strict=True):
                rows.append(
                    {
                        "phase": group["phase"],
                        **{f"x{i}": x for i, x in enumerate(X)},
                        **{f"f{i}": f for i, f in enumerate(F)},
                        **{f"g{i}": g for i, g in enumerate(G)},
                    }
                )
...
class RegisteredProblem(Problem):
    def __init__(self, evaluator, objective_scale=None):
        ...
        super().__init__(
            n_var=len(prepared.lower), n_obj=2, n_ieq_constr=prepared.constraints, xl=prepared.lower, xu=prepared.upper
        )
...
def make_optimizer(name, population, scale, n_var):
    ...
    if name in BAYESIAN_OPTIMIZERS:
        return BayesianAdapter(name, population, scale)
    raise WorkbenchError("没有该优化器", "OPTIMIZER_NOT_FOUND")


def metric_configuration(prepared, pilot_f):
    if prepared.context is None:
        scale = np.array([18.0, 8.0])  # Fixed bounds of the declared quadratic problem.
    ...
    return {
        "origin": [0.0, 0.0],
        "scale": scale.tolist(),
        "reference_point": (scale * 1.1).tolist(),
        "reference_kind": reference_kind,
        "normalized_reference": [1.1, 1.1],
    }
    ...
    try:
        ...
        write_json(
            output / "failure.json",
            {
                ...
                "completed_evaluation_rows": completed_rows,
                "charged_without_saved_values": evaluator.used - completed_rows,
...
def _run_optimizer(prepared, optimizer_id, seed, budget, seconds, output, progress, evaluator, started):
    ...
    spec = {
        **prepared.specification(),
        "metric": metric,
        "evaluator_version": "vector-evaluator.g2b.v1",
        "evaluator_hashes": {
            ...
            for name in (
                ...
                "hype.py",
                "scalarization.py",
                "bayesian_optimizers.py",
                "bayesian_problems.py",
                "neural_runtime.py",
            )
        },
        ...
        "total_budget": budget,
        "cache_policy": "disabled_for_comparison",
    }
    ...
    if dimensions:
        ...
        X = np.vstack((pilot_x, initial_x))
        F = np.vstack((pilot_f, initial_f))
        G = np.vstack((pilot_g, initial_g))
        objective_scale = (
            np.asarray(metric["scale"])
            if optimizer_id in EXTENDED_OPTIMIZERS + PLATYPUS_OPTIMIZERS + SCALAR_OPTIMIZERS + BAYESIAN_OPTIMIZERS
            else None
        )
        ...
        termination = (
            ("n_gen", 1 + int(np.ceil((budget - reserve - evaluator.used) / 64)))
            if optimizer_id == "RVEA"
            else ("n_eval", budget)
        )
        algorithm.setup(
            RegisteredProblem(evaluator, objective_scale), termination=termination, seed=seed, verbose=False
        )
        if optimizer_id in PLATYPUS_OPTIMIZERS + SCALAR_OPTIMIZERS + BAYESIAN_OPTIMIZERS:
            algorithm.deadline = started + seconds
        if optimizer_id in BAYESIAN_OPTIMIZERS:
            algorithm.artifact_dir = output
        ...
        while algorithm.has_next():
            ...
            if perf_counter() - started >= seconds:
                stop = "time_budget"
                break
            algorithm.n_offsprings = (
                remaining
                if optimizer_id in SCALAR_OPTIMIZERS
                else min(1 if optimizer_id in ("SMS-EMOA", "MOEA-D") else 64, remaining)
            )
        ...
        if (
            optimizer_id in PLATYPUS_OPTIMIZERS + SCALAR_OPTIMIZERS + BAYESIAN_OPTIMIZERS
            and algorithm.stop_reason is not None
        ):
    ...
    if len(front_indices):
        ...
        if (
            not np.allclose(verified_f, F[front_indices], rtol=1e-9, atol=1e-8)
        ...
        details = {}
    scale = np.array(metric["scale"])
    normalized = verified_f / scale
    ...
    for i, (x, f, g) in enumerate(zip(verified_x, verified_f, verified_g, strict=True)):
        ...
        if prepared.context:
            ...
            row["support_distance"] = (
                float(
                    np.min(np.linalg.norm((prepared.historical_currents - currents) / prepared.current_scale, axis=1))
                )
    ...
    result = safe(
        {
            "schema_version": "optimizer-result.g2b.v1",
            ...
            "feasible_search_points": len(feasible),
            "feasible_rate": len(feasible) / len(X),
            "decision_front_points": decision_front_count,
            "objective_front_points": objective_front_count,
            "verified_front_points": len(candidates),
            "first_feasible_ms": evaluator.first_feasible_ms,
            "timing_scope": "pilot_initialization_search_and_verification_excludes_problem_assembly",
            "elapsed_ms": (perf_counter() - started) * 1000,
    ...
    elif optimizer_id in BAYESIAN_OPTIMIZERS:
        result["optimizer_spec"] = bayesian_spec(optimizer_id)
        result["bayesian"] = algorithm.details() if dimensions else {"status": "fixed_reference", "iterations": []}
    evaluator.save(output)
...
def compare_optimizers(data, models, request, output, progress=lambda detail: None):
        ...
        for case, event in enumerate(request.event_ids if request.mode == "plant" else (None,)):
            ...
            for seed in request.seeds:
                for optimizer in request.optimizers:
                    ...
                    prepared = build_comparison_problem(data, models, request, event, seed)
                    result = run_optimizer(
                        prepared, optimizer, seed, request.total_budget, request.seconds_per_run, folder, progress
                    )
    ...
    summary = {
        "schema_version": "optimizer-comparison.g2b.v1",
        "status": "completed",
        "cases": len(request.event_ids) if request.mode == "plant" else 1,
        "optimizers": list(request.optimizers),
    ...
    rows = [
        {
            ...
            for k in (
                ...
                "total_evaluations",
                "feasible_rate",
                "decision_front_points",
                "objective_front_points",
    ...
    for row, result in zip(rows, results, strict=True):
        row.update(
            ...
            mode=request.mode,
            model_profile=request.model_profile if request.mode == "plant" else request.benchmark_problem,
        )
        detail = result.get("scalarization")
        row["scalarization_converged"] = (
            detail["converged_subproblems"] if detail and "converged_subproblems" in detail else None
        )
    ...
    report += (
        ...
        f"{request.mode}"
        "；问题/代理："
        f"{(request.benchmark_problem if request.mode == 'benchmark' else request.model_profile)}"
        "；范围："
        f"{(request.model_scope if request.mode == 'plant' else 'mathematical')}"
        "。\n"
        "\n"
    )
    report += (
        ...
        f"{summary['cases']}"
        " 个，优化器 "
        f"{len(request.optimizers)}"
        " 个，种子 "
        f"{len(request.seeds)}"
        " 个。每次总求值上限 "
        f"{request.total_budget}"
        "，包含参考、探测、初始种群、搜索和复算。\n"
        "\n"
    )
    report += (
        "| 工况 | 优化器 | 种子 | 候选状态 | 停止原因 | 子问题收敛 | HV | I"
        "GD+ | 求值 | 复核前沿点 | 耗时(ms) |\n"
        "|---|---|---:|---|---|---|---:|---:|---:|---:|---:|\n"
    )
    for r in results:
        ...
        convergence = (
            f"{detail['converged_subproblems']}/{len(detail['jobs'])}"
            if detail and "converged_subproblems" in detail
            else "—"
        )
        report += (
            "| "
            ...
            f"{igd_text}"
            " | "
            f"{r['total_evaluations']}"
            " | "
            f"{r['verified_front_points']}"
            " | "
            f"{r['elapsed_ms']:.1f}"
            " |\n"
        )
    report += (
        "\n"
        "HV 只在同一问题签名下比较。SMS-EMOA 采用单后代更新及固定参考点；目"
        "标点数和决策点数分别记录。工厂结果是既有模型中的局部情景，不改变执"
        "行资格。\n"
    )
