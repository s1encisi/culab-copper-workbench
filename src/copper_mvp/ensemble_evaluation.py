...
def hierarchical_interval(differences,decisions,seed=20260911,replicates=1000):
    ...
    if len(unique)<20:
        return {"low":None,"high":None,"blocks":len(unique),"seeds":len(differences),"status":"INSUFFICIENT_BLOCKS"}
    ...
    return {"low":float(low),"high":float(high),"blocks":len(unique),"seeds":len(differences),"status":"COMPUTED",
        "method":"seed_and_calendar_week_block_bootstrap","replicates":replicates,
...
def error_correlation(predictions,actual,names):
    ...
    for i in range(len(names)):
        ...
        for j in range(len(names)):
            row.append(float(centered[:,i]@centered[:,j]/(norms[i]*norms[j])) if norms[i]*norms[j]>0 else None)
        result.append(row)
    return {"methods":list(names),"correlations":result}


def verify_fold(root,manifest,book,expected_fold):
    for name,key in (("predictions.csv","predictions_sha256"),("weight_decisions.jsonl","decisions_sha256"),
                     ("inner_oof.csv","inner_oof_sha256"),("inner_plan.json","inner_plan_sha256"),
                     ("selection.json","selection_sha256"),("bagging_selection.json","bagging_selection_sha256")):
    ...
    if set(plan["outer_train_ids"])!=set(expected_fold["train_event_ids"]) or set(plan["outer_validation_ids"])!=set(expected_fold["validation_event_ids"]):
    ...
    for part in plan["inner_folds"]:
        ...
        if (set(train)&set(valid) or set(train)-set(plan["outer_train_ids"]) or set(valid)-set(plan["outer_train_ids"])
            or any(book.decision(e)>=cutoff for e in train) or any(book.decision(e)<cutoff for e in valid)):
    ...
    eligible=book.mature([e for e in oof.event_id if book.decision(e)<source_time(meta["fit_cutoff_at"])],meta["fit_cutoff_at"])
...
def evaluate_ensembles(data,root):
    ...
    if frame.duplicated(["event_id","method_id","seed_key"]).any() or set(zip(frame.event_id,frame.method_id,frame.seed_key))!=expected:
    ...
    eligible={e for e in data.fold_for if all((e,t) in labels and labels[(e,t)].quality_eligible and labels[(e,t)].value is not None for t in ("cu","as"))}
    common=set(eligible);groups={};coverage={}
    for (method,seed),part in frame.groupby(["method_id","seed_key"]):
        ...
        coverage[f"{method}:{seed}"]={"expected_events":len(data.fold_for),"valid_predictions":len(valid),
    ...
    metric_rows=[];seed_rows=[];residuals={}
    ...
    for target_index,(target,unit) in enumerate((("cu","g/L"),("as","mg/L"))):
        ...
        residuals[target]=error_correlation(np.column_stack([groups[(m,-1)].loc[events,target].to_numpy() for m in names]),actual,names)
        for method,seeds in protocol["expected_methods"].items():
            ...
            for seed,predicted in zip(seeds,values):
                seed_rows.append({"method_id":method,"seed":None if seed==-1 else seed,"target":target,"unit":unit,
                                  **numerical_metrics(actual,predicted)})
            by_mode={};by_fold={}
            for mapping,attribute in ((by_mode,lambda e:data.mode_cards[e]["mode_code"]),(by_fold,lambda e:data.fold_for[e])):
                for group in sorted({attribute(e) for e in events}):
                    mask=np.array([attribute(e)==group for e in events])
                    mapping[group]={"n":int(mask.sum()),"mae_mean_over_seeds":float(errors[:,mask].mean()),
                                    "bias_mean_over_seeds":float((values[:,mask]-actual[mask]).mean())}
            metric_rows.append({"method_id":method,"target":target,"unit":unit,"n":len(events),"seed_count":len(seeds),
                "mae":float(seed_mae.mean()),"seed_mae_sd":float(seed_mae.std(ddof=1)) if len(seeds)>1 else None,
                "rmse":float(np.sqrt(np.square(values-actual).mean(axis=1)).mean()),
                "bias":float((values-actual).mean()),"paired_mae_difference":float((errors-base_error).mean()),
                "paired_ci95":hierarchical_interval(errors-base_error,times),"mode_metrics":by_mode,"fold_metrics":by_fold,
    ...
    for fold in manifest["folds"]:
        for artifact in fold["artifacts"]:
            ...
            row=resources.setdefault(method,{"samples":[],"scope":artifact["timing_scope"],"fold_seed_artifacts":0})
            row["samples"].extend(artifact["latency_samples_ms"]);row["fold_seed_artifacts"]+=1
    resources={m:{"p50_ms":float(np.percentile(r["samples"],50)),"p95_ms":float(np.percentile(r["samples"],95)),
                  "samples":len(r["samples"]),"scope":r["scope"],"fold_seed_artifacts":r["fold_seed_artifacts"]}
               for m,r in resources.items()}
    result=safe({"schema_version":"ensemble-evaluation.g5b.v1","created_at":utc_now(),"status":"completed",
        "evaluation_mode":"historical_replay","common_events":len(events),"common_event_hash":digest(events),
        "metrics":metric_rows,"seed_metrics":seed_rows,"coverage":coverage,"residual_correlations":residuals,
        "resources":resources,"training_resources":manifest["training_resources"],
        "replay_manifest_sha256":file_hash(root/"replay_manifest.json"),"protocol_sha256":manifest["protocol_sha256"],
        "live_model_change":False,"optimization_proxy_approval":False,"causal_control":False,
        "method_inventory_increment":0,"evaluator_sha256":file_hash(Path(__file__))})
    write_json(root/"evaluation.json",result)
    flat=[{k:v for k,v in row.items() if k not in ("paired_ci95","mode_metrics","fold_metrics")} for row in metric_rows]
    ...
    text+=f"共同评价 {len(events)} 个原外层 OOF 事件。Bagging 使用 {len(protocol['request']['seeds'])} 个固定种子；二层模型和融合超参数仅在各外层训练段内选择。\n\n"
    text+="| 方法 / 对照 | 目标 | 平均 MAE | 种子间 SD | 相对 Persistence 的 MAE 差 |\n|---|---|---:|---:|---:|\n"
    for row in metric_rows:
        sd="—" if row["seed_mae_sd"] is None else f"{row['seed_mae_sd']:.6g}"
        text+=f"| {row['method_id']} | {row['target']} ({row['unit']}) | {row['mae']:.6g} | {sd} | {row['paired_mae_difference']:.6g} |\n"
    text+="\n分数先逐个已拟合种子计算，再求均值；没有把多个种子的平均预测当成单次模型结果。区间联合重采样种子与日历周块。Bagging_d1/d7 是同一方法的块长度对照，不计入新增方法数。\n\n"
    text+="MeanEnsemble 与 Stacking/OOFConvex 共享相同的全训练段基模型；DynamicConvex 的更新只使用已可得标签。Blending 保留较早训练的基模型，并使用独立尾段拟合权重。\n\n"
    text+="训练账本记录实际所有拟合，多个对照共享的基模型只计一次。原 G2a 的 ElasticNet、Huber、PLS 预测作为历史参考保留，本次没有重训这三种方法。动态权重更新时间与静态模型完整推理时间分别列示。\n\n"
    text+="[完整评价](evaluation.json) · [逐种子指标](seed_metrics.csv) · [固定协议](protocol.json) · [训练与工件账本](replay_manifest.json) · [外层预测](predictions.csv)\n\n"
