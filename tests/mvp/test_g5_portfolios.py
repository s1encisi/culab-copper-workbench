...
from copper_mvp.optimizer_portfolio import STRATEGIES,PortfolioEvaluator,run_portfolio,signed_archive,import_archive,merge_archives
...
def independent_hv(front,reference):
    total=0.;previous=reference[1]
    for x,y in front[np.argsort(front[:,0])]:
        if x<=reference[0] and y<previous:
            total+=(reference[0]-x)*(previous-y);previous=y
    return total
...
def test_shared_budget_pilots_and_independent_front_verification(tmp_path):
    ...
    for strategy in STRATEGIES:
        ...
        if strategy in ("adaptive","equal_share"):
            trace=json.loads((root/"allocation_trace.json").read_text())
            pilots=[r for r in trace if r["stage"]=="arm_pilot"]
            assert {r["arm"] for r in pilots}=={"NSGA-II","SPEA2"}
            assert len({sum(r["charged_evaluations"] for r in pilots if r["arm"]==arm) for arm in ("NSGA-II","SPEA2")})==1
...
def test_archive_signature_change_recomputes_and_tamper_is_rejected():
    prepared=problem();meter=PortfolioEvaluator(prepared,64)
    X=np.array([[.5,.5],[1.,1.]])
    ...
    altered={**archive,"F":[[999.,999.],[999.,999.]]}
...
def test_infeasible_plateau_and_new_problem_do_not_share_algorithm_state(tmp_path):
    ...
    for name,violation in (("infeasible",1.),("plateau",-1.)):
        prepared=problem()
        prepared.evaluate=lambda X,g=violation:(np.ones((len(X),2)),np.full((len(X),1),g),{})
        prepared.ref_F=np.ones((1,2));prepared.ref_G=np.full((1,1),violation);prepared.reference_front=None
...
def test_portfolio_api_identity_and_saved_run_evidence(tmp_path,monkeypatch):
    ...
    with TestClient(create_app(run_dir=tmp_path/"api"),base_url="http://127.0.0.1") as client:
        ...
        response=client.post("/api/v2/optimizer-portfolios",json={"request_key":"api-math","mode":"benchmark","seeds":[17],"total_budget":128})
        ...
        run=client.get("/api/v2/optimizer-portfolios/"+identifier+"/runs",params={"case":0,"seed":17,"strategy":"adaptive"})
        ...
        assert client.post("/api/v2/optimizer-portfolios",headers=headers,json={"request_key":"denied","mode":"benchmark","seeds":[17]}).status_code==403
...
def test_failed_attempt_is_preserved_after_recovery_and_not_counted_as_fair_pair(tmp_path,monkeypatch):
    ...
    def once(prepared,strategy,seed,budget,seconds,output,**kwargs):
        if strategy=="adaptive" and not state["failed"]:
            ...
            write_json(output/"failure.json",{"total_evaluations":7,"charged_evaluations":{"reference":1,"pilot":6}})
    ...
    assert second["total_recorded_evaluations"]==sum(a["total_evaluations"] or 0 for r in second["results"] for a in r["attempts"])
