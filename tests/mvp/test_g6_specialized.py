...
def test_specialized_registered_model_roundtrip_and_probability(tmp_path, name, monkeypatch, specialized_runtime):
    ...
    X = rng.normal(size=(80, 114))
    X[:, 0] = 4 + 0.2 * X[:, 0]
    X[:, 1] = 300 + 10 * X[:, 1]
    X[:, 110:] = rng.integers(0, 3, (80, 4))
    X[::7, 5] = np.nan
    ...
    values = model.predict(X[:12])
    assert values.shape == (12, 2) and np.isfinite(values).all()
    ...
    if name == "Cubist":
        ...
        original = native_module._copper_original_parser(
            broken.replace("-nan(ind)", "nan"), model.models[0].feature_names_in_
        )
        for index in (1, 2, 3):
