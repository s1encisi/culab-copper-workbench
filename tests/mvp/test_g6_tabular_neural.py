...
def test_missing_mask_and_scaling_use_training_state():
    X=np.zeros((6,114));X[:,0]=np.arange(6);X[:,1]=10
    X[:,110:]=1;X[0,3]=np.nan
    pre=NeuralInputs().fit(X)
    means=pre.scaler.mean_.copy()
    query=X[:2].copy();query[1,4]=np.nan;query[0,0]=10000
...
def test_registered_network_trains_and_roundtrips(method,tmp_path,monkeypatch):
    ...
    X=rng.normal(size=(65,114));X[:,0]=4+.2*X[:,0];X[:,1]=300+10*X[:,1]
    X[:,110:]=rng.integers(0,3,(65,4));X[::7,3]=np.nan
    y=X[:,:2]+np.column_stack((.2*X[:,2],20*X[:,4]))
    before=torch.get_rng_state().clone()
    model=RegisteredModel(method,21).fit(X,y,sample_weight=np.linspace(.5,1.5,len(X)))
    assert torch.equal(before,torch.get_rng_state())
    values=model.predict(X[:5])
    path=tmp_path/"network.joblib";joblib.dump(model,path)
