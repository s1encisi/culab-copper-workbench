...
def test_finite_sample_quantile_uses_order_statistic_without_interpolation():
    assert finite_sample_quantile(np.arange(9),.9)==8
    with pytest.raises(WorkbenchError) as error:
        finite_sample_quantile(np.arange(8),.9)
...
def test_marginal_and_joint_coverage_are_distinct_and_unit_equivariant():
    ...
    assert marginal_joint==.82
    assert joint_joint>=.9
    multiplier=np.array([1000.,.001])
...
def test_interval_scoring_and_invalid_bounds():
    result=interval_metrics([3,0],[2,2],[1,1],[4,4],.9,scale=2)
    assert result["picp"]==.5
    assert result["mean_width"]==3 and result["normalized_width"]==1.5
    assert result["wis"]==pytest.approx((.65/1.5+2.15/1.5)/2)
...
def test_calibration_split_purges_labels_not_available_at_base_fit_time():
    ...
    for i,event in enumerate(identifiers):
        for target,unit,delay in (("cu","g/L",1),("as","mg/L",3)):
            ...
            records.append(LabelRecord(event_id=event,pair_id=event,target=target,value=float(i+1),unit=unit,
                decision_at=rows[event].decision_at,available_at=available,assumed_sample_time=available-timedelta(hours=2),
                quality_eligible=True,source_version="synthetic-v1",source_kind="synthetic_fixture"))
    class Ledger:
        def latest(self,cutoff):
            return {(r.event_id,r.target):r for r in records if r.available_at<=source_time(cutoff)}
    request=SimpleNamespace(minimum_calibration=10,minimum_fit=20,calibration_fraction=.2)
    split=temporal_split(data,Ledger(),identifiers[:97],identifiers[100:],start+timedelta(hours=100),request,"fold")
