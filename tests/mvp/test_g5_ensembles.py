...
def fixture_data():
    ...
    X[:,0]=40+np.arange(len(events))*.02+rng.normal(scale=.05,size=len(events))
    ...
    data=SimpleNamespace(X=values,row=lambda e:frame.loc[e],mode_cards={e:{"mode_code":"A" if i%3 else "B"} for i,e in enumerate(events)})
    labels=[]
    for i,e in enumerate(events):
        actual=X[min(i+1,len(events)-1),:2]
        for t,(target,unit) in enumerate((("cu","g/L"),("as","mg/L"))):
            labels.append(LabelRecord(event_id=e,pair_id="pair-"+e,target=target,value=actual[t],unit=unit,
                decision_at=dates[i],available_at=dates[i]+timedelta(hours=8),
                assumed_sample_time=dates[i]+timedelta(hours=6),quality_eligible=True,
                source_version="synthetic-ensemble",source_kind="synthetic_fixture"))
    fold={"fold_id":"FOLD_SYNTHETIC","fit_cutoff_at":dates[120].isoformat(),
          "train_event_ids":events[:120],"validation_event_ids":events[120:144]}
    settings=EnsembleSettings(inner_boundaries=(.5,.75,1.),ridge_alphas=(1.,),convex_penalties=(0.,.1),
        dynamic_windows=(8,16),dynamic_minimum=5,bag_members=(2,3))
    return data,LabelLedger(labels),fold,settings


def test_simplex_solution_is_nonnegative_normalized_and_beats_vertices():
    P=np.array([[0.,2.,4.],[0.,4.,2.],[0.,3.,3.],[0.,2.,2.]])
    actual=P@np.array([.25,.5,.25])
    weights=simplex_weights(P,actual,1.)
    assert np.min(weights)>=0 and weights.sum()==pytest.approx(1)
    assert np.mean((P@weights-actual)**2)<1e-12
    regularized=simplex_weights(P,actual,1.,100.)
...
def test_nested_plan_uses_only_available_label_versions_and_reproducible_blocks():
    ...
    late=original.model_copy(update={"revision":2,"supersedes":original.record_id,
        "available_at":data.row("ensemble-080").decision_at,"value":original.value+1})
    ...
    ids=plan[0]["train_event_ids"];dates=[book.decision(e) for e in ids]
...
def test_real_nested_fit_serializes_and_future_labels_do_not_change_earlier_predictions(tmp_path):
    ...
    expected={"Persistence","DeltaRidge","DeltaHGB","Stacking","OOFConvex","Blending","MeanEnsemble","DynamicConvex",
              "Bagging_d1","Bagging_d7","BlockBagging"}
    ...
    changed=LabelLedger([r.model_copy(update={"value":r.value+1000}) if r.event_id>="ensemble-134" else r for r in ledger.records])
    second=tmp_path/"second"
    train_outer_fold(EnsembleData(data,changed),fold,settings,(19,),20260905,second,TrainingBudget(180,lambda value:None))
    ...
    with (first/"weight_decisions.jsonl").open() as stream:
        for line in stream:
            ...
            assert decision["latest_label_available_at"] is None or decision["latest_label_available_at"]<=decision["as_of"]
...
def test_independent_evaluation_keeps_all_seeds_and_checks_hashes(tmp_path,monkeypatch):
    from copper_mvp.common import file_hash,write_json,WorkbenchError
    from copper_mvp.ensemble_evaluation import evaluate_ensembles
    data,ledger,fold,settings=fixture_data()
    data.fold_for={e:fold["fold_id"] for e in fold["validation_event_ids"]}
    events=fold["validation_event_ids"]
    records=[]
    for method,seeds,error in (("Persistence",[-1],2.),("DeltaRidge",[-1],1.),
                               ("DeltaHGB",[-1],.5),("BlockBagging",[11,12],.25)):
        for seed in seeds:
            for i,event in enumerate(events):
                actual=[next(r.value for r in ledger.records if r.event_id==event and r.target==t) for t in ("cu","as")]
                offset=error if seed!=12 else error+.1
                records.append({"event_id":event,"fold_id":fold["fold_id"],"method_id":method,
                    "seed":None if seed==-1 else seed,"decision_at":data.row(event).decision_at.isoformat(),
                    "fit_cutoff_at":fold["fit_cutoff_at"],"cu":actual[0]+offset,"as":actual[1]+offset,"status":"completed"})
    root=tmp_path
    protocol={"source":{"fixture":"synthetic"},"base_methods":["Persistence","DeltaRidge","DeltaHGB"],
        "expected_methods":{"Persistence":[-1],"DeltaRidge":[-1],"DeltaHGB":[-1],"BlockBagging":[11,12]},
        "request":{"seeds":[11,12]},"evaluation_as_of":"2025-01-01T00:00:00+00:00"}
    ...
    manifest={"protocol_sha256":file_hash(root/"protocol.json"),"predictions_sha256":file_hash(root/"predictions.csv"),
        "decisions_sha256":file_hash(root/"decisions.jsonl"),"folds":[],"training_resources":{}}
    ...
    assert metric["mae"]==pytest.approx(.3) and metric["seed_count"]==2
...
def test_resume_is_once_and_never_reopens_a_running_study(tmp_path):
    ...
    from copper_mvp.common import digest,file_hash,write_json,WorkbenchError
    ...
    folder=service.directory(identifier);folder.mkdir()
    ...
    fingerprint=digest({"request":request.model_dump(),"source":"synthetic-source","policy":service.policy_spec(),
                         "code":{n:file_hash(package/n) for n in service.signature_files}})
    write_json(folder/"state.json",{"id":identifier,"owner_id":"owner","project_id":actor.project_id,
        "status":"failed","request":request.model_dump(mode="json"),"fingerprint":fingerprint,"created_at":"synthetic"})
    queue=[]
