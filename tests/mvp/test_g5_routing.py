        ...
        for j, (target, unit) in enumerate((("cu", "g/L"), ("as", "mg/L"))):
            labels.append(
                LabelRecord(
                    ...
                    available_at=decision + timedelta(hours=6),
                    assumed_sample_time=decision + timedelta(hours=4),
                    quality_eligible=True,
                    source_version="synthetic-routing",
                    source_kind="synthetic_fixture",
                )
            )
        for method, error in (("Persistence", 2), ("Challenger", 0.2)):
            forecasts.append(
                {
                    "event_id": event,
                    "method_id": method,
                    "decision_at": decision.isoformat(),
                    "fit_cutoff_at": (begin - timedelta(days=1)).isoformat(),
...
def engine(parts, **changes):
    ...
    return MatureMetrics(
        changes.get("forecasts", forecasts),
        changes.get("labels", labels),
        changes.get("attributes", attributes),
        ("Persistence", "Challenger"),
        {"cu": 10.0, "as": 100.0},
        {"Persistence": {"p95_ms": 0.01}, "Challenger": {"p95_ms": 0.1}},
...
def test_future_labels_and_predictions_cannot_change_earlier_metrics_or_routes():
    ...
    changed_labels = LabelLedger(
        [r.model_copy(update={"value": r.value + 1000000}) if r.available_at > cutoff else r for r in parts[1].records]
    )
...
def test_revision_recomputes_future_snapshot_without_rewriting_original():
    ...
    changed = source.model_copy(
        update={
            ...
            "available_at": datetime(2024, 1, 20, 12, tzinfo=UTC),
        }
    )
    revised = parts[1].with_revision(changed)
    original = engine(parts)
    newer = engine(parts, labels=revised)
    early = datetime(2024, 1, 20, tzinfo=UTC)
...
def test_common_window_counts_missing_predictions_without_extending_it():
    ...
    policy = parts[3].model_copy(update={"minimum_coverage": 0.8})
    state = engine(parts, forecasts=data, policy=policy).snapshot(datetime(2024, 1, 21, tzinfo=UTC), "A")
    short = state["short"]
    ...
    unsupported = engine(parts).snapshot(datetime(2024, 1, 21, tzinfo=UTC), "unseen-mode")
...
def test_future_fit_and_insufficient_blocks_are_explicit():
    ...
    dates = [datetime(2024, 1, 1, tzinfo=UTC) + timedelta(days=i) for i in range(8)]
    ...
    state = original.snapshot(datetime(2024, 1, 21, tzinfo=UTC), "A")
...
def test_confirming_evidence_is_retained_at_switch_and_cooldown():
    ...
    snapshots = {}
    router = PolicyRouter(
        engine(parts, forecasts=data, policy=policy),
        by_mode=False,
...
def test_routing_api_authorization_and_evidence_integrity(tmp_path, monkeypatch):
    ...
    with TestClient(create_app(run_dir=tmp_path / "runtime"), base_url="http://127.0.0.1") as http:
        ...
        write_json(
            ...
            {"scope": "synthetic", "replay_manifest_sha256": file_hash(root / "replay_manifest.json")},
        )
        write_json(
            root / "state.json",
            {
        ...
        owner = wb.access.authenticate(key=wb.access.owner_key_path.read_text(encoding="utf-8").strip())
        http.post(
        ...
        assert (
            http.get(f"/api/v2/prediction-studies/{identifier}/decisions", params={"event_id": "synthetic"}).status_code
            == 200
        )
        ...
        assert (
            http.post(
                "/api/v2/prediction-studies", headers=headers, json={"request_key": "denied", "comparison_id": "b" * 32}
            ).status_code
            == 403
        )
