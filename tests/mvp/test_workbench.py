...
def test_actual_data_and_candidate_feature_consistency(data):
    ...
    np.testing.assert_allclose(candidate, data.X.loc[[event["event_id"]]].to_numpy(), rtol=1e-8, atol=1e-5, equal_nan=True)
    ...
    for j, stage in enumerate((3, 4)):
        ...
        assert changed[0, mean_index] - candidate[0, mean_index] == pytest.approx(currents[0, j] * .01 / count)
...
def test_real_optimization_recomputes_power_and_as(data, trained, tmp_path):
    manager, _ = trained
    event = data.events(scope="dual", limit=1)["items"][0]
    result = solve(data, manager, RunRequest(task_type="optimize", request_key="test-opt", event_id=event["event_id"], evaluation_budget=128), tmp_path, lambda *args: None)
    assert result["search_evaluations"] <= 128
    row = data.row(event["event_id"])
    for c in result["candidates"]:
        power = sum(row[f"stage{s}_voltage_v__t_minus_0h"] * c["variables"][f"stage{s}_current_a"] for s in result["supported_stages"]) / 1000
...
def test_api_rejects_future_fields_and_remote_origin(client, data):
    ...
    assert client.post("/api/runs", json={"task_type": "predict", "request_key": "future", "event_id": event_id, "target_cu_g_l": 1}).status_code == 422
...
def test_api_benchmark_selection_explanation_and_export(client):
    ...
    selected = client.post(f"/api/runs/{run['run_id']}/selection", json={"candidate_id": candidate["id"], "label": "测试候选"})
...
@pytest.mark.parametrize("scenario,blocked", [("clean", False), ("future_field", True), ("model_mismatch", True), ("invalid_numeric", True), ("invalid_interval", True)])
def test_actual_diagnostic_paths(client, scenario, blocked):
    created = client.post("/api/runs", json={"task_type": "diagnostic", "scenario": scenario, "request_key": scenario}).json()
...
def test_concurrent_idempotency_and_budget(tmp_path):
    ...
    store.reserve_cost("a", "2026-09", .7, 1)
    with pytest.raises(WorkbenchError): store.reserve_cost("b", "2026-09", .4, 1)
    store.settle_cost("a", .1)
    store.reserve_cost("b", "2026-09", .4, 1)
    with pytest.raises(WorkbenchError): store.reserve_cost("c", "2026-09", float("nan"), 1)


def test_completed_runs_survive_restart(tmp_path, data):
    wb = Workbench(tmp_path, data)
    request = RunRequest(task_type="predict", request_key="persist", event_id=data.summaries[-1]["event_id"], model_profile="Persistence")
...
def test_llm_metadata_only_and_cached_budget(client, monkeypatch):
    ...
    for key, value in {"COPPER_MVP_LIVE_LLM": "1", "COPPER_MVP_MONTHLY_CNY": "2", "COPPER_MVP_CALL_CAP_CNY": "1", "COPPER_MVP_INPUT_CNY_PER_MILLION": "2", "COPPER_MVP_OUTPUT_CNY_PER_MILLION": "4", "DEEPSEEK_MODEL": "deepseek-v4-pro", "DEEPSEEK_API_KEY": "sk-test-synthetic-not-real", "DEEPSEEK_BASE_URL": "https://api.deepseek.com"}.items():
        monkeypatch.setenv(key, value)
    created = client.post("/api/runs", json={"task_type": "diagnostic", "request_key": "explain", "scenario": "clean"}).json()
    ...
    def response(request):
        ...
        calls.append(body)
        value = {"artifact_ids": [run["run_id"]], "summary": "本次任务已经完成，计算依据可以从运行记录追溯。", "reason_codes": ["RUN_COMPLETED"], "changed_numeric_result": False}
        return httpx.Response(200, json={"choices": [{"message": {"content": json.dumps(value, ensure_ascii=False)}}], "usage": {"prompt_tokens": 100, "completion_tokens": 50}})
