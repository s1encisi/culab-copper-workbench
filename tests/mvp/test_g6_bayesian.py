...
def test_entropy_methods_require_explicit_unconstrained_problem():
    ...
    request = OptimizerComparisonRequest(
        request_key="entropy", mode="benchmark", benchmark_problem="unconstrained_quadratic", optimizers=("MES",)
    )
...
def test_one_bayesian_iteration_fits_gp_proposes_and_evaluates_real_candidates(tmp_path, name):
    ...
    request = OptimizerComparisonRequest(
        request_key="iteration", mode="benchmark", benchmark_problem=problem_id, optimizers=(name,)
    )
    prepared = build_comparison_problem(None, None, request, None, 17)
    ...
    expected_class = {
        ...
        "MES": "qLowerBoundMultiObjectiveMaxValueEntropySearch",
        "JES": "qLowerBoundMultiObjectiveJointEntropySearch",
    }[name]
    ...
    model = SingleTaskGP(
        saved["train_X"], saved["train_Y"], outcome_transform=Standardize(m=saved["train_Y"].shape[-1])
    )
...
def test_native_constraint_probability_changes_acquisition_preference(name):
    ...
    with tensor_random_scope(torch, 17):
        ...
        model = SingleTaskGP(X, Y, outcome_transform=Standardize(m=3))
        model.eval()
