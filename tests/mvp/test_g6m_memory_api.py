...
def test_document_answer_and_memory_revalidate_source_without_provider_disclosure(tmp_path):
    ...
    def provider(request):
        ...
        if not feedback or "error" in feedback[-1]:
            ...
            arguments = {
                "kind": "answer",
                "answer": "本地文档依据：{{" + reference + ".document_excerpt_1}}",
    ...
    with TestClient(create_app(tmp_path, DataRepository()), base_url="http://127.0.0.1") as client:
        code = client.app.state.workbench.access.owner_key_path.read_text(encoding="utf-8").strip()
        ...
        task = client.post(
            f"/api/v2/sessions/{conversation}/messages",
...
def test_document_revocation_invalidates_repeated_tool_cache(tmp_path):
    ...
    def provider(request):
        ...
        feedback = [json.loads(m["content"]) for m in payload["messages"] if m["role"] == "tool"]
        if len(calls) == 2:
            holder["wb"].knowledge.change_access(
                Principal("owner", "owner"), "cache-fixture", DocumentAccess(revoked=True)
            )
        if len(calls) < 3:
            ...
            name, arguments = (
                "finish_answer",
                {"kind": "clarification", "answer": "文档已撤销，请重新选择可访问的来源。", "evidence_ids": []},
            )
        return httpx.Response(
            200,
            json={
    ...
    with TestClient(create_app(tmp_path, DataRepository()), base_url="http://127.0.0.1") as client:
        ...
        key = holder["wb"].access.owner_key_path.read_text(encoding="utf-8").strip()
        ...
        task = client.post(
            f"/api/v2/sessions/{session}/messages", json={"question": "检查当前文档", "request_key": "cache"}
        ).json()
        for _ in range(600):
