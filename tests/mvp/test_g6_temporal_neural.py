...
def sample_data():
    rng=np.random.default_rng(32);ids=[f"e{i}" for i in range(40)]
    ...
    X=np.zeros((40,114));X[:,0]=4+.2*rng.normal(size=40);X[:,1]=300+10*rng.normal(size=40)
    ...
    labels={(e,t):SimpleNamespace(quality_eligible=True,value=float(matrix.loc[e,0 if t=="cu" else 1]+
             (.2 if t=="cu" else 20)*frame.loc[e,"signal0__t_minus_0h"])) for e in ids for t in ("cu","as")}
...
def test_future_sequence_values_do_not_change_past_outputs(method):
    ...
    X=torch.randn(2,7,109);changed=X.clone();changed[:,4:]+=100
    with torch.no_grad():
        a=network.encode(X);b=network.encode(changed)
...
def test_anchor_times_and_training_preprocessing_remain_causal():
    ...
    data.frame.loc[ids[0],"signal0__t_minus_0h__age_h"]=-.1
    with pytest.raises(WorkbenchError,match="锚点"):raw_anchor_windows(data,ids[:1])
...
def test_context_models_train_serialize_and_do_not_read_labels_at_prediction(method,tmp_path,monkeypatch):
    ...
    monkeypatch.setattr(module,"DataService",lambda data:SimpleNamespace(labels=SimpleNamespace(latest=lambda cutoff:labels)))
    monkeypatch.setitem(TEMPORAL_TRAINING,"epochs",2)
    monkeypatch.setitem(TEMPORAL_TRAINING,"batch_size",8)
    torch=load_tensor_runtime();before=torch.get_rng_state().clone()
    ...
    np.testing.assert_allclose(duplicate.predict_context(data,ids[25:30]),model.predict_context(data,ids[25:30]),rtol=1e-6,atol=1e-6)
    assert torch.equal(before,torch.get_rng_state())
    def no_labels(*args):raise AssertionError("Prediction must not query target labels")
    monkeypatch.setattr(module,"DataService",no_labels)
    events=ids[25:30];prediction=model.predict_context(data,events)
    np.testing.assert_allclose(model.predict_context(data,list(reversed(events)))[::-1],prediction,rtol=1e-6,atol=1e-6)
...
def test_temporal_parameter_counts_are_comparable():
    torch=load_tensor_runtime()
    from copper_mvp.temporal_networks import TemporalNetwork
    counts=[sum(p.numel() for p in TemporalNetwork(m,TEMPORAL_ARCHITECTURES[m]).parameters())
