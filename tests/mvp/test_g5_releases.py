...
from copper_mvp.release_contracts import BASELINE_ID, TASK_ID
...
class Source:
    ...
    def load(self, actor, request):
        d = {
            ...
            "unit": "g/L" if request.target == "cu" else "mg/L",
            "task_id": TASK_ID,
            "benchmark": {
                "n": 100,
                "coverage": 1.0,
                "mae": 1 - self.improvement,
                "baseline_mae": 1.0,
                "paired_ci95": {"low": -0.7, "high": -0.5},
                "negative_predictions": 0,
                "p95_ms": 1.0,
                "mode_metrics": {"A": {"n": 100, "mae_mean_over_seeds": 1 - self.improvement}},
                "baseline_modes": {"A": {"n": 100, "mae_mean_over_seeds": 1.0}},
            },
    ...
    def verify(self, descriptor):
        if self.broken:
            raise WorkbenchError("synthetic artifact changed", "MODEL_HASH_MISMATCH")
        return True
    ...
    def event(self, event):
        return {
            "event_id": event,
            "decision_at": "2024-01-01T12:00:00+00:00",
            "snapshot_id": "synthetic-snapshot",
            "dataset_version": "synthetic",
            "current": {"cu": 10.0, "as": 1000.0},
        }

    def predict(self, descriptor, event):
        self.verify(descriptor)
        return (11.0 if descriptor["target"] == "cu" else 1100.0), {
            "artifact_sha256": "synthetic",
            "execution": "synthetic_model",
        }
...
def system(tmp_path):
    ...
    clock = [datetime(2024, 1, 2, tzinfo=UTC).timestamp()]
    source = Source()
...
def proposal(service, actor, artifact, shadow, key="release", version=0):
    return service.propose(
        actor,
        {
            ...
            "reason": "synthetic workflow verification",
        },
    )
...
def test_release_changes_new_predictions_and_rollback_preserves_old_records(system):
    ...
    rollback = service.rollback(
        owner, {"request_key": "rollback", "target": "cu", "expected_version": 1, "reason": "synthetic rollback"}
    )
...
def test_expired_revoked_and_tampered_evidence_cannot_apply(system):
    ...
    with service.connection() as c:
        c.execute(
            "UPDATE model_shadows SET evidence=? WHERE id=?", (json.dumps({"parity_passed": False}), shadow["id"])
        )
...
def test_reject_can_close_expired_proposal_and_preserves_applied_release(system):
    ...
    release = proposal(service, owner, artifact, shadow)
    clock[0] += 1000
    closed = service.approve(
        owner, release["id"], {"payload_hash": release["payload_hash"], "decision": "reject", "reason": "expired"}
    )
...
def test_release_api_identity_and_exact_pointer_flow(tmp_path, monkeypatch):
    ...
    with TestClient(create_app(run_dir=tmp_path / "api"), base_url="http://127.0.0.1") as client:
        ...
        proposed = client.post(
            "/api/v2/releases",
            json={
                "request_key": "api-release",
                "candidate_id": registered["id"],
                "shadow_id": shadow["id"],
                "target": "cu",
                "expected_version": 0,
                "reason": "synthetic API check",
            },
        )
        ...
        assert (
            client.post(
                "/api/v2/releases/" + release["id"] + "/approve",
                json={"payload_hash": release["payload_hash"]},
                headers=headers,
            ).status_code
            == 403
        )
        assert (
            client.post(
                "/api/v2/releases/" + release["id"] + "/approve", json={"payload_hash": release["payload_hash"]}
            ).json()["status"]
            == "applied"
        )
        prediction = client.post(
            "/api/v2/release-predictions", json={"request_key": "api-prediction", "event_id": "synthetic-event"}
        ).json()
        assert prediction["predictions"]["cu"]["value"] == 11
        rollback = client.post(
            "/api/v2/release-rollbacks",
            json={"request_key": "api-rollback", "target": "cu", "expected_version": 1, "reason": "synthetic rollback"},
        ).json()
        assert (
            client.post(
                "/api/v2/releases/" + rollback["id"] + "/approve", json={"payload_hash": rollback["payload_hash"]}
            ).json()["status"]
            == "applied"
        )
...
def test_missing_current_value_is_explicit_and_does_not_invent_prediction(system):
    ...
    source.event = lambda event: {**original(event), "current": {"cu": None, "as": 1000.0}}
...
def test_prediction_integrity_is_checked_on_read_and_idempotent_reuse(system):
    ...
    altered = {
        **output,
        "predictions": {**output["predictions"], "cu": {**output["predictions"]["cu"], "value": 999.0}},
    }
    with service.connection() as c:
...
def test_calibration_intervals_are_retained_in_immutable_prediction_records(system):
        ...
        return value, {
            **evidence,
            "prediction_intervals": {
                "marginal_c90": {"lower": 10.0, "upper": 12.0, "unit": "g/L", "joint_region": False}
            },
            "calibration": {
                "schema_version": "synthetic-calibration",
    ...
    assert result["predictions"]["cu"]["intervals"]["marginal_c90"]["lower"] == 10.0
    assert result["predictions"]["cu"]["calibration"]["nominal_coverage"] == 0.9
    assert (
        service.prediction(owner, result["id"])["predictions"]["cu"]["intervals"]
        == result["predictions"]["cu"]["intervals"]
    )
...
def test_target_pointers_only_form_joint_region_when_calibration_group_matches(system, shared_calibrator):
    ...
    def calibrated(descriptor, event):
        ...
        return value, {
            **evidence,
            "joint_prediction_region": {
                "id": group,
                "nominal_coverage": 0.9,
                "bounds": {
                    "cu": {"lower": 10, "upper": 12, "unit": "g/L"},
                    "as": {"lower": 1000, "upper": 1200, "unit": "mg/L"},
                },
            },
        }
    ...
    for target in ("cu", "as"):
        ...
        release = service.propose(
            owner,
            {
                "request_key": "joint-release-" + target,
                "candidate_id": artifact["id"],
                "shadow_id": shadow["id"],
                "target": target,
                "expected_version": 0,
