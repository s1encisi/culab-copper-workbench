...
from copper_mvp.model_registry import ComparisonPredictionRequest, ComparisonRequest
from copper_mvp.statistical_registry import SEQUENCE_METHODS
...
class SeriesData:
    def __init__(self, root):
        ...
        times = [datetime(2024, 1, 1, tzinfo=UTC)]
        for i in range(1, n):
            times.append(times[-1] + timedelta(hours=6 + (i % 3) * 2))
        self.feature_columns = [
            "origin_cu_g_l",
            "origin_as_mg_l",
            "stage3_current_a__t_minus_0h",
            "stage4_current_a__t_minus_0h",
        ] + [f"x{i}" for i in range(4, 114)]
        X = rng.normal(size=(n, 114))
        X[:, 2:4] = 100 + 10 * rng.normal(size=(n, 2))
        X[:, 110:] = rng.integers(0, 3, (n, 4))
        X[0, :2] = [40, 1000]
        for i in range(1, n):
            X[i, :2] = (
                np.array([40, 1000])
                + 0.85 * (X[i - 1, :2] - [40, 1000])
                + (X[i - 1, 2:4] - 100) * [0.01, 0.3]
                + rng.normal(size=2) * [0.05, 2.0]
            )
        self.X = pd.DataFrame(X, index=self.ids, columns=self.feature_columns)
        quality = np.ones(n, dtype=bool)
        quality[[20, 35, 65]] = False
        self.frame = pd.DataFrame(
            {
                "origin_event_id": self.ids,
                "decision_at": times,
                "origin_result_quality_eligible": quality,
                "stage3_current_a__t_minus_0h__age_h": np.zeros(n),
                "stage4_current_a__t_minus_0h__age_h": np.zeros(n),
            },
            index=self.ids,
        )
        self.admissions = {
            e: {"admission_status": "ADMITTED", "feature_cutoff_at": t.isoformat()}
            for e, t in zip(self.ids, times, strict=True)
        }
            ...
            for target, unit, j in (("cu", "g/L", 0), ("as", "mg/L", 1)):
                records.append(
                    LabelRecord(
                        ...
                        quality_eligible=e in self.primary_ids,
            ...
            [
                {
                    "origin_event_id": e,
                    "fold_id": "FOLD_1",
                    "fold_role": "TRAIN" if e in train else "VALIDATION",
                    "fold_fit_cutoff_at": times[60].isoformat(),
                }
                for e in train + list(self.fold_for)
            ]
        )
        sources = {}
        for name in (
    ...
    def training_ids(self, fold):
        return (
            [e for e in self.ids if e in self.primary_ids]
            if fold is None
            else self.cv.loc[self.cv.fold_role.eq("TRAIN"), "origin_event_id"].tolist()
        )
...
def test_sequence_keeps_positions_and_future_changes_do_not_leak(method, tmp_path):
    ...
    cutoff = data.row("event-060").decision_at
    ...
    assert all(np.array_equal(m.params, p) for m, p in zip(model.models, params, strict=True))
...
def test_contextual_training_comparison_and_api_prediction(tmp_path, monkeypatch):
    ...
    result = ComparisonService(tmp_path, data).predict(
        root.name, ComparisonPredictionRequest(event_id="event-070", method_id="VAR", include_uncertainty=True)
    )
...
def test_ets_missing_update_keeps_trained_noise_and_parameters(tmp_path):
    ...
    assert all(np.array_equal(a, r.params) for a, r in zip(parameters, model.models, strict=True))
...
def test_context_predictions_equal_separate_fixed_parameter_prefix_filters(method, tmp_path):
    ...
    seq = model._sequence
    ids, times, Y, exog = event_arrays(data, method == "SARIMAX")
    for i, event in enumerate(events):
        index = ids.index(event)
        end = index + 1
        ...
        if method == "SARIMAX":
            ...
            extra["exog"] = np.column_stack(
                (np.ones(1), seq.exog_scale.transform(seq.exog_imputer.transform(exog[end - 1 : end])))
            )
        expected = []
        for t, result in enumerate(seq.models):
            cloned = (
                result.model.clone(normalized[:, t], exog=regressors)
                if regressors is not None
                else result.model.clone(normalized[:, t])
            )
            filtered = cloned.filter(result.params)
            expected.append(
                float(np.asarray(filtered.get_forecast(steps=1, **extra).predicted_mean)[0]) * seq.scale_[t]
                + seq.mean_[t]
            )
