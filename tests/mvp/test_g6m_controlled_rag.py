...
pytestmark = pytest.mark.usefixtures("knowledge_runtime")
...
def document(client, text, readers=None):
    spec = {
        "doc_id": "controlled-sop",
        "title": "合成规程",
        "author": "fixture",
        "source": "synthetic fixture",
        "authorization": "local integration test",
        "license": "synthetic",
        "version_label": "v1",
        "format": "md",
...
def authorize(client, base, record):
    consent = {
        ...
        "parse_hash": record["index_parse_hash"],
    ...
    def provider(request):
        ...
        if not feedback:
            ...
            name, args = (
                ...
                {
                    "kind": "answer",
                    "answer": marker + "：根据已授权规程，浓度采用 g/L。" + answer_suffix,
...
def test_authorized_context_reaches_model_and_delete_removes_derived_answer(tmp_path, data):
    ...
    with TestClient(create_app(tmp_path, data), base_url="http://127.0.0.1") as client:
        ...
        client.app.state.workbench.research.transport = httpx.MockTransport(
            responder(sent, marker, answer_suffix=json_example)
        )
        task = client.post(
            f"/api/v2/sessions/{session}/messages", json={"question": "解释浓度单位", "request_key": "rag"}
        ).json()
...
def test_revocation_while_provider_is_answering_fences_the_result(tmp_path, data):
    ...
    with TestClient(create_app(tmp_path, data), base_url="http://127.0.0.1") as client:
        ...
        actor = workbench.access.authenticate(key=workbench.access.owner_key_path.read_text(encoding="utf-8").strip())
        ...
        task = client.post(
            f"/api/v2/sessions/{session}/messages", json={"question": "解释浓度单位", "request_key": "revoke"}
        ).json()
        result = wait_result(client, task["id"])
...
def test_consent_requires_owner_and_exact_document_parse(tmp_path, data):
    with TestClient(create_app(tmp_path, data), base_url="http://127.0.0.1") as client:
        ...
        assert (
            client.post(base + "/disclosures", json=payload, headers={"Authorization": "Bearer " + key}).status_code
            == 403
        )
    ...
    with TestClient(create_app(tmp_path, data), base_url="http://127.0.0.1") as client:
        ...
        for user in headers:
            ...
            task = client.post(
                ...
                json={"question": "解释浓度单位", "request_key": "reader-rag"},
...
def test_restart_cleans_answers_after_source_deleted_while_service_was_closed(tmp_path, data):
    ...
    with TestClient(create_app(tmp_path, data), base_url="http://127.0.0.1") as client:
        ...
        client.app.state.workbench.research.transport = httpx.MockTransport(responder([], marker))
        task = client.post(
            f"/api/v2/sessions/{session}/messages", json={"question": "解释浓度单位", "request_key": "restart"}
        ).json()
...
def test_expired_disclosure_does_not_send_text(tmp_path, data, monkeypatch):
    ...
    with TestClient(create_app(tmp_path, data), base_url="http://127.0.0.1") as client:
        ...
        monkeypatch.setattr(
            module, "timestamp", lambda value=None: real_timestamp(value) if value is not None else future
        )
        calls = []
        ...
        def provider(request):
            ...
            if not feedback:
                ...
                name, args = (
                    ...
                    {
                        "kind": "answer",
                        "answer": "本地原文：{{" + identifier + ".document_excerpt_1}}",
        ...
        task = client.post(
            f"/api/v2/sessions/{session}/messages", json={"question": "解释浓度单位", "request_key": "expired"}
        ).json()
        result = wait_result(client, task["id"])
    ...
    with TestClient(create_app(tmp_path, data), base_url="http://127.0.0.1") as client:
        ...
        def provider(request):
            ...
            if len(calls) == 1:
                ...
                name, args = (
                    "finish_answer",
                    {"kind": "clarification", "answer": "文档不能授予控制权限。", "evidence_ids": []},
                )
            return httpx.Response(
                200,
                json={
        ...
        client.app.state.workbench.research.transport = httpx.MockTransport(provider)
        task = client.post(
            f"/api/v2/sessions/{session}/messages", json={"question": "解释浓度单位", "request_key": "injection"}
        ).json()
        result = wait_result(client, task["id"])
        assert result["status"] == "completed"
        with client.app.state.workbench.store.connection() as c:
...
def test_model_cannot_cite_an_unshared_candidate_as_its_read_evidence(tmp_path, data):
    ...
    with TestClient(create_app(tmp_path, data), base_url="http://127.0.0.1") as client:
        ...
        assert (
            client.put(other + "/content", content=(denied_marker + "。电解液浓度单位为 mg/L。").encode()).status_code
            == 200
        )
        ...
        def provider(request):
            ...
            if len(calls) == 1:
                ...
                raw = next(
                    v["content"]
                    for v in payload["messages"]
                    if v["role"] == "user" and v["content"].startswith("以下是按准确版本")
                )
        ...
        client.app.state.workbench.research.transport = httpx.MockTransport(provider)
        task = client.post(
            f"/api/v2/sessions/{session}/messages", json={"question": "说明浓度单位", "request_key": "citation"}
        ).json()
        result = wait_result(client, task["id"])
        assert result["status"] == "completed", result.get("error")
