...
def responder(sent):
    def respond(request):
        ...
        if not feedback:
            name,args=("read_document",{"purpose":"读取评测资料"}) if docs else ("project_status",{"purpose":"核对目录"})
        else:
            assert "error" not in feedback[-1],feedback[-1]
            identifier=feedback[-1]["evidence_id"]
            name,args="finish_answer",{"kind":"answer","answer":"测试回答：已根据证据完成核对。","evidence_ids":[identifier]}
            if docs:args["document_refs"]=[{"evidence_id":identifier,"field":"document_excerpt"}]
        return httpx.Response(200,json={"model":"synthetic-provider","choices":[{"finish_reason":"tool_calls","message":{
            "role":"assistant","content":None,"tool_calls":[{"id":"fixture","type":"function","function":{"name":name,"arguments":json.dumps(args)}}]}}],
...
def test_domain_comparison_reuses_research_records_and_scoped_budget(tmp_path):
    with TestClient(create_app(tmp_path,DataRepository()),base_url="http://127.0.0.1") as client:
        wb=login(client);sent=[];wb.research.transport=httpx.MockTransport(responder(sent))
        request={"request_key":"domain-fixture"}
        created=client.post("/api/v2/domain-evaluations",json=request)
        assert created.status_code==201,created.text
        study=created.json();identifier=study["id"]
        ...
        actor=wb.access.authenticate(key=wb.access.owner_key_path.read_text().strip())
        ...
        reviewed=client.post("/api/v2/domain-evaluations/"+identifier+"/review",json={"verdict":"accepted","notes":"测试替身流程核对完成"})
        assert reviewed.status_code==200 and reviewed.json()["review"]["verdict"]=="accepted"
        saved=json.loads((tmp_path/"domain_evaluations"/identifier/"state.json").read_text())
        assert all("result" not in row for row in saved["cases"])
        reader=wb.access.issue(wb.access.authenticate(key=wb.access.owner_key_path.read_text().strip()),"reader","researcher")
        assert client.get("/api/v2/domain-evaluations/"+identifier,headers={"Authorization":"Bearer "+reader}).status_code==404

def test_domain_plan_is_local_until_model_execution_is_enabled(tmp_path):
    with TestClient(create_app(tmp_path,DataRepository()),base_url="http://127.0.0.1") as client:
        wb=login(client);wb.research.allow_live=False
        ...
        assert client.post("/api/v2/domain-evaluations",json={"request_key":"local-plan","title":"changed"}).status_code==409


def test_failed_case_does_not_abort_or_repeat_the_remaining_comparison(tmp_path):
    with TestClient(create_app(tmp_path,DataRepository()),base_url="http://127.0.0.1") as client:
        wb=login(client);sent=[];answer=responder(sent);calls=[]
        def provider(request):
            calls.append(1)
            if len(calls)==1:return httpx.Response(400,json={"error":{"message":"synthetic rejected request"}})
            return answer(request)
        wb.research.transport=httpx.MockTransport(provider)
        study=client.post("/api/v2/domain-evaluations",json={"request_key":"failed-baseline","questions":["Cu 的单位？"]}).json()
