...
def test_scalar_sweeps_charge_anchors_returns_and_every_joint_call(tmp_path, budget):
    request = OptimizerComparisonRequest(request_key="sweep", mode="benchmark", seeds=(17,), total_budget=budget,
    ...
    for run in comparison["results"]:
        ...
        for job in jobs:
            assert job["actual_evaluations"] == job["solver_nfev"] + job["return_verification_evaluations"] <= job["quota"]
...
def test_augmented_chebyshev_returns_an_unsupported_concave_front_point(tmp_path):
    ...
    def middle(run):
        return [r["returned"]["variables"][0] for r in run["scalarization"]["jobs"]
