...
from copper_mvp.model_registry import LEGACY_METHOD_IDS, METHOD_IDS, ComparisonRequest, catalog
...
def data(n=96):
    ...
    y = X[:, :2] + np.column_stack((0.2 * X[:, 2], 2 * X[:, 3])) + rng.normal(0, 0.02, (n, 2))
    return X, y
...
def test_new_uncertainty_is_saved_scored_and_returned_through_comparison(tmp_path, monkeypatch):
    ...
    answer = ComparisonService(tmp_path, data_set).predict(
        root.name, ComparisonPredictionRequest(event_id=event, method_id="BayesianRidge", include_uncertainty=True)
    )
    assert answer["uncertainty"]["kind"] == "marginal_standard_deviation"
    with (root / "oof_uncertainty.csv").open("a") as stream:
        stream.write("\n")
